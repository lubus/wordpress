...
class WC_Swatches_Product_Attribute_Images {
    ...
    public function __construct($attribute_image_key = 'thumbnail_id', $image_size = 'shop_thumb') {
        ...
        if (is_admin()) {
            ...
            add_action('created_term', array(&$this, 'eshopbox_attribute_thumbnail_field_save'), 10, 3);
            add_action('edit_term', array(&$this, 'eshopbox_attribute_thumbnail_field_save'), 10, 3);
        }
    }

    //Enqueue the scripts if on a product attribute page
    public function on_admin_scripts() {
        global $eshopbox_swatches;
    ...
    public function init_attribute_image_selector() {
        ...
        if (strpos($screen->id, 'pa_') !== false) :
            ...
            if (taxonomy_exists($_REQUEST['taxonomy'])) {
                ...
                $this->image_size = apply_filters('eshopbox_get_swatches_image_size', $this->image_size, $_REQUEST['taxonomy'], $term_id);
            }
            ...
            if ($attribute_taxonomies) {
                foreach ($attribute_taxonomies as $tax) {
                    ...
                    add_action('pa_' . $tax->attribute_name . '_add_form_fields', array(&$this, 'eshopbox_add_attribute_thumbnail_field'));
                    add_action('pa_' . $tax->attribute_name . '_edit_form_fields', array(&$this, 'eshopbox_edit_attributre_thumbnail_field'), 10, 2);

                    add_filter('manage_edit-pa_' . $tax->attribute_name . '_columns', array(&$this, 'eshopbox_product_attribute_columns'));
                    add_filter('manage_pa_' . $tax->attribute_name . '_custom_column', array(&$this, 'eshopbox_product_attribute_column'), 10, 3);
                }
            }
    ...
    public function eshopbox_add_attribute_thumbnail_field() {
        global $eshopbox;
        ?>
        ...
        <div class="form-field swatch-field swatch-field-photo" style="overflow:visible;display:none;">
            <div id="swatch-photo" class="<?php echo sanitize_title($this->meta_key); ?>-photo">
                ...
                <div id="product_attribute_thumbnail_<?php echo $this->meta_key; ?>" style="float:left;margin-right:10px;">
                    <img src="<?php echo $eshopbox->plugin_url() . '/assets/images/placeholder.png' ?>" width="<?php echo $this->image_width; ?>px" height="<?php echo $this->image_height; ?>px" />
                </div>
                ...
                <script type="text/javascript">
                    ...
                    jQuery('.remove_image_button').live('click', function(){
                        jQuery('#product_attribute_thumbnail_<?php echo $this->meta_key; ?> img').attr('src', '<?php echo $eshopbox->plugin_url() . '/assets/images/placeholder.png'; ?>');
    ...
    public function eshopbox_edit_attributre_thumbnail_field($term, $taxonomy) {
        global $eshopbox;
    ...
    public function eshopbox_attribute_thumbnail_field_save($term_id, $tt_id, $taxonomy) {
        if (isset($_POST['product_attribute_meta'])) {
            ...
            if (isset($metas[$this->meta_key])) {
                ...
                update_eshopbox_term_meta($term_id, $taxonomy . '_' . $this->meta_key . '_type', $type);
                update_eshopbox_term_meta($term_id, $taxonomy . '_' . $this->meta_key . '_photo', $photo);
                update_eshopbox_term_meta($term_id, $taxonomy . '_' . $this->meta_key . '_color', $color);
            }
        }
    }

    //Registers a column for this attribute taxonomy for this image
    public function eshopbox_product_attribute_columns($columns) {
        $new_columns = array();
    ...
    //Renders the custom column as defined in eshopbox_product_attribute_columns
    public function eshopbox_product_attribute_column($columns, $column, $id) {
