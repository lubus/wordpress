...
class WC_magicshipment{
    ...
    public function __construct(){
     //   register_activation_hook( __FILE__, array( $this, 'createTables' ));
        add_action('eshopbox_order_status_processing', array( &$this, 'change_order_status' )); 
        add_action('load-edit.php', array( &$this, 'change_order_status_fromlisting' )); 
     //   add_action('load-edit.php', array( &$this, 'eshopbox_pip_assignno' ));
     //    add_action('eshopbox_order_status_processing', array( &$this, 'eshopbox_pip_assignno')); 
      add_action('admin_menu', array( &$this, 'eshopbox_magicshipment_admin_menu' )); 
    ...
    function eshopbox_magicshipment_admin_menu() {
...
update_option( 'shipping_provider_priority', $income_data );
  ...
  function eshopbox_pip_assignno($order_id){
...
//	    return get_post_meta( $order_id, '_pip_invoice_date', true );
    ...
    public function config_options(){
     global $wpdb;
   //update_option( 'shipping_provider_priority', 'Delhivery-Bluedart-indiapost' );
   ?>
...
<body>
    <?php
    //update_option( 'shipping_provider_priority', 'Delhivery-Bluedart');
...
<!--<li id="indiapost" class="ui-state-default"><span class="ui-icon ui-icon-arrowthick-2-n-s"></span>India Post</li>-->
...
<?php
    ...
    } //echo '<li id="delhivery" class="ui-state-default"><span class="ui-icon ui-icon-arrowthick-2-n-s"></span>delhivery</li>';
  ...
  // exit;
    ...
    if($getPriority==''){   
     ...
     // $income_data =  lcfirst('Delhivery-Aramex-Bluedart');
         $income_data =  lcfirst('Delhivery-Bluedart');
...
//            if($provide=='indiapost'){
//                                  //  echo 'test12'; exit;
//                $trackingNum = '111111';
//                $_POST['tracking_number'] = $trackingNum;
//                $_POST['tracking_provider']='india-post';
//                update_post_meta($order_id, '_tracking_number',$trackingNum);
//                pdate_post_meta($order_id,'_tracking_provider','india-post');
//                }
        ...
        if($insertT){
            if($provide=='bluedart'){
             //   echo 'test'; die('done');
                ...
                update_post_meta($order_id, '_tracking_number',$trackingNum);
                update_post_meta($order_id,'_tracking_provider','blue-dart');
                }
             ...
             if($provide=='aramex'){                
                ...
                update_post_meta($order_id, '_tracking_number',$trackingNum);
                update_post_meta($order_id,'_tracking_provider','aramex');
              ...
              if($provide=='delhivery'){
                ...
                $_POST['tracking_provider']='delhivery';
                update_post_meta($order_id, '_tracking_number',$trackingNum);
                update_post_meta($order_id,'_tracking_provider','delhivery');
                }    
...
//                $_POST['tracking_provider']='india-post';
//                update_post_meta($order_id, '_tracking_number',$trackingNum);
//                update_post_meta($order_id,'_tracking_provider','india-post');
//                }
         // $trackingNum =  $this->delhiveryshipment();
        } else {
...
//echo "SELECT count(*) as  aramexcount FROM ".$wpdb->prefix."cod_aramex WHERE pincode = ".$zipcode;
...
//echo 'fsdfsdfasd';
   ...
   function bluedartShipment($theorder){
       global $eshopbox, $post,$wpdb;
      if($theorder->payment_method=='cod'){
   //echo "SELECT * FROM ".$wpdb->prefix."bluedart_codawb where assigned='0' and `lock`=0"; exit;       
   ...
   //   $_POST['tracking_provider']='blue-dart';
               if($theorder->payment_method=='cod'){
                                           $wpdb->get_row( "update ".$wpdb->prefix."bluedart_codawb set `assigned`='1',`orderid`='".$order_id."',`lock`='1' where `CAWBNO`='".trim($trackingNum)."'") ;
                                     
                                             } else {
                                        $wpdb->get_row("update ".$wpdb->prefix."bluedart_prepaidawb set `assigned`='1',`orderid`='".$order_id."', `lock`='1' where `CAWBNO`='".trim($trackingNum)."'") ;           
                                             }   
      return $trackingNum;
      exit;
   } 
   ...
   function delhiveryshipment(){
       //commented /**/ need to uncomment
       $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, 'http://track.delhivery.com/waybill/api/fetch/json/?token=8d24c18bf79d6be642676f63112d3729b62a9cd7'); 
        //curl_setopt($ch, CURLOPT_URL, 'http://track.delhivery.com/waybill/api/fetch/json/?token=b1ab4801e5f294c7d332281656839993434775cc');
 ...
 function aramexShipments($theorder) {
	global $eshopbox, $post,$wpdb;
       ...
       // echo '<pre>';print_r($theorder);die('test');
        $orderid=$theorder->id;
