<?php
/*
Plugin Name: EshopBox CCAvenue gateway
Plugin URI: http://www.mrova.com/
Description: Extends EshopBox with mrova ccavenue gateway.
Version: 1.2.2
Author: mRova
...
add_action('plugins_loaded', 'eshopbox_mrova_ccave_init', 0);

function eshopbox_mrova_ccave_init() {
    ...
    protected $msg = array();
        public function __construct(){
            ...
            //update for eshopbox >2.0
            add_action( 'eshopbox_api_' . strtolower( get_class( $this ) ), array( $this, 'check_ccavenue_response' ) );
            ...
            add_action('eshopbox_receipt_ccavenue', array(&$this, 'receipt_page'));
            add_action('eshopbox_thankyou_ccavenue',array(&$this, 'thankyou_page'));
        }
        ...
        function process_payment($order_id){
            ...
            return array('result' => 'success', 'redirect' => add_query_arg('order',
                $order->id, add_query_arg('key', $order->order_key, get_permalink(get_option('eshopbox_pay_page_id'))))
            );
        }
        /**
...
jQuery(function(){
    jQuery("body").block(
            {
                message: "<img src=\"'.$eshopbox->plugin_url().'/assets/images/ajax-loader.gif\" alt=\"Redirecting…\" style=\"float:left; margin-right: 10px;\" />'.__('Thank you for your order. We are now redirecting you to CCAvenue to make payment.', 'mrova').'",
                    overlayCSS:
            {
                background: "#fff",
                    opacity: 0.6
     ...
     **/
    function eshopbox_add_mrova_ccave_gateway($methods) {
        $methods[] = 'WC_Mrova_Ccave';
        return $methods;
    }

    add_filter('eshopbox_payment_gateways', 'eshopbox_add_mrova_ccave_gateway' );
    }
