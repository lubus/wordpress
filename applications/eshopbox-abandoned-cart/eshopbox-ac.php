<?php
/*
Plugin Name: Eshopbox Abandoned Cart
...
// Deletion Settings
register_uninstall_hook( __FILE__, 'eshopbox_ac_delete');
if ( !defined('ABONDONED_CART_PATH') )
define( 'ABONDONED_CART_PATH', plugin_dir_path( __FILE__ ) );
function eshopbox_ac_delete(){
...
//if (is_eshopbox_active())
{
	 ...
	 * Localisation
	 **/
	load_plugin_textdomain('eshopbox-ac', false, dirname( plugin_basename( __FILE__ ) ) . '/');

	/**
	 * eshopbox_abandon_cart class
	 **/
	if (!class_exists('eshopbox_abandon_cart')) {

		class eshopbox_abandon_cart {
			...
			public function __construct() {
				...
				// Initialize settings
				register_activation_hook( __FILE__, array(&$this, 'eshopbox_ac_activate'));

				// BoxBeat Administration Menu
				add_action('admin_menu', array(&$this, 'eshopbox_ac_admin_menu'));

				// Actions to be done on cart update
				add_action('eshopbox_cart_updated', array(&$this, 'eshopbox_ac_store_cart_timestamp'));

				// delete added temp fields after order is placed
				add_filter('eshopbox_order_details_after_order_table', array(&$this, 'action_after_delivery_session'));
			...
			function eshopbox_ac_admin_menu(){

				$page = add_submenu_page('eshopbox', __( 'Abandoned Carts', 'eshopbox-ac' ), __( 'Abandoned Carts', 'eshopbox-ac' ), 'manage_eshopbox', 'eshopbox_ac_page', array(&$this, 'eshopbox_ac_page' ));

			}

			function eshopbox_ac_store_cart_timestamp() {
				...
				$cut_off_time = json_decode(get_option('eshopbox_ac_settings'));
				...
				if ( count($results) == 0 )
				{
					$cart_info = json_encode(get_user_meta($user_id, '_eshopbox_persistent_cart', true));
				...
				{
					$updated_cart_info = json_encode(get_user_meta($user_id, '_eshopbox_persistent_cart', true));
					if (! $this->compare_carts( $user_id, $results[0]->abandoned_cart_info) )
					{
						...
						update_user_meta($user_id, '_eshopbox_ac_modified_cart', md5("yes"));
					}
					else
					{
						update_user_meta($user_id, '_eshopbox_ac_modified_cart', md5("no"));
					}
				}
				else
				{
			...
			{
				$current_woo_cart = get_user_meta($user_id, '_eshopbox_persistent_cart', true);
			...
			function action_after_delivery_session( $order ) {
				...
				global $wpdb;
				$user_id = get_current_user_id();
				delete_user_meta($user_id, '_eshopbox_ac_persistent_cart_time');
				delete_user_meta($user_id, '_eshopbox_ac_persistent_cart_temp_time');
				...
				if ( get_user_meta($user_id, '_eshopbox_ac_modified_cart', true) == md5("yes") ||
						get_user_meta($user_id, '_eshopbox_ac_modified_cart', true) == md5("no") )
				{
					...
					delete_user_meta($user_id, '_eshopbox_ac_modified_cart');
				}
				else
				{
...
global $wpdb;
				...
				if ($action == 'emailtemplates')
                                {
                                    ...
                                    $check_results = mysql_num_rows(mysql_query($check_query));
                                    if($check_results==0)
                                        {
                                            $active=1;
                                            $query = 'INSERT INTO '.$wpdb->prefix.'ac_email_templates(is_active,frequency, template_name,day_or_hour)
                                            VALUES ('.$active.',1,"Abondoned cart Template","Days")';
                                            //echo $query;
                                            mysql_query($query);
                                        }
                                }
				...
				<div style="background-image: url('<?php echo plugins_url(); ?>/eshopbox-abandoned-cart/images/ac_tab_icon.png') !important;" class="icon32"><br></div>
				<!--<span class="mce_abandoncart_email_variables"><br></span>-->

				<h2 class="nav-tab-wrapper woo-nav-tab-wrapper">
				<a href="admin.php?page=eshopbox_ac_page&action=listcart" class="nav-tab <?php echo $active_listcart; ?>"> <?php _e( 'Abandoned Orders', 'eshopbox-ac' );?> </a>
				<a href="admin.php?page=eshopbox_ac_page&action=emailtemplates" class="nav-tab <?php echo $active_emailtemplates; ?>"> <?php _e( 'Email Templates', 'eshopbox-ac' );?> </a>
				<a href="admin.php?page=eshopbox_ac_page&action=emailsettings" class="nav-tab <?php echo $active_settings; ?>"> <?php _e( 'Settings', 'eshopbox-ac' );?> </a>
				<a href="admin.php?page=eshopbox_ac_page&action=stats" class="nav-tab <?php echo $active_stats; ?>"> <?php _e( 'Recovered Orders', 'eshopbox-ac' );?> </a>
				</h2>
			...
			function my_enqueue_scripts_js( $hook ) {
				...
				if ( $hook != 'eshopbox_page_eshopbox_ac_page' )
				{
					return;
				}
				else
				{
					...
					wp_register_script( 'eshopbox_admin', plugins_url() . '/eshopbox/assets/js/admin/eshopbox_admin.js', array('jquery', 'jquery-ui-widget', 'jquery-ui-core'));
					...
					function delete_email_template( id )
					{
						...
						if(y==true)
						{
							location.href='admin.php?page=eshopbox_ac_page&action=emailtemplates&mode=removetemplate&id='+id;
					    }
					}
				    </script>
				    <!-- /////////////////////////////////////////////////////////////// -->
				    ...
				    <?php
				    wp_enqueue_script('tinyMCE_ac', plugins_url() . '/eshopbox-abandoned-cart/js/tinymce/jscripts/tiny_mce/tiny_mce.js');
				    wp_enqueue_script('ac_email_variables', plugins_url() . '/eshopbox-abandoned-cart/js/abandoncart_plugin_button.js');
				    ?>
			...
			function tinyMCE_ac(){
				...
				tinyMCE.init({
					theme : "advanced",
					mode: "exact",
					elements : "eshopbox_ac_email_body",
			...
			function my_enqueue_scripts_css( $hook ) {
				...
				else
				{
					...
					wp_enqueue_style( 'eshopbox_admin_styles', plugins_url() . '/eshopbox/assets/css/admin.css' );
					...
					{
					    background-image: url("<?php echo plugins_url(); ?>/eshopbox-abandoned-cart/images/ac_editor_icon.png") !important;
			...
			function eshopbox_ac_page()
			{
				...
				{
					wp_die( __( 'You do not have sufficient permissions to access this page.', 'eshopbox-ac' ) );
				}

				?>

					<div class="wrap">
						<div class="icon32" style="background-image: url('<?php echo plugins_url(); ?>/eshopbox-abandoned-cart/images/abandoned_cart_small.png') !important;">
							<br />
						</div>
							<h2><?php _e( 'EshopBox - Abandon Cart Lite', 'eshopbox-ac' ); ?></h2>
					<?php
					...
					if ($action == 'emailsettings')
					{
						...
						if ( isset( $_POST['ac_settings_frm'] ) && $_POST['ac_settings_frm'] == 'save' )
						{
							...
							$eshopbox_ac_settings = json_encode($woo_ac_settings);

							update_option('eshopbox_ac_settings',$eshopbox_ac_settings);
						}
						?>

							<?php if ( isset( $_POST['ac_settings_frm'] ) && $_POST['ac_settings_frm'] == 'save' ) { ?>
							<div id="message" class="updated fade"><p><strong><?php _e( 'Your settings have been saved.', 'eshopbox-ac' ); ?></strong></p></div>
							<?php } ?>

							<?php
								//$enable_email_sett = array();
								$enable_email_sett = json_decode(get_option('eshopbox_ac_settings'));
								?>
							<div id="content">
							  <form method="post" action="" id="ac_settings">
								  ...
								  <div id="poststuff">
										<div class="postbox">
											<h3 class="hndle"><?php _e( 'Settings', 'eshopbox-ac' ); ?></h3>
											<div>
											  <table class="form-table">

				    							<tr>
				    								<th>
				    									<label for="eshopbox_ac_email_frequency"><b><?php _e( 'Cart abandoned cut-off time', 'eshopbox-ac' ); ?></b></label>
				    								</th>
				    								<td>
														<?php
														$cart_time = "";
				    									...
				    									';?>
				    									<img class="help_tip" width="16" height="16" data-tip='<?php _e( 'Consider cart abandoned after X minutes of item being added to cart & order not placed', 'eshopbox') ?>' src="<?php echo plugins_url(); ?>/eshopbox/assets/images/help.png" /></p>
				    									<!-- <span class="description"><?php
				    										_e( 'Consider cart abandoned after X minutes of item being added to cart & order not placed', 'eshopbox-ac' );
				    									?></span> -->
				    								</td>
				    							</tr>
							  ...
							  <p class="submit">
								<input type="submit" name="Submit" class="button-primary" value="<?php esc_attr_e( 'Save Changes', 'eshopbox-ac' ); ?>" />
							  </p>
						    </form>
						  </div>
						<?php
			...
			<p> <?php _e('The list below shows all Abandoned Carts which have remained in cart for a time higher than the "Cart abandoned cut-off time" setting.', 'eshopbox-ac');?> </p>
			...
			if($count > 0) {
				...
				$p->target("admin.php?page=eshopbox_ac_page&action=listcart");
			...
			$ac_cutoff_time = json_decode(get_option('eshopbox_ac_settings'));
			...
			?>
						...
						<tr>
							<th> <?php _e( 'Customer', 'eshopbox-ac' ); ?> </th>
							<th> <?php _e( 'Order Total', 'eshopbox-ac' ); ?> </th>
							<th scope="col" id="date_ac" class="manage-column column-date_ac sorted <?php echo $order;?>" style="">
								<a href="admin.php?page=eshopbox_ac_page&action=listcart&orderby=abandoned_cart_time&order=<?php echo $order_next;?>">
									<span> <?php _e( 'Date', 'eshopbox-ac' ); ?> </span>
									<span class="sorting-indicator"></span>
								</a>
							</th>
							<th scope="col" id="status_ac" class="manage-column column-status_ac sorted <?php echo $order;?>" style="">
								<a href="admin.php?page=eshopbox_ac_page&action=listcart&orderby=cart_ignored&order=<?php echo $order_next;?>">
									<span> <?php _e( 'Status', 'eshopbox-ac' ); ?> </span>
									<span class="sorting-indicator"></span>
								</a>
							</th>
							<th> <?php _e( 'Actions', 'eshopbox-ac' ); ?> </th>
						</tr>
				...
				<?php
						foreach ($results as $key => $value)
						{
							...
							<tr id="row_<?php echo $abandoned_order_id; ?>">
								...
								<td><?php echo get_eshopbox_currency_symbol()." ".$line_total; ?></td>
								<td><?php echo $order_date; ?></td>
								<td><?php echo $ac_status; ?>
								<td id="<?php echo $abandoned_order_id; ?>">
								<?php echo "<a href='#' id='$abandoned_order_id-$user_id' class='remove_cart'> <img src='".plugins_url()."/eshopbox-abandoned-cart/images/delete.png' alt='Remove Cart Data' title='Remove Cart Data'></a>"; ?>
								&nbsp;
					...
					elseif ($action == 'emailtemplates' && ($mode != 'edittemplate' && $mode != 'addnewtemplate' ) )
					{
							...
							<p> <?php //_e('Add email templates at different intervals to maximize the possibility of recovering your abandoned carts.', 'eshopbox-ac');?> </p>
							...
							echo 	$active_post = (empty($_POST['is_active'])) ? '0' : '1';
								//$active_post = 1;
                                                        //echo $active_post;
                                                      //  echo "<pre>";print_r()
								if ( $active_post == 1 )
								{
									...
									if (count($check_results) == 0 )
									{
										...
										VALUES ('".$_POST[eshopbox_ac_email_subject]."',
												'".$_POST[eshopbox_ac_email_body]."',
												'".$active_post."',
												'".$_POST[email_frequency]."',
												'".$_POST[day_or_hour]."',
												'".$_POST[eshopbox_ac_template_name]."',
												'".$_POST[eshopbox_ac_from_name]."' )";
										//echo $query;
										mysql_query($query);
									}
									else
							...
							if ( isset( $_POST['ac_settings_frm'] ) && $_POST['ac_settings_frm'] == 'update' )
							{
								...
								if ( $active == 1 )
								{
									...
									if (count($check_results) == 0 )
									{
										$query_update = "UPDATE `".$wpdb->prefix."ac_email_templates`
										SET
										subject='".$_POST[eshopbox_ac_email_subject]."',
										body='".$_POST[eshopbox_ac_email_body]."',
										is_active='".$active."', frequency='".$_POST[email_frequency]."',
										day_or_hour='".$_POST[day_or_hour]."',
										template_name='".$_POST[eshopbox_ac_template_name]."',
										from_name='".$_POST[eshopbox_ac_from_name]."'
										WHERE id='".$_POST[id]."' ";
										mysql_query($query_update);
									}
									else
							...
							if ( isset( $_POST['ac_settings_frm'] ) && $_POST['ac_settings_frm'] == 'save' ) { ?>
							<div id="message" class="updated fade"><p><strong><?php _e( 'The Email Template has been successfully added.', 'eshopbox-ac' ); ?></strong></p></div>
							<?php }
							if ( isset( $_POST['ac_settings_frm'] ) && $_POST['ac_settings_frm'] == 'update' ) { ?>
							<div id="message" class="updated fade"><p><strong><?php _e( 'The Email Template has been successfully updated.', 'eshopbox-ac' ); ?></strong></p></div>
							<?php }?>

							<div class="tablenav">
							<p style="float:left;">
                                                        <strong> Edit Email Template to maximize the possibility of recovering your abandoned carts  </strong>
							
<!--                                                        <input type="button" value="+ Add New Template" id="add_new_template" onclick="location.href='admin.php?page=eshopbox_ac_page&action=emailtemplates&mode=addnewtemplate';" style="font-weight: bold; color: green; font-size: 18px; cursor: pointer;">-->
							<!--<a href="admin.php?page=eshopbox_ac_page&action=emailtemplates&mode=addnewtemplate">Add New Template</a>-->
				...
				if($count > 0) {
					...
					$p->items($count);
					$p->limit(10); // Limit entries per page
					$p->target("admin.php?page=eshopbox_ac_page&action=emailtemplates");
					$p->currentPage($_GET[$p->paging]); // Gets and validates the current page
					$p->calculate(); // Calculates what to show
					$p->parameterName('paging');
					$p->adjacents(1); //No. of page away from the current page
				...
				?>
						...
						<tr>
							<th> <?php _e( 'Sr', 'eshopbox-ac' ); ?> </th>
							<th scope="col" id="temp_name" class="manage-column column-temp_name sorted <?php echo $order;?>" style="">
								<a href="admin.php?page=eshopbox_ac_page&action=emailtemplates&orderby=template_name&order=<?php echo $order_next;?>">
									<span> <?php _e( 'Template Name', 'eshopbox-ac' ); ?> </span>
									<span class="sorting-indicator"></span>
								</a>
							</th>
							<th scope="col" id="sent" class="manage-column column-sent sorted <?php echo $order;?>" style="">
								<a href="admin.php?page=eshopbox_ac_page&action=emailtemplates&orderby=frequency&order=<?php echo $order_next;?>">
									<span> <?php _e( 'Sent', 'eshopbox-ac' ); ?> </span>
									<span class="sorting-indicator"></span>
								</a>
							</th>
							<th> <?php _e( 'Active ?', 'eshopbox-ac' ); ?> </th>
							<th> <?php _e( 'Actions', 'eshopbox-ac' ); ?> </th>
						</tr>
						...
						foreach ($results as $key => $value)
						{
								...
								<td>
									<a href="admin.php?page=eshopbox_ac_page&action=emailtemplates&mode=edittemplate&id=<?php echo $id; ?>">Settings</a>&nbsp;
<!--									<a href="#" onclick="delete_email_template( <?php //echo $id; ?> )" > <img src="<?php echo plugins_url(); ?>/eshopbox-abandoned-cart/images/delete.png" alt="Delete" title="Delete" width="20" height="20"> </a>&nbsp;-->
									<a href="<?php echo admin_url().'/admin.php?page=eshopbox_settings&tab=email&section=WC_Email_Abondoned_Cart';?>" title="Edit">Edit Template</a>&nbsp;
								</td>
					...
					elseif ($action == 'stats' || $action == '')
					{
						...
						//location.href= 'admin.php?page=eshopbox_ac_page&action=stats&durationselect='+group_name;
						});
						});
						</script>
						<?php
						...
						<div id="recovered_stats" class="postbox" style="display:block">
							...
							<div class="inside">
							<form method="post" action="admin.php?page=eshopbox_ac_page&action=stats" id="ac_stats">
							...
							<label class="start_label" for="start_day"> <?php _e( 'Start Date:', 'eshopbox-ac' ); ?> </label>
							<input type="text" id="start_date" name="start_date" readonly="readonly" value="<?php echo $start_date_range; ?>"/>

							<label class="end_label" for="end_day"> <?php _e( 'End Date:', 'eshopbox-ac' ); ?> </label>
							<input type="text" id="end_date" name="end_date" readonly="readonly" value="<?php echo $end_date_range; ?>"/>

							<input type="submit" name="Submit" class="button-primary" value="<?php esc_attr_e( 'Go', 'eshopbox-ac' ); ?>"  />
							</form>
							</div>
						</div>
						<?php
						...
						if($count > 0) {
							...
							$p->items($count);
							$p->limit(10); // Limit entries per page
							$p->target("admin.php?page=eshopbox_ac_page&action=stats&duration_select=$duration_range");
							$p->currentPage($_GET[$p->paging]); // Gets and validates the current page
							$p->calculate(); // Calculates what to show
							$p->parameterName('paging');
							$p->adjacents(1); //No. of page away from the current page
						...
						foreach ( $ac_results as $key => $value)
						{
							...
							/*	$order_items = unserialize($rec_order['_order_items'][0]);
								...
								$table_data .="<tr>
											  ...
											  <td>".get_eshopbox_currency_symbol()." ".$rec_order['_order_total'][0]."</td>
						...
						<div id="recovered_stats" class="postbox" style="display:block">
						<div class="inside" >
						<p style="font-size: 15px"> During the selected range <strong><?php echo $count_carts; ?> </strong> carts totaling <strong><?php echo get_eshopbox_currency_symbol()." ".$total_value; ?></strong> were abandoned. We were able to recover <strong><?php echo $recovered_item; ?></strong> of them, which led to an extra <strong><?php echo get_eshopbox_currency_symbol()." ".$recovered_total; ?></strong> in sales</p>
						</div>
						</div>
						...
						<table class='wp-list-table widefat fixed posts' cellspacing='0' id='cart_data'>
												...
												<th scope="col" id="created_date" class="manage-column column-created_date sorted <?php echo $order;?>" style="">
													<a href="admin.php?page=eshopbox_ac_page&action=stats&orderby=abandoned_cart_time&order=<?php echo $order_next;?>&durationselect=<?php echo $duration_range;?>">
														<span> <?php _e( 'Created On', 'eshopbox-ac' ); ?> </span>
														<span class="sorting-indicator"></span>
													</a>
												</th>
												<th scope="col" id="rec_order" class="manage-column column-rec_order sorted <?php echo $order;?>" style="">
													<a href="admin.php?page=eshopbox_ac_page&action=stats&orderby=recovered_cart&order=<?php echo $order_next;?>&durationselect=<?php echo $duration_range;?>">
														<span> <?php _e( 'Recovered Date', 'eshopbox-ac' ); ?> </span>
														<span class="sorting-indicator"></span>
													</a>
												</th>
												<th> <?php _e( 'Order Total', 'eshopbox-ac' ); ?> </th>
				...
				if ( $action == 'emailtemplates' && ($mode == 'addnewtemplate' || $mode == 'edittemplate' ))
				{
					...
					$active_post = (empty($_POST['is_active'])) ? '0' : '1';
						...
						?>
							...
							<div id="content">
							  <form method="post" action="admin.php?page=eshopbox_ac_page&action=emailtemplates" id="ac_settings">
							  ...
							  <?php
								$button_mode = "save";
								$display_message = "Add Email Settings";
								if ( $mode == 'edittemplate' )
								{
									$button_mode = "update";
									$display_message = "Edit Email Settings";
								}
								  print'<input type="hidden" name="ac_settings_frm" value="'.$button_mode.'">';?>
								  <div id="poststuff">
										<div class="postbox">
											<h3 class="hndle"><?php _e( $display_message, 'eshopbox-ac' ); ?></h3>
											<div>
											  <table class="form-table" id="addedit_template">

												<tr>
													<th>
														<label for="eshopbox_ac_template_name"><b><?php _e( 'Template Name:', 'eshopbox-ac ');?></b></label>
													</th>
													<td>
													<?php
													$template_name = "";
													...
													print'<input type="text" name="eshopbox_ac_template_name" id="eshopbox_ac_template_name" class="regular-text" value="'.$template_name.'">';?>
													<img class="help_tip" width="16" height="16" data-tip='<?php _e('Enter a template name for reference', 'eshopbox') ?>' src="<?php echo plugins_url(); ?>/eshopbox/assets/images/help.png" /></p>
												</tr>

				    							<tr>
				    								<th>
				    									<label for="eshopbox_ac_email_frequency"><b><?php _e( 'Send this email:', 'eshopbox-ac' ); ?></b></label>
				    								</th>
				    								<td>
				    									...
				    									<span class="description"><?php
				    									echo __( 'after cart is abandoned.', 'eshopbox-ac' );
				    									?></span>
				    								</td>
				    							</tr>
				    																		</table>
							  ...
							  <p class="submit">
								...
								<input type="submit" name="Submit" class="button-primary" value="<?php esc_attr_e( $button_value, 'eshopbox-ac' ); ?>"  />
							  </p>
						    </form>
						  </div>
						<?php
					...
					function my_action_send_preview()
					{
						?>
							...
							jQuery(document).ready(function($)
							{
								...
								{
									//alert('hello there');
									var from_name_preview = $('#eshopbox_ac_from_name').val();
									var subject_email_preview = $('#eshopbox_ac_email_subject').val();
									...
									$.post(ajaxurl, data, function(response)
									{
										$("#preview_email_sent_msg").html("<img src='<?php echo plugins_url(); ?>/eshopbox-abandoned-cart/images/check.jpg'>&nbsp;Email has been sent successfully.");
		...
		$eshopbox_abandon_cart = new eshopbox_abandon_cart();

}
function eshop_abondoned_cart_email_ui($classobj){

    include_once( ABONDONED_CART_PATH.'class-wc-email-abondoned-cart.php' );
     $classobj['WC_Email_Abondoned_Cart'] = new WC_Email_Abondoned_Cart();
     return $classobj;
}

add_filter('eshopbox_email_classes','eshop_abondoned_cart_email_ui',1,3);
?>