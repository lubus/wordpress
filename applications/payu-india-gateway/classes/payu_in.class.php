...
class WC_Gateway_Payu_In extends WC_Payment_Gateway {
	...
	public function __construct() {
        ...
        $this->id				= 'payu_in';
        $this->method_title = __( 'PayU India', 'eshopbox_payu_in' );
		...
		if ( get_option( 'eshopbox_currency' ) == 'INR' ) {
		...
		// Receipt
		add_action( 'eshopbox_receipt_payu_in', array( $this, 'receipt_page' ) );

		/* 1.6.6 */
		add_action( 'eshopbox_update_options_payment_gateways', array( $this, 'process_admin_options' ) );
	...
	*/
	function init_form_fields() {
		$this->form_fields = array(
			'enabled' => array(
							'title' => __( 'Enable/Disable', 'eshopbox_payu_in' ),
							'type' => 'checkbox',
							'label' => __( 'Enable PayU India', 'eshopbox_payu_in' ),
							'default' => 'yes'
						),
			'title' => array(
							'title' => __( 'Title', 'eshopbox_payu_in' ),
							'type' => 'text',
							'description' => __( 'This controls the title which the user sees during checkout.', 'eshopbox_payu_in' ),
							'default' => __( 'PayU', 'eshopbox_payu_in' )
						),
			'description' => array(
							'title' => __( 'Description', 'eshopbox_payu_in' ),
							'type' => 'textarea',
							'description' => __( 'This controls the title which the user sees during checkout.', 'eshopbox_payu_in' ),
							'default' => __( 'Direct payment via PayU. PayU accepts VISA, MasterCard, Debit Cards and the Net Banking of all major banks.', 'eshopbox_payu_in' ),
						),
			'merchantid' => array(
							'title' => __( 'Merchant ID', 'eshopbox_payu_in' ),
							'type' => 'text',
							'description' => __( 'This ID is generated at the time of activation of your site and helps to uniquely identify you to PayU', 'eshopbox_payu_in' ),
							'default' => ''
						),
			'salt' => array(
							'title' => __( 'SALT', 'eshopbox_payu_in' ),
							'type' => 'text',
							'description' => __( 'String of characters provided by PayU', 'eshopbox_payu_in' ),
							'default' => ''
						),

			'cc_method' => array(
							'title' => __( 'Credit Card', 'eshopbox_payu_in' ),
							'type' => 'checkbox',
							'label' => __( 'Enable Credit Card payment method tab pre-select on the Checkout page', 'eshopbox_payu_in' ),
							'default' => 'no'
						),

			'dc_method' => array(
							'title' => __( 'Debit Card', 'eshopbox_payu_in' ),
							'type' => 'checkbox',
							'label' => __( 'Enable Debit Card payment method tab pre-select on the Checkout page', 'eshopbox_payu_in' ),
							'default' => 'no'
						),

			'nb_method' => array(
							'title' => __( 'Net Banking', 'eshopbox_payu_in' ),
							'type' => 'checkbox',
							'label' => __( 'Enable Net Banking payment method tab pre-select on the Checkout page', 'eshopbox_payu_in' ),
							'default' => 'no'
						),

			'emi_method' => array(
							'title' => __( 'EMI', 'eshopbox_payu_in' ),
							'type' => 'checkbox',
							'label' => __( 'Enable EMI payment method tab pre-select on the Checkout page', 'eshopbox_payu_in' ),
							'default' => 'no'
						),

			'cod_method' => array(
							'title' => __( 'COD', 'eshopbox_payu_in' ),
							'type' => 'checkbox',
							'label' => __( 'Enable COD payment method tab pre-select on the Checkout page', 'eshopbox_payu_in' ),
							'default' => 'no'
						),

			'testmode' => array(
							'title' => __( 'Test Mode', 'eshopbox_payu_in' ),
							'type' => 'checkbox',
							'label' => __( 'Enable PayU Test Mode', 'eshopbox_payu_in' ),
							'default' => 'no'
						)
			);
	...
	public function admin_options() {
    	?>
    	<h3><?php _e( 'PayU India', 'eshopbox_payu_in' ); ?></h3>
    	<p><?php _e( 'PayU India works by sending the user to <a href="http://www.payu.in/">PayU</a> to enter their payment information. Note that PayU will only take payments in Indian Rupee.', 'eshopbox_payu_in' ); ?></p>
		<?php
			if ( get_option( 'eshopbox_currency' ) == 'INR' ) {
		?>
		...
		?>
		<div class="inline error"><p><strong><?php _e( 'Gateway Disabled', 'eshopbox_payu_in' ); ?></strong> <?php echo sprintf( __( 'Choose Indian Rupee (₹) as your store currency in <a href="%s">Pricing Options</a> to enable the PayU Gateway.', 'eshopbox_payu_in' ), admin_url( '?page=eshopbox&tab=general' ) ); ?></p></div>
		<?php
		} // End check currency
	} // End admin_options()
    ...
    function payment_fields() {
		...
		<fieldset>
			<p class="form-row">
				<label for="pg"><?php echo __("Choose a Payment Method", 'eshopbox_payu_in') ?></label>
    ...
    function generate_payu_in_form($order_id) {
		...
		$productinfo = sprintf( __( 'Order #%s from ', 'eshopbox_payu_in' ), $order_id ) . get_bloginfo( 'name' );
		...
		return '<form action="' . $posturl . '" method="POST" name="payform" id="payform">
				' . $payuform . '
				<input type="submit" class="button" id="submit_payu_in_payment_form" value="' . __( 'Pay via PayU', 'eshopbox_payu_in' ) . '" /> <a class="button cancel" href="' . $order->get_cancel_order_url() . '">'.__( 'Cancel order &amp; restore cart', 'eshopbox_payu_in' ) . '</a>
				<script type="text/javascript">
					jQuery(function(){
						jQuery("body").block(
							{
								//message: "<img src=\"' . $eshopbox->plugin_url() . '/assets/images/ajax-loader.gif\" alt=\"Redirecting...\" />'.__('Thank you for your order. We are now redirecting you to PayU to make payment.', 'woothemes').'",
								overlayCSS:
								{
									background: "#fff",
									opacity: 0.6
	...
	function process_payment($order_id) {
		...
		return array(
			'result' 	=> 'success',
			'redirect'	=> add_query_arg( 'order', $order->id, add_query_arg( 'key', $order->order_key, add_query_arg( 'pg', $this->get_post_var('pg'), get_permalink( get_option( 'eshopbox_pay_page_id' ) ) ) ) )
		);
	} // End process_payment()
	...
	function receipt_page($order) {
		echo '<p>' . __( 'Thank you for your order, please click the button below to pay with PayU.', 'eshopbox_payu_in' ) . '</p>';
	...
	function payment_success($txnid) {
		...
		if($this->payu_in_transaction_verification($txnid) == 'success') {
			...
			$order->add_order_note( __( 'PayU payment completed', 'eshopbox_payu_in' ) . ' (Transaction id: ' . $txnid . ')' );
	...
	function payment_pending($txnid) {
		...
		if($this->payu_in_transaction_verification($txnid) == 'pending') {
			...
			$order->add_order_note(sprintf( __( 'PayU payment pending (Transaction id: %s)', 'eshopbox_payu_in' ), $txnid ));
	...
	function payment_failure($txnid) {
		global $eshopbox;
			...
			$order->add_order_note(sprintf( __( 'PayU payment failed (Transaction id: %s)', 'eshopbox_payu_in' ), $txnid ));
	...
	function send_request($host, $data) {
        ...
        if (is_wp_error($response))
            throw new Exception(__('There was a problem connecting to the payment gateway.', 'eshopbox_payu_in'));

        if (empty($response['body']))
            throw new Exception(__('Empty PayU response.', 'eshopbox_payu_in'));
