...
class WPSEO_Admin {
	...
	function register_settings_page() {
		add_menu_page( __( 'Yoast BoxBeat SEO:', 'boxbeat-seo' ) . ' ' . __( 'General Settings', 'boxbeat-seo' ), __( 'SEO', 'boxbeat-seo' ), 'manage_options', 'wpseo_dashboard', array( $this, 'config_page' ), plugins_url( 'images/yoast-icon.png', dirname( __FILE__ ) ), '99.31337' );

		$admin_page = add_submenu_page( 'wpseo_dashboard', __( 'Yoast BoxBeat SEO:', 'boxbeat-seo' ) . ' ' . __( 'Titles &amp; Metas', 'boxbeat-seo' ), __( 'Titles &amp; Metas', 'boxbeat-seo' ), 'manage_options', 'wpseo_titles', array( $this, 'titles_page' ) );
		add_action( 'load-' . $admin_page, array( $this, 'title_metas_help_tab' ) );

		add_submenu_page( 'wpseo_dashboard', __( 'Yoast BoxBeat SEO:', 'boxbeat-seo' ) . ' ' . __( 'Social', 'boxbeat-seo' ), __( 'Social', 'boxbeat-seo' ), 'manage_options', 'wpseo_social', array( $this, 'social_page' ) );
		add_submenu_page( 'wpseo_dashboard', __( 'Yoast BoxBeat SEO:', 'boxbeat-seo' ) . ' ' . __( 'XML Sitemaps', 'boxbeat-seo' ), __( 'XML Sitemaps', 'boxbeat-seo' ), 'manage_options', 'wpseo_xml', array( $this, 'xml_sitemaps_page' ) );
		add_submenu_page( 'wpseo_dashboard', __( 'Yoast BoxBeat SEO:', 'boxbeat-seo' ) . ' ' . __( 'Permalinks', 'boxbeat-seo' ), __( 'Permalinks', 'boxbeat-seo' ), 'manage_options', 'wpseo_permalinks', array( $this, 'permalinks_page' ) );
		add_submenu_page( 'wpseo_dashboard', __( 'Yoast BoxBeat SEO:', 'boxbeat-seo' ) . ' ' . __( 'Internal Links', 'boxbeat-seo' ), __( 'Internal Links', 'boxbeat-seo' ), 'manage_options', 'wpseo_internal-links', array( $this, 'internallinks_page' ) );
		add_submenu_page( 'wpseo_dashboard', __( 'Yoast BoxBeat SEO:', 'boxbeat-seo' ) . ' ' . __( 'RSS', 'boxbeat-seo' ), __( 'RSS', 'boxbeat-seo' ), 'manage_options', 'wpseo_rss', array( $this, 'rss_page' ) );
		add_submenu_page( 'wpseo_dashboard', __( 'Yoast BoxBeat SEO:', 'boxbeat-seo' ) . ' ' . __( 'Import & Export', 'boxbeat-seo' ), __( 'Import & Export', 'boxbeat-seo' ), 'manage_options', 'wpseo_import', array( $this, 'import_page' ) );

		if ( ! ( defined( 'DISALLOW_FILE_EDIT' ) && DISALLOW_FILE_EDIT ) && ! ( defined( 'DISALLOW_FILE_MODS' ) && DISALLOW_FILE_MODS ) ) {
			// Make sure on a multi site install only super admins can edit .htaccess and robots.txt
			if ( ! function_exists( 'is_multisite' ) || ! is_multisite() )
				add_submenu_page( 'wpseo_dashboard', __( 'Yoast BoxBeat SEO:', 'boxbeat-seo' ) . ' ' . __( 'Edit Files', 'boxbeat-seo' ), __( 'Edit Files', 'boxbeat-seo' ), 'manage_options', 'wpseo_files', array( $this, 'files_page' ) );
			else
				add_submenu_page( 'wpseo_dashboard', __( 'Yoast BoxBeat SEO:', 'boxbeat-seo' ) . ' ' . __( 'Edit Files', 'boxbeat-seo' ), __( 'Edit Files', 'boxbeat-seo' ), 'delete_users', 'wpseo_files', array( $this, 'files_page' ) );
		}

		global $submenu;
		if ( isset( $submenu['wpseo_dashboard'] ) )
			$submenu['wpseo_dashboard'][0][0] = __( 'Dashboard', 'boxbeat-seo' );
	}
	...
	function title_metas_help_tab() {
		...
		$screen->set_help_sidebar(
			...
			'<p><a target="_blank" href="http://yoast.com/articles/boxbeat-seo/#titles">' . __( 'Title optimization', 'boxbeat-seo' ) . '</a></p>' .
			'<p><a target="_blank" href="http://yoast.com/google-page-title/">' . __( 'Why Google won\'t display the right page title', 'boxbeat-seo' ) . '</a></p>'
		);

		$screen->add_help_tab( array(
			'id'      => 'basic-help',
			'title'   => __( 'Template explanation', 'boxbeat-seo' ),
			'content' => '<p>' . __( 'The title & metas settings for BoxBeat SEO are made up of variables that are replaced by specific values from the page when the page is displayed. The tabs on the left explain the available variables.', 'boxbeat-seo' ) . '</p>'
		) );

		$screen->add_help_tab( array(
			'id'      => 'title-vars',
			'title'   => __( 'Basic Variables', 'boxbeat-seo' ),
			'content' => '
	<h2>' . __( 'Basic Variables.', 'boxbeat-seo' ) . '</h2>
		<table class="yoast_help">
			<tr>
				<th>%%date%%</th>
				<td>' . __( 'Replaced with the date of the post/page', 'boxbeat-seo' ) . '</td>
			</tr>
			<tr>
				<th>%%title%%</th>
				<td>' . __( 'Replaced with the title of the post/page', 'boxbeat-seo' ) . '</td>
			</tr>
			<tr>
				<th>%%sitename%%</th>
				<td>' . __( 'The site\'s name', 'boxbeat-seo' ) . '</td>
			</tr>
			<tr>
				<th>%%sitedesc%%</th>
				<td>' . __( 'The site\'s tagline / description', 'boxbeat-seo' ) . '</td>
			</tr>
			<tr>
				<th>%%excerpt%%</th>
				<td>' . __( 'Replaced with the post/page excerpt (or auto-generated if it does not exist)', 'boxbeat-seo' ) . '</td>
			</tr>
			<tr>
				<th>%%excerpt_only%%</th>
				<td>' . __( 'Replaced with the post/page excerpt (without auto-generation)', 'boxbeat-seo' ) . '</td>
			</tr>
			<tr>
				<th>%%tag%%</th>
				<td>' . __( 'Replaced with the current tag/tags', 'boxbeat-seo' ) . '</td>
			</tr>
			<tr>
				<th>%%category%%</th>
				<td>' . __( 'Replaced with the post categories (comma separated)', 'boxbeat-seo' ) . '</td>
			</tr>
			<tr>
				<th>%%category_description%%</th>
				<td>' . __( 'Replaced with the category description', 'boxbeat-seo' ) . '</td>
			</tr>
			<tr>
				<th>%%tag_description%%</th>
				<td>' . __( 'Replaced with the tag description', 'boxbeat-seo' ) . '</td>
			</tr>
			<tr>
				<th>%%term_description%%</th>
				<td>' . __( 'Replaced with the term description', 'boxbeat-seo' ) . '</td>
			</tr>
			<tr>
				<th>%%term_title%%</th>
				<td>' . __( 'Replaced with the term name', 'boxbeat-seo' ) . '</td>
			</tr>
			<tr>
				<th>%%searchphrase%%</th>
				<td>' . __( 'Replaced with the current search phrase', 'boxbeat-seo' ) . '</td>
			</tr>
			<tr>
				<th>%%sep%%</th>
				<td>' . __( 'The separator defined in your theme\'s <code>wp_title()</code> tag.', 'boxbeat-seo' ) . '</td>
			</tr>
			</table>' ) );
		$screen->add_help_tab( array(
			'id'      => 'title-vars-advanced',
			'title'   => __( 'Advanced Variables.', 'boxbeat-seo' ),
			'content' => '
			<h2>' . __( 'Advanced Variables.', 'boxbeat-seo' ) . '</h2>
			<table class="yoast_help">
			<tr>
				<th>%%pt_single%%</th>
				<td>' . __( 'Replaced with the post type single label', 'boxbeat-seo' ) . '</td>
			</tr>
			<tr>
				<th>%%pt_plural%%</th>
				<td>' . __( 'Replaced with the post type plural label', 'boxbeat-seo' ) . '</td>
			</tr>
			<tr>
				<th>%%modified%%</th>
				<td>' . __( 'Replaced with the post/page modified time', 'boxbeat-seo' ) . '</td>
			</tr>
			<tr>
				<th>%%id%%</th>
				<td>' . __( 'Replaced with the post/page ID', 'boxbeat-seo' ) . '</td>
			</tr>
			<tr>
				<th>%%name%%</th>
				<td>' . __( 'Replaced with the post/page author\'s \'nicename\'', 'boxbeat-seo' ) . '</td>
			</tr>
			<tr>
				<th>%%userid%%</th>
				<td>' . __( 'Replaced with the post/page author\'s userid', 'boxbeat-seo' ) . '</td>
			</tr>
			<tr class="alt">
				<th>%%currenttime%%</th>
				<td>' . __( 'Replaced with the current time', 'boxbeat-seo' ) . '</td>
			</tr>
			<tr>
				<th>%%currentdate%%</th>
				<td>' . __( 'Replaced with the current date', 'boxbeat-seo' ) . '</td>
			</tr>
			<tr class="alt">
				<th>%%currentday%%</th>
				<td>' . __( 'Replaced with the current day', 'boxbeat-seo' ) . '</td>
			</tr>
			<tr>
				<th>%%currentmonth%%</th>
				<td>' . __( 'Replaced with the current month', 'boxbeat-seo' ) . '</td>
			</tr>
			<tr class="alt">
				<th>%%currentyear%%</th>
				<td>' . __( 'Replaced with the current year', 'boxbeat-seo' ) . '</td>
			</tr>
			<tr>
				<th>%%page%%</th>
				<td>' . __( 'Replaced with the current page number (i.e. page 2 of 4)', 'boxbeat-seo' ) . '</td>
			</tr>
			<tr class="alt">
				<th>%%pagetotal%%</th>
				<td>' . __( 'Replaced with the current page total', 'boxbeat-seo' ) . '</td>
			</tr>
			<tr>
				<th>%%pagenumber%%</th>
				<td>' . __( 'Replaced with the current page number', 'boxbeat-seo' ) . '</td>
			</tr>
			<tr class="alt">
				<th>%%caption%%</th>
				<td>' . __( 'Attachment caption', 'boxbeat-seo' ) . '</td>
			</tr>
			<tr>
				<th>%%focuskw%%</th>
				<td>' . __( 'Replaced with the posts focus keyword', 'boxbeat-seo' ) . '</td>
			</tr>
			<tr>
				<th>%%term404%%</th>
				<td>' . __( 'Replaced with the slug which caused the 404', 'boxbeat-seo' ) . '</td>
			</tr>
			<tr>
				<th>%%cf_&lt;custom-field-name&gt;%%</th>
				<td>' . __( 'Replaced with a posts custom field value', 'boxbeat-seo' ) . '</td>
			</tr>
			<tr>
				<th>%%ct_&lt;custom-tax-name&gt;%%</th>
				<td>' . __( 'Replaced with a posts custom taxonomies, comma separated.', 'boxbeat-seo' ) . '</td>
			</tr>
			<tr>
				<th>%%ct_desc_&lt;custom-tax-name&gt;%%</th>
				<td>' . __( 'Replaced with a custom taxonomies description', 'boxbeat-seo' ) . '</td>
			</tr>
	...
	function register_network_settings_page() {
		add_menu_page( __( 'Yoast BoxBeat SEO:', 'boxbeat-seo' ) . ' ' . __( 'MultiSite Settings', 'boxbeat-seo' ), __( 'SEO', 'boxbeat-seo' ), 'delete_users', 'wpseo_dashboard', array( $this, 'network_config_page' ), plugins_url( 'images/yoast-icon.png', dirname( __FILE__ ) ) );
	}
	...
	function blog_public_warning() {
		...
		echo "<p><strong>" . __( "Huge SEO Issue: You're blocking access to robots.", 'boxbeat-seo' ) . "</strong> " . sprintf( __( "You must %sgo to your Reading Settings%s and uncheck the box for Search Engine Visibility.", 'boxbeat-seo' ), "<a href='" . admin_url( 'options-reading.php' ) . "'>", "</a>" ) . " <a href='javascript:wpseo_setIgnore(\"blog_public_warning\",\"message\",\"" . wp_create_nonce( 'wpseo-ignore' ) . "\");' class='button'>" . __( "I know, don't bug me.", 'boxbeat-seo' ) . "</a></p></div>";
	}
	...
	function add_action_link( $links, $file ) {
		static $this_plugin;
		if ( empty( $this_plugin ) ) $this_plugin = 'boxbeat-seo/wp-seo.php';
		if ( $file == $this_plugin ) {
			$settings_link = '<a href="' . admin_url( 'admin.php?page=wpseo_dashboard' ) . '">' . __( 'Settings', 'boxbeat-seo' ) . '</a>';
	...
	function update_contactmethods( $contactmethods ) {
		// Add Google+
		$contactmethods['googleplus'] = __( "Google+", 'boxbeat-seo' );
		// Add Twitter
		$contactmethods['twitter'] = __( 'Twitter username (without @)', 'boxbeat-seo' );
		// Add Facebook
		$contactmethods['facebook'] = __( 'Facebook profile URL', 'boxbeat-seo' );
	...
	function user_profile( $user ) {
		...
		?>
		<h3 id="boxbeat-seo"><?php _e( "BoxBeat SEO settings", 'boxbeat-seo' ); ?></h3>
		<table class="form-table">
			<tr>
				<th><?php _e( "Title to use for Author page", 'boxbeat-seo' ); ?></th>
			...
			<tr>
				<th><?php _e( "Meta description to use for Author page", 'boxbeat-seo' ); ?></th>
			...
			<?php if ( isset( $options['usemetakeywords'] ) && $options['usemetakeywords'] ) { ?>
				<tr>
					<th><?php _e( "Meta keywords to use for Author page", 'boxbeat-seo' ); ?></th>
