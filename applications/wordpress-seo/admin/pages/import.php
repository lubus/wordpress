...
if ( isset( $_POST['import'] ) ) {
	...
	if ( isset( $_POST['wpseo']['importwoo'] ) ) {
		...
		$msg .= __( 'WooThemes SEO framework settings &amp; data successfully imported.', 'boxbeat-seo' );
	}
	if ( isset( $_POST['wpseo']['importheadspace'] ) ) {
		...
		$msg .= __('HeadSpace2 data successfully imported','boxbeat-seo');
	}
	if ( isset( $_POST['wpseo']['importaioseo'] ) ) {
		...
		$msg .= __( 'All in One SEO data successfully imported.', 'boxbeat-seo' );
	}
	if ( isset( $_POST['wpseo']['importaioseoold'] ) ) {
		...
		$msg .= __( 'All in One SEO (Old version) data successfully imported.', 'boxbeat-seo' );
	}
	if ( isset( $_POST['wpseo']['importrobotsmeta'] ) ) {
		...
		$msg .= __( 'Robots Meta values imported.', 'boxbeat-seo' );
	}
	if ( isset( $_POST['wpseo']['importrssfooter'] ) ) {
		...
		$msg .= __( 'RSS Footer options imported successfully.', 'boxbeat-seo' );
	}
	if ( isset( $_POST['wpseo']['importbreadcrumbs'] ) ) {
		...
		if ( is_array( $optold ) ) {
			...
			$msg .= __( 'Yoast Breadcrumbs options imported successfully.', 'boxbeat-seo' );
		} else {
			$msg .= __( 'Yoast Breadcrumbs options could not be found', 'boxbeat-seo' );
		}
	}
	if ( $replace )
		$msg .= __( ', and old data deleted.', 'boxbeat-seo' );
	if ( $deletekw )
		$msg .= __( ', and meta keywords data deleted.', 'boxbeat-seo' );
}
...
$content = "<p>" . __( "No doubt you've used an SEO plugin before if this site isn't new. Let's make it easy on you, you can import the data below. If you want, you can import first, check if it was imported correctly, and then import &amp; delete. No duplicate data will be imported.", 'boxbeat-seo' ) . "</p>";
$content .= '<p>' . sprintf( __( "If you've used another SEO plugin, try the %sSEO Data Transporter%s plugin to move your data into this plugin, it rocks!", 'boxbeat-seo' ), "<a href='http://boxbeat.org/extend/plugins/seo-data-transporter/'>", "</a>" ) . '</p>';
...
$content .= $wpseo_admin_pages->checkbox( 'importheadspace', __( 'Import from HeadSpace2?', 'boxbeat-seo' ) );
$content .= $wpseo_admin_pages->checkbox( 'importaioseo', __( 'Import from All-in-One SEO?', 'boxbeat-seo' ) );
$content .= $wpseo_admin_pages->checkbox( 'importaioseoold', __( 'Import from OLD All-in-One SEO?', 'boxbeat-seo' ) );
$content .= $wpseo_admin_pages->checkbox( 'importwoo', __( 'Import from WooThemes SEO framework?', 'boxbeat-seo' ) );
$content .= '<br/>';
$content .= $wpseo_admin_pages->checkbox( 'deleteolddata', __( 'Delete the old data after import? (recommended)', 'boxbeat-seo' ) );
$content .= '<br/>';
$content .= '<input type="submit" class="button-primary" name="import" value="' . __( 'Import', 'boxbeat-seo' ) . '" />';
$content .= '<br/><br/>';
$content .= '<h2>' . __( 'Import settings from other plugins', 'boxbeat-seo' ) . '</h2>';
$content .= $wpseo_admin_pages->checkbox( 'importrobotsmeta', __( 'Import from Robots Meta (by Yoast)?', 'boxbeat-seo' ) );
$content .= $wpseo_admin_pages->checkbox( 'importrssfooter', __( 'Import from RSS Footer (by Yoast)?', 'boxbeat-seo' ) );
$content .= $wpseo_admin_pages->checkbox( 'importbreadcrumbs', __( 'Import from Yoast Breadcrumbs?', 'boxbeat-seo' ) );
$content .= '<br/>';
$content .= '<input type="submit" class="button-primary" name="import" value="' . __( 'Import', 'boxbeat-seo' ) . '" />';
$content .= '</form><br/>';

$wpseo_admin_pages->postbox( 'import', __( 'Import', 'boxbeat-seo' ), $content );

do_action( 'wpseo_import', $this );

$content = '<h4>' . __( 'Export', 'boxbeat-seo' ) . '</h4>';
$content .= '<form method="post" accept-charset="' . get_bloginfo( 'charset' ) . '">';
$content .= wp_nonce_field( 'wpseo-export', '_wpnonce', true, false );
$content .= '<p>' . __( 'Export your BoxBeat SEO settings here, to import them again later or to import them on another site.', 'boxbeat-seo' ) . '</p>';
if ( phpversion() > 5.2 )
	$content .= $wpseo_admin_pages->checkbox( 'include_taxonomy_meta', __( 'Include Taxonomy Metadata', 'boxbeat-seo' ) );
$content .= '<br/><input type="submit" class="button" name="wpseo_export" value="' . __( 'Export settings', 'boxbeat-seo' ) . '"/>';
$content .= '</form>';
...
$content .= '<h4>' . __( 'Import', 'boxbeat-seo' ) . '</h4>';
if ( !isset( $_FILES['settings_import_file'] ) || empty( $_FILES['settings_import_file'] ) ) {
	$content .= '<p>' . __( 'Import settings by locating <em>settings.zip</em> and clicking', 'boxbeat-seo' ) . ' "' . __( 'Import settings', 'boxbeat-seo' ) . '":</p>';
	...
	$content .= '<input type="submit" class="button" value="' . __( 'Import settings', 'boxbeat-seo' ) . '"/>';
	$content .= '</form><br/>';
	...
	if ( isset( $file['file'] ) && !is_wp_error( $file ) ) {
		...
		if ( $unzipped[0]['stored_filename'] == 'settings.ini' ) {
			...
			$content .= '<p><strong>' . __( 'Settings successfully imported.', 'boxbeat-seo' ) . '</strong></p>';
		} else {
			$content .= '<p><strong>' . __( 'Settings could not be imported:', 'boxbeat-seo' ) . ' ' . __( 'Unzipping failed.', 'boxbeat-seo' ) . '</strong></p>';
		}
	} else {
		if ( is_wp_error( $file ) )
			$content .= '<p><strong>' . __( 'Settings could not be imported:', 'boxbeat-seo' ) . ' ' . $file['error'] . '</strong></p>';
		else
			$content .= '<p><strong>' . __( 'Settings could not be imported:', 'boxbeat-seo' ) . ' ' . __( 'Upload failed.', 'boxbeat-seo' ) . '</strong></p>';
	}
}
$wpseo_admin_pages->postbox( 'wpseo_export', __( 'Export & Import SEO Settings', 'boxbeat-seo' ), $content );
