...
class WPSEO_Metabox {
	...
	public function publish_box() {
		...
		if ( wpseo_get_value( 'meta-robots-noindex' ) == 1 ) {
			$score_label = 'noindex';
			$title       = __( 'Post is set to noindex.', 'boxbeat-seo' );
		}
		else {
			...
			if ( $score ) {
				...
				if ( ! $score || empty( $score ) ) {
					$score_label = 'na';
					$title       = __( 'No focus keyword set.', 'boxbeat-seo' );
				}
				else {
					$score_label = wpseo_translate_score( $score );
				}
		...
		echo __( 'SEO: ', 'boxbeat-seo' ) . $result . ' <a class="wpseo_tablink scroll" href="#wpseo_linkdex">' . __( 'Check', 'boxbeat-seo' ) . '</a>';

		echo '</div>';
	}

	/**
	 * Adds the BoxBeat SEO box to the edit boxes in the edit post / page overview.
	 */
	public function add_custom_box() {
		...
		foreach ( get_post_types( array( 'public' => true ) ) as $posttype ) {
			if ( isset( $options['hideeditbox-' . $posttype] ) && $options['hideeditbox-' . $posttype] )
				continue;
			add_meta_box( 'wpseo_meta', __( 'BoxBeat SEO by Yoast', 'boxbeat-seo' ), array( $this, 'meta_box' ), $posttype, 'normal', apply_filters( 'wpseo_metabox_prio', 'high' ) );
		}
	}
	...
	public function script() {
		...
		if ( isset( $options['showdate-' . $post->post_type] ) && $options['showdate-' . $post->post_type] ) {
			...
			$this->meta_length_reason = __( ' (because of date display)', 'boxbeat-seo' );
		}
	...
	public function get_meta_boxes( $post_type = 'post' ) {
		...
		$mbs['snippetpreview'] = array(
			"name"  => "snippetpreview",
			"type"  => "snippetpreview",
			"title" => __( "Snippet Preview", 'boxbeat-seo' ),
		);
		$mbs['focuskw']        = array(
			"name"         => "focuskw",
			"std"          => "",
			"type"         => "text",
			"title"        => __( 'Focus Keyword', 'boxbeat-seo' ),
			"autocomplete" => "off",
			"help"         => sprintf( __( "Pick the main keyword or keyphrase that this post/page is about.<br/><br/>Read %sthis post%s for more info.", 'boxbeat-seo' ), "<a href='http://yoast.com/focus-keyword/#utm_source=boxbeat-seo-metabox&utm_medium=inline-help&utm_campaign=focus-keyword'>", '</a>' ),
		...
		$mbs['title']          = array(
			...
			"std"         => "",
			"type"        => "text",
			"title"       => __( "SEO Title", 'boxbeat-seo' ),
			"description" => sprintf( __( "Title display in search engines is limited to 70 chars, %s chars left.", 'boxbeat-seo' ), "<span id='yoast_wpseo_title-length'></span>" ),
			"help"        => __( "The SEO Title defaults to what is generated based on this sites title template for this posttype.", 'boxbeat-seo' )
		);
		$mbs['metadesc']       = array(
			"name"        => "metadesc",
			"std"         => "",
			"class"       => "metadesc",
			"type"        => "textarea",
			"title"       => __( "Meta Description", 'boxbeat-seo' ),
			"rows"        => 2,
			"richedit"    => false,
			"description" => sprintf( __( "The <code>meta</code> description will be limited to %s chars%s, %s chars left.", 'boxbeat-seo' ), $this->meta_length, $this->meta_length_reason, "<span id='yoast_wpseo_metadesc-length'></span>" ) . " <div id='yoast_wpseo_metadesc_notice'></div>",
			"help"        => __( "If the meta description is empty, the snippet preview above shows what is generated based on this sites meta description template.", 'boxbeat-seo' ),
		);
		if ( isset( $options['usemetakeywords'] ) && $options['usemetakeywords'] ) {
			$mbs['metakeywords'] = array(
				...
				"class"       => "metakeywords",
				"type"        => "text",
				"title"       => __( "Meta Keywords", 'boxbeat-seo' ),
				"description" => sprintf( __( "If you type something above it will override your %smeta keywords template%s.", 'boxbeat-seo' ), "<a target='_blank' href='" . admin_url( 'admin.php?page=wpseo_titles#' . esc_url( $post_type ) ) . "'>", "</a>" )
			);
		}
	...
	function get_advanced_meta_boxes() {
		...
		$mbs['meta-robots-noindex']  = array(
			...
			"std"     => "-",
			"title"   => __( "Meta Robots Index", 'boxbeat-seo' ),
			"type"    => "select",
			"options" => array(
				"0" => sprintf( __( "Default for post type, currently: %s", 'boxbeat-seo' ), ( isset( $options['noindex-' . $post_type] ) && $options['noindex-' . $post_type] ) ? 'noindex' : 'index' ),
				"2" => __( "index", 'boxbeat-seo' ),
				"1" => __( "noindex", 'boxbeat-seo' ),
			),
		);
		$mbs['meta-robots-nofollow'] = array(
			"name"    => "meta-robots-nofollow",
			"std"     => "follow",
			"title"   => __( "Meta Robots Follow", 'boxbeat-seo' ),
			"type"    => "radio",
			"options" => array(
				"0" => __( "Follow", 'boxbeat-seo' ),
				"1" => __( "Nofollow", 'boxbeat-seo' ),
			),
		);
		$mbs['meta-robots-adv']      = array(
			...
			"std"         => "none",
			"type"        => "multiselect",
			"title"       => __( "Meta Robots Advanced", 'boxbeat-seo' ),
			"description" => __( "Advanced <code>meta</code> robots settings for this page.", 'boxbeat-seo' ),
			"options"     => array(
				"noodp"     => __( "NO ODP", 'boxbeat-seo' ),
				"noydir"    => __( "NO YDIR", 'boxbeat-seo' ),
				"noarchive" => __( "No Archive", 'boxbeat-seo' ),
				"nosnippet" => __( "No Snippet", 'boxbeat-seo' ),
			),
		);
		if ( isset( $options['breadcrumbs-enable'] ) && $options['breadcrumbs-enable'] ) {
			$mbs['bctitle'] = array(
				...
				"std"         => "",
				"type"        => "text",
				"title"       => __( "Breadcrumbs title", 'boxbeat-seo' ),
				"description" => __( "Title to use for this page in breadcrumb paths", 'boxbeat-seo' ),
			);
		}
		if ( isset( $options['enablexmlsitemap'] ) && $options['enablexmlsitemap'] ) {
			$mbs['sitemap-include'] = array(
				...
				"std"         => "-",
				"type"        => "select",
				"title"       => __( "Include in Sitemap", 'boxbeat-seo' ),
				"description" => __( "Should this page be in the XML Sitemap at all times, regardless of Robots Meta settings?", 'boxbeat-seo' ),
				"options"     => array(
					"-"      => __( "Auto detect", 'boxbeat-seo' ),
					"always" => __( "Always include", 'boxbeat-seo' ),
					"never"  => __( "Never include", 'boxbeat-seo' ),
				),
			);
			$mbs['sitemap-prio']    = array(
				...
				"std"         => "-",
				"type"        => "select",
				"title"       => __( "Sitemap Priority", 'boxbeat-seo' ),
				"description" => __( "The priority given to this page in the XML sitemap.", 'boxbeat-seo' ),
				"options"     => array(
					"-"   => __( "Automatic prioritization", 'boxbeat-seo' ),
					"1"   => __( "1 - Highest priority", 'boxbeat-seo' ),
					"0.9" => "0.9",
					"0.8" => "0.8 - " . __( "Default for first tier pages", 'boxbeat-seo' ),
					"0.7" => "0.7",
					"0.6" => "0.6 - " . __( "Default for second tier pages and posts", 'boxbeat-seo' ),
					"0.5" => "0.5 - " . __( "Medium priority", 'boxbeat-seo' ),
					"0.4" => "0.4",
					"0.3" => "0.3",
					"0.2" => "0.2",
					"0.1" => "0.1 - " . __( "Lowest priority", 'boxbeat-seo' ),
				),
			);
		}
		$mbs['sitemap-html-include'] = array(
			...
			"std"         => "-",
			"type"        => "select",
			"title"       => __( "Include in HTML Sitemap", 'boxbeat-seo' ),
			"description" => __( "Should this page be in the HTML Sitemap at all times, regardless of Robots Meta settings?", 'boxbeat-seo' ),
			"options"     => array(
				"-"      => __( "Auto detect", 'boxbeat-seo' ),
				"always" => __( "Always include", 'boxbeat-seo' ),
				"never"  => __( "Never include", 'boxbeat-seo' ),
		...
		$mbs['canonical']            = array(
			...
			"std"         => "",
			"type"        => "text",
			"title"       => __( "Canonical URL", 'boxbeat-seo' ),
			"description" => sprintf( __( "The canonical URL that this page should point to, leave empty to default to permalink. %sCross domain canonical%s supported too.", 'boxbeat-seo' ), "<a target='_blank' href='http://googlewebmastercentral.blogspot.com/2009/12/handling-legitimate-cross-domain.html'>", "</a>" )
		);
		$mbs['redirect']             = array(
			"name"        => "redirect",
			"std"         => "",
			"type"        => "text",
			"title"       => __( "301 Redirect", 'boxbeat-seo' ),
			"description" => __( "The URL that this page should redirect to.", 'boxbeat-seo' )
		);
	...
	function meta_box() {
		...
		<ul class="wpseo-metabox-tabs" id="wpseo-metabox-tabs">
			<li class="general"><a class="wpseo_tablink"
														 href="#wpseo_general"><?php _e( "General", 'boxbeat-seo' ); ?></a></li>
			<li id="linkdex" class="linkdex"><a class="wpseo_tablink"
																					href="#wpseo_linkdex"><?php _e( "Page Analysis", 'boxbeat-seo' ); ?></a>
			</li>
			<?php if ( current_user_can( 'manage_options' ) || ! isset( $options['disableadvanced_meta'] ) || ! $options['disableadvanced_meta'] ): ?>
				<li class="advanced"><a class="wpseo_tablink"
																href="#wpseo_advanced"><?php _e( "Advanced", 'boxbeat-seo' ); ?></a></li>
			<?php endif; ?>
		...
		$this->do_tab( 'general', __( 'General', 'boxbeat-seo' ), $content );

		$this->do_tab( 'linkdex', __( 'Page Analysis', 'boxbeat-seo' ), $this->linkdex_output( $post ) );
		...
		if ( current_user_can( 'manage_options' ) || ! isset( $options['disableadvanced_meta'] ) || ! $options['disableadvanced_meta'] ) {
			...
			$this->do_tab( 'advanced', __( 'Advanced', 'boxbeat-seo' ), $content );
		}
...
//		$content .= '<pre>' . print_r( $gplus, 1 ) . '</pre>';
	...
	function get_gplus_data( $user_id ) {
		...
		$args = array(
			'headers' => array(
				'Referer' => 'http://yoast.com/eshop-admin/',
			),
		);
	...
	public function enqueue() {
		...
		if ( $pagenow == 'edit.php' ) {
			...
			wp_localize_script( 'wp-seo-metabox', 'wpseoMetaboxL10n', array(
				'keyword_header'        => __( 'Your focus keyword was found in:', 'boxbeat-seo' ),
				'article_header_text'   => __( 'Article Heading: ', 'boxbeat-seo' ),
				'page_title_text'       => __( 'Page title: ', 'boxbeat-seo' ),
				'page_url_text'         => __( 'Page URL: ', 'boxbeat-seo' ),
				'content_text'          => __( 'Content: ', 'boxbeat-seo' ),
				'meta_description_text' => __( 'Meta description: ', 'boxbeat-seo' ),
				'choose_image'          => __( 'Use Image', 'boxbeat-seo' )
			) );
		}
	}
	...
	function posts_filter_dropdown() {
		...
		echo '<option value="">' . __( "All SEO Scores", 'boxbeat-seo' ) . '</option>';
		foreach ( array(
								'na'      => __( 'SEO: No Focus Keyword', 'boxbeat-seo' ),
								'bad'     => __( 'SEO: Bad', 'boxbeat-seo' ),
								'poor'    => __( 'SEO: Poor', 'boxbeat-seo' ),
								'ok'      => __( 'SEO: OK', 'boxbeat-seo' ),
								'good'    => __( 'SEO: Good', 'boxbeat-seo' ),
								'noindex' => __( 'SEO: Post Noindexed', 'boxbeat-seo' )
							) as $val => $text ) {
			$sel = '';
	...
	function column_heading( $columns ) {
		return array_merge( $columns, array( 'wpseo-score' => __( 'SEO', 'boxbeat-seo' ), 'wpseo-title' => __( 'SEO Title', 'boxbeat-seo' ), 'wpseo-metadesc' => __( 'Meta Desc.', 'boxbeat-seo' ), 'wpseo-focuskw' => __( 'Focus KW', 'boxbeat-seo' ) ) );
	}
	...
	function column_content( $column_name, $post_id ) {
		if ( $column_name == 'wpseo-score' ) {
			if ( wpseo_get_value( 'meta-robots-noindex', $post_id ) == 1 ) {
				...
				if ( ! $score || empty( $score ) ) {
					$score_label = 'na';
					$title       = __( 'Focus keyword not set.', 'boxbeat-seo' );
				}
				else {
					$score_label = wpseo_translate_score( $score );
					$title       = wpseo_translate_score( $score, $css = false );
	...
	function linkdex_output( $post ) {
		...
		$output = '<div class="wpseo_msg"><p>' . __( 'To update this page analysis, save as draft or update and check this tab again', 'boxbeat-seo' ) . '.</p></div>' . $output;
	...
	function calculate_results( $post ) {
		...
		if ( ! class_exists( 'DOMDocument' ) ) {
			$result = new WP_Error( 'no-domdocument', sprintf( __( "Your hosting environment does not support PHP's %sDocument Object Model%s.", 'boxbeat-seo' ), '<a href="http://php.net/manual/en/book.dom.php">', '</a>' ) . ' ' . __( "To enjoy all the benefits of the page analysis feature, you'll need to (get your host to) install it.", 'boxbeat-seo' ) );
			return $result;
		}

		if ( ! wpseo_get_value( 'focuskw', $post->ID ) ) {
			$result = new WP_Error( 'no-focuskw', sprintf( __( 'No focus keyword was set for this %s. If you do not set a focus keyword, no score can be calculated.', 'boxbeat-seo' ), $post->post_type ) );
		...
		elseif ( apply_filters( 'wpseo_use_page_analysis', true ) !== true ) {
			$result = new WP_Error( 'page-analysis-disabled', sprintf( __( 'Page Analysis has been disabled.', 'boxbeat-seo' ), $post->post_type ) );
	...
	function check_double_focus_keyword( $job, &$results ) {
		...
		if ( count( $posts ) == 0 )
			$this->save_score_result( $results, 9, __( "You've never used this focus keyword before, very good.", 'boxbeat-seo' ), 'keyword_overused' );
		else if ( count( $posts ) == 1 )
			$this->save_score_result( $results, 6, sprintf( __( 'You\'ve used this focus keyword %1$sonce before%2$s, be sure to make very clear which URL on your site is the most important for this keyword.', 'boxbeat-seo' ), '<a href="' . admin_url( 'post.php?post=' . $posts[0] . '&action=edit' ) . '">', '</a>' ), 'keyword_overused' );
		else
			$this->save_score_result( $results, 1, sprintf( __( 'You\'ve used this focus keyword %3$s%4$d times before%2$s, it\'s probably a good idea to read %1$sthis post on cornerstone content%2$s and improve your keyword strategy.', 'boxbeat-seo' ), '<a href="http://yoast.com/cornerstone-content-rank/">', '</a>', '<a href="' . admin_url( 'edit.php?seo_kw_filter=' . urlencode( $job['keyword'] ) ) . '">', count( $posts ) ), 'keyword_overused' );
	}
	...
	function score_keyword( $keyword, &$results ) {
		...
		$keywordStopWord = __( "The keyword for this page contains one or more %sstop words%s, consider removing them. Found '%s'.", 'boxbeat-seo' );
	...
	function score_url( $job, &$results ) {
		...
		$urlGood      = __( "The keyword / phrase appears in the URL for this page.", 'boxbeat-seo' );
		$urlMedium    = __( "The keyword / phrase does not appear in the URL for this page. If you decide to rename the URL be sure to check the old URL 301 redirects to the new one!", 'boxbeat-seo' );
		$urlStopWords = __( "The slug for this page contains one or more <a href=\"http://en.wikipedia.org/wiki/Stop_words\">stop words</a>, consider removing them.", 'boxbeat-seo' );
		$longSlug     = __( "The slug for this page is a bit long, consider shortening it.", 'boxbeat-seo' );
	...
	function score_title( $job, &$results ) {
		...
		$scoreTitleMissing          = __( "Please create a page title.", 'boxbeat-seo' );
		$scoreTitleCorrectLength    = __( "The page title is more than 40 characters and less than the recommended 70 character limit.", 'boxbeat-seo' );
		$scoreTitleTooShort         = __( "The page title contains %d characters, which is less than the recommended minimum of 40 characters. Use the space to add keyword variations or create compelling call-to-action copy.", 'boxbeat-seo' );
		$scoreTitleTooLong          = __( "The page title contains %d characters, which is more than the viewable limit of 70 characters; some words will not be visible to users in your listing.", 'boxbeat-seo' );
		$scoreTitleKeywordMissing   = __( "The keyword / phrase %s does not appear in the page title.", 'boxbeat-seo' );
		$scoreTitleKeywordBeginning = __( "The page title contains keyword / phrase, at the beginning which is considered to improve rankings.", 'boxbeat-seo' );
		$scoreTitleKeywordEnd       = __( "The page title contains keyword / phrase, but it does not appear at the beginning; try and move it to the beginning.", 'boxbeat-seo' );
	...
	function score_anchor_texts( $job, &$results, $anchor_texts, $count ) {
		$scoreNoLinks               = __( "No outbound links appear in this page, consider adding some as appropriate.", 'boxbeat-seo' );
		$scoreKeywordInOutboundLink = __( "You're linking to another page with the keyword you want this page to rank for, consider changing that if you truly want this page to rank.", 'boxbeat-seo' );
		$scoreLinksDofollow         = __( "This page has %s outbound link(s).", 'boxbeat-seo' );
		$scoreLinksNofollow         = __( "This page has %s outbound link(s), all nofollowed.", 'boxbeat-seo' );
		$scoreLinks                 = __( "This page has %s nofollowed link(s) and %s normal outbound link(s).", 'boxbeat-seo' );
	...
	function score_images_alt_text( $job, &$results, $imgs ) {
		$scoreImagesNoImages          = __( "No images appear in this page, consider adding some as appropriate.", 'boxbeat-seo' );
		$scoreImagesNoAlt             = __( "The images on this page are missing alt tags.", 'boxbeat-seo' );
		$scoreImagesAltKeywordIn      = __( "The images on this page contain alt tags with the target keyword / phrase.", 'boxbeat-seo' );
		$scoreImagesAltKeywordMissing = __( "The images on this page do not have alt tags containing your keyword / phrase.", 'boxbeat-seo' );
	...
	function score_headings( $job, &$results, $headings ) {
		$scoreHeadingsNone           = __( "No subheading tags (like an H2) appear in the copy.", 'boxbeat-seo' );
		$scoreHeadingsKeywordIn      = __( "Keyword / keyphrase appears in %s (out of %s) subheadings in the copy. While not a major ranking factor, this is beneficial.", 'boxbeat-seo' );
		$scoreHeadingsKeywordMissing = __( "You have not used your keyword / keyphrase in any subheading (such as an H2) in your copy.", 'boxbeat-seo' );
	...
	function score_description( $job, &$results, $description, $maxlength = 155 ) {
		...
		$scoreDescriptionCorrectLength  = __( "In the specified meta description, consider: How does it compare to the competition? Could it be made more appealing?", 'boxbeat-seo' );
		$scoreDescriptionTooShort       = __( "The meta description is under 120 characters, however up to %s characters are available. %s", 'boxbeat-seo' );
		$scoreDescriptionTooLong        = __( "The specified meta description is over %s characters, reducing it will ensure the entire description is visible. %s", 'boxbeat-seo' );
		$scoreDescriptionMissing        = __( "No meta description has been specified, search engines will display copy from the page instead.", 'boxbeat-seo' );
		$scoreDescriptionKeywordIn      = __( "The meta description contains the primary keyword / phrase.", 'boxbeat-seo' );
		$scoreDescriptionKeywordMissing = __( "A meta description has been specified, but it does not contain the target keyword / phrase.", 'boxbeat-seo' );

		$metaShorter = '';
		if ( $maxlength != 155 )
			$metaShorter = __( "The available space is shorter than the usual 155 characters because Google will also include the publication date in the snippet.", 'boxbeat-seo' );
	...
	function score_body( $job, &$results, $body, $firstp ) {
		...
		$scoreBodyGoodLength = __( "There are %d words contained in the body copy, this is more than the %d word recommended minimum.", 'boxbeat-seo' );
		$scoreBodyPoorLength = __( "There are %d words contained in the body copy, this is below the %d word recommended minimum. Add more useful content on this topic for readers.", 'boxbeat-seo' );
		$scoreBodyOKLength   = __( "There are %d words contained in the body copy, this is slightly below the %d word recommended minimum, add a bit more copy.", 'boxbeat-seo' );
		$scoreBodyBadLength  = __( "There are %d words contained in the body copy. This is far too low and should be increased.", 'boxbeat-seo' );

		$scoreKeywordDensityLow  = __( "The keyword density is %s%%, which is a bit low, the keyword was found %s times.", 'boxbeat-seo' );
		$scoreKeywordDensityHigh = __( "The keyword density is %s%%, which is over the advised 4.5%% maximum, the keyword was found %s times.", 'boxbeat-seo' );
		$scoreKeywordDensityGood = __( "The keyword density is %s%%, which is great, the keyword was found %s times.", 'boxbeat-seo' );

		$scoreFirstParagraphLow  = __( "The keyword doesn't appear in the first paragraph of the copy, make sure the topic is clear immediately.", 'boxbeat-seo' );
		$scoreFirstParagraphHigh = __( "The keyword appears in the first paragraph of the copy.", 'boxbeat-seo' );

		$fleschurl   = '<a href="http://en.wikipedia.org/wiki/Flesch-Kincaid_readability_test#Flesch_Reading_Ease">' . __( 'Flesch Reading Ease', 'boxbeat-seo' ) . '</a>';
		$scoreFlesch = __( "The copy scores %s in the %s test, which is considered %s to read. %s", 'boxbeat-seo' );
		...
		if ( $keywordWordCount > 10 ) {
			$this->save_score_result( $results, 0, __( 'Your keyphrase is over 10 words, a keyphrase should be shorter and there can be only one keyphrase.', 'boxbeat-seo' ), 'focus_keyword_length' );
		}
		else {
		...
		if ( substr( $lang, 0, 2 ) == 'en' && $wordCount > 100 ) {
			...
			$score = 1;
			if ( $flesch >= 90 ) {
				$level = __( 'very easy', 'boxbeat-seo' );
				$score = 9;
			}
			else if ( $flesch >= 80 ) {
				$level = __( 'easy', 'boxbeat-seo' );
				$score = 9;
			}
			else if ( $flesch >= 70 ) {
				$level = __( 'fairly easy', 'boxbeat-seo' );
				$score = 8;
			}
			else if ( $flesch >= 60 ) {
				$level = __( 'OK', 'boxbeat-seo' );
				$score = 7;
			}
			else if ( $flesch >= 50 ) {
				$level = __( 'fairly difficult', 'boxbeat-seo' );
				$note  = __( 'Try to make shorter sentences to improve readability.', 'boxbeat-seo' );
				$score = 6;
			}
			else if ( $flesch >= 30 ) {
				$level = __( 'difficult', 'boxbeat-seo' );
				$note  = __( 'Try to make shorter sentences, using less difficult words to improve readability.', 'boxbeat-seo' );
				$score = 5;
			}
			else if ( $flesch >= 0 ) {
				$level = __( 'very difficult', 'boxbeat-seo' );
				$note  = __( 'Try to make shorter sentences, using less difficult words to improve readability.', 'boxbeat-seo' );
				$score = 4;
			}
