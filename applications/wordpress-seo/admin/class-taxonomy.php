...
class WPSEO_Taxonomy {
	...
	function term_seo_form( $term ) {
		...
		echo '<h2>' . __( 'Yoast BoxBeat SEO Settings', 'boxbeat-seo' ) . '</h2>';
		echo '<table class="form-table">';

		$this->form_row( 'wpseo_title', __( 'SEO Title', 'boxbeat-seo' ), __( 'The SEO title is used on the archive page for this term.', 'boxbeat-seo' ), $tax_meta );
		$this->form_row( 'wpseo_desc', __( 'SEO Description', 'boxbeat-seo' ), __( 'The SEO description is used for the meta description on the archive page for this term.', 'boxbeat-seo' ), $tax_meta );
		if ( isset( $options['usemetakeywords'] ) && $options['usemetakeywords'] )
			$this->form_row( 'wpseo_metakey', __( 'Meta Keywords', 'boxbeat-seo' ), __( 'Meta keywords used on the archive page for this term.', 'boxbeat-seo' ), $tax_meta );
		$this->form_row( 'wpseo_canonical', __( 'Canonical', 'boxbeat-seo' ), __( 'The canonical link is shown on the archive page for this term.', 'boxbeat-seo' ), $tax_meta );
		$this->form_row( 'wpseo_bctitle', __( 'Breadcrumbs Title', 'boxbeat-seo' ), sprintf( __( 'The Breadcrumbs title is used in the breadcrumbs where this %s appears.', 'boxbeat-seo' ), $term->taxonomy ), $tax_meta );
		...
		$noindex_options = array(
			'default' => sprintf( __( 'Use %s default (Currently: %s)', 'boxbeat-seo' ), $term->taxonomy, $current ),
			'index'   => __( 'Always index', 'boxbeat-seo' ),
			'noindex' => __( 'Always noindex', 'boxbeat-seo' ) );
		$this->form_row( 'wpseo_noindex', sprintf( __( 'Noindex this %s', 'boxbeat-seo' ), $term->taxonomy ), sprintf( __( 'This %s follows the indexation rules set under Metas and Titles, you can override it here.', 'boxbeat-seo' ), $term->taxonomy ), $tax_meta, 'select', $noindex_options );

		$this->form_row( 'wpseo_sitemap_include', __( 'Include in sitemap?', 'boxbeat-seo' ), '', $tax_meta, 'select', array(
