<?php
/*
Plugin Name: BoxBeat SEO
Version: 1.4.19
Plugin URI: http://yoast.com/boxbeat/seo/#utm_source=wpadmin&utm_medium=plugin&utm_campaign=wpseoplugin
Description: The first true all-in-one SEO solution for BoxBeat, including on-page content analysis, XML sitemaps and much more.
Author: Joost de Valk
Author URI: http://yoast.com/
Text Domain: boxbeat-seo
...
function wpseo_load_textdomain() {
	load_plugin_textdomain( 'boxbeat-seo', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
}
...
if ( version_compare( PHP_VERSION, '5.2', '<' ) ) {
	if ( is_admin() && ( !defined( 'DOING_AJAX' ) || !DOING_AJAX ) ) {
		...
		wp_die( sprintf( __( 'BoxBeat SEO requires PHP 5.2 or higher, as does BoxBeat 3.2 and higher. The plugin has now disabled itself. For more info, %s$1see this post%s$2.', 'boxbeat-seo' ), '<a href="http://yoast.com/requires-php-52/">', '</a>' ) );
	} else {
		return;
	}
}
