jQuery( function($){
	...
	// Type box
	$('.type_box').appendTo( '#eshopbox-product-data h3.hndle span' );

	$(function(){
		// Prevent inputs in meta box headings opening/closing contents
		$('#eshopbox-product-data h3.hndle').unbind('click.postboxes');

		jQuery('#eshopbox-product-data').on('click', 'h3.hndle', function(event){

			// If the user clicks on some form input inside the h3 the box should not be toggled
			if ( $(event.target).filter('input, option, label, select').length )
				return;

			$('#eshopbox-product-data').toggleClass('closed');
		});
	});
	...
	$('#catalog-visibility .save-post-visibility').click(function () {
		...
		if ( $('input[name=_featured]').is(':checked') ) {
			label = label + ', ' + eshopbox_writepanel_params.featured_label
	...
	jQuery("select.ajax_chosen_select_products").ajaxChosen({
	    method: 	'GET',
	    url: 		eshopbox_writepanel_params.ajax_url,
	    dataType: 	'json',
	    afterTypeDelay: 100,
	    data:		{
	    	action: 		'eshopbox_json_search_products',
			security: 		eshopbox_writepanel_params.search_products_nonce
	    }
	}, function (data) {
	...
	jQuery("select.ajax_chosen_select_products_and_variations").ajaxChosen({
	    ...
	    afterTypeDelay: 100,
	    data:		{
	    	action: 		'eshopbox_json_search_products_and_variations',
			security: 		eshopbox_writepanel_params.search_products_nonce
	    }
	}, function (data) {
	...
	// ORDERS
	jQuery('#eshopbox-order-actions input, #eshopbox-order-actions a').click(function(){
	...
	$('#eshopbox-order-totals').on( 'change', '#_order_tax, #_order_shipping_tax, #_cart_discount, #_order_discount', function() {
		...
		var formatted_total = accounting.formatMoney( total, {
			symbol 		: eshopbox_writepanel_params.currency_format_symbol,
			decimal 	: eshopbox_writepanel_params.currency_format_decimal_sep,
			thousand	: eshopbox_writepanel_params.currency_format_thousand_sep,
			precision 	: eshopbox_writepanel_params.currency_format_num_decimals,
			format		: eshopbox_writepanel_params.currency_format
		} );
	...
	$('button.calc_line_taxes').click(function(){
		// Block write panel
		$('.eshopbox_order_items_wrapper').block({ message: null, overlayCSS: { background: '#fff url(' + eshopbox_writepanel_params.plugin_url + '/assets/images/ajax-loader.gif) no-repeat center', opacity: 0.6 } });

		var answer = confirm(eshopbox_writepanel_params.calc_line_taxes);
		...
		if (answer) {
			...
			if (shipping_country) {
				...
				var country = eshopbox_writepanel_params.base_country;
				var state = '';
				var postcode = '';
				var city = '';
			}
			...
			var data = {
				action: 		'eshopbox_calc_line_taxes',
				order_id: 		eshopbox_writepanel_params.post_id,
				...
				postcode:		postcode,
				city:			city,
				security: 		eshopbox_writepanel_params.calc_totals_nonce
			};

			$.post( eshopbox_writepanel_params.ajax_url, data, function( response ) {
				...
				$('.eshopbox_order_items_wrapper').unblock();
			});
	...
	$('button.calc_totals').click( function(){
		// Block write panel
		$('#eshopbox-order-totals').block({ message: null, overlayCSS: { background: '#fff url(' + eshopbox_writepanel_params.plugin_url + '/assets/images/ajax-loader.gif) no-repeat center', opacity: 0.6 } });

		var answer = confirm(eshopbox_writepanel_params.calc_totals);
		...
		if (answer) {
			...
			$('#order_items_list tr.item').each(function(){
				...
				if ( eshopbox_writepanel_params.round_at_subtotal=='no' ) {
			...
			// Tax
			if (eshopbox_writepanel_params.round_at_subtotal=='yes') {
		...
		$('#eshopbox-order-totals .calculated').css('background-color', '#e3d2dd');
	}, function() {
		$('#eshopbox-order-totals .calculated').css('background-color', '');
	});

	// Add a line item
	$('#eshopbox-order-items button.add_order_item').click(function(){
		...
		if ( add_item_ids ) {
			...
			$('table.eshopbox_order_items').block({ message: null, overlayCSS: { background: '#fff url(' + eshopbox_writepanel_params.plugin_url + '/assets/images/ajax-loader.gif) no-repeat center', opacity: 0.6 } });

			$.each( add_item_ids, function( index, value ) {

				var data = {
					action: 		'eshopbox_add_order_item',
					item_to_add: 	value,
					order_id:		eshopbox_writepanel_params.post_id,
					security: 		eshopbox_writepanel_params.order_item_nonce
				};

				$.post( eshopbox_writepanel_params.ajax_url, data, function( response ) {

					$('table.eshopbox_order_items tbody#order_items_list').append( response );
					...
					if (!--count) {
					    ...
					    $('table.eshopbox_order_items').unblock();
					}
	...
	// Add a fee
	$('#eshopbox-order-items button.add_order_fee').click(function(){

		$('table.eshopbox_order_items').block({ message: null, overlayCSS: { background: '#fff url(' + eshopbox_writepanel_params.plugin_url + '/assets/images/ajax-loader.gif) no-repeat center', opacity: 0.6 } });

		var data = {
			action: 		'eshopbox_add_order_fee',
			order_id:		eshopbox_writepanel_params.post_id,
			security: 		eshopbox_writepanel_params.order_item_nonce
		};
	...
	$('#order_items_list').on('click', 'button.add_order_item_meta', function(){
		...
		var data = {
			...
			action: 	'eshopbox_add_order_item_meta',
			security: 	eshopbox_writepanel_params.order_item_nonce
		};
	...
	$('#order_items_list').on('click', 'button.remove_order_item_meta', function(){
		var answer = confirm( eshopbox_writepanel_params.remove_item_meta )
		if ( answer ) {
			var $row = $(this).closest('tr');

			var data = {
				meta_id: 			$row.attr( 'data-meta_id' ),
				action: 			'eshopbox_remove_order_item_meta',
				security: 			eshopbox_writepanel_params.order_item_nonce
			};
	...
	$('#eshopbox-order-items').on( 'click', 'input.check-column', function() {
		if ( $(this).is(':checked') )
			$('#eshopbox-order-items').find('.check-column input').attr('checked', 'checked');
		else
			$('#eshopbox-order-items').find('.check-column input').removeAttr('checked');
	} );

	$('#eshopbox-order-items').on( 'click', '.do_bulk_action', function() {

		var action = $(this).closest('.bulk_actions').find('select').val();
		var selected_rows = $('#eshopbox-order-items').find('.check-column input:checked');
		var item_ids = [];
		...
		if ( item_ids.length == 0 ) {
			alert( eshopbox_writepanel_params.i18n_select_items );
			return;
		}

		if ( action == 'delete' ) {

			var answer = confirm( eshopbox_writepanel_params.remove_item_notice );
			...
			if ( answer ) {
				...
				var data = {
					...
					action: 			'eshopbox_remove_order_item',
					security: 			eshopbox_writepanel_params.order_item_nonce
				};
			...
			var data = {
				...
				action: 			'eshopbox_reduce_order_item_stock',
				security: 			eshopbox_writepanel_params.order_item_nonce
			};
			...
			var data = {
				...
				order_item_ids: 	item_ids,
				order_item_qty: 	quantities,
				action: 			'eshopbox_increase_order_item_stock',
				security: 			eshopbox_writepanel_params.order_item_nonce
			};
	...
	$('button.load_customer_billing').click(function(){
		...
		var answer = confirm(eshopbox_writepanel_params.load_billing);
		if (answer){
			...
			if (!user_id) {
				alert(eshopbox_writepanel_params.no_customer_selected);
				return false;
			}
			...
			var data = {
				...
				action: 			'eshopbox_get_customer_details',
				security: 			eshopbox_writepanel_params.get_customer_details_nonce
			};

			$(this).closest('.edit_address').block({ message: null, overlayCSS: { background: '#fff url(' + eshopbox_writepanel_params.plugin_url + '/assets/images/ajax-loader.gif) no-repeat center', opacity: 0.6 } });
	...
	$('button.load_customer_shipping').click(function(){
		...
		var answer = confirm(eshopbox_writepanel_params.load_shipping);
		if (answer){
	...
	$('button.billing-same-as-shipping').click(function(){
		var answer = confirm(eshopbox_writepanel_params.copy_billing);
		if (answer){
	...
	$('a.add_tax_row').click(function(){
		...
		var data = {
			order_id: 	eshopbox_writepanel_params.post_id,
			action: 	'eshopbox_add_line_tax',
			security: 	eshopbox_writepanel_params.calc_totals_nonce
		};

		$('#tax_rows').closest('.totals_group').block({ message: null, overlayCSS: { background: '#fff url(' + eshopbox_writepanel_params.plugin_url + '/assets/images/ajax-loader.gif) no-repeat center', opacity: 0.6 } });
	...
	$('#tax_rows').on('click','a.delete_tax_row',function(){
		...
		var data = {
			...
			action: 	'eshopbox_remove_line_tax',
			security: 	eshopbox_writepanel_params.calc_totals_nonce
		};
	...
	$('select#product-type').change(function(){
		...
		$('body').trigger('eshopbox-product-type-change', select_val, $(this) );
	}).change();
	...
	function show_and_hide_panels() {
		...
		$.each( eshopbox_writepanel_params.product_types, function( index, value ) {
	...
	$('#eshopbox-product-data').on( 'click', '.sale_schedule', function() {
	...
	$('#eshopbox-product-data').on( 'click', '.cancel_sale_schedule', function() {
		var $wrap = $(this).closest( 'div, table' );
	...
	var dates = $( ".sale_price_dates_fields input" ).datepicker({
		...
		showOn: "button",
		buttonImage: eshopbox_writepanel_params.calendar_image,
	...
	// ATTRIBUTE TABLES
		...
		$(".eshopbox_attributes select.multiselect").chosen();

		// Initial order
		var eshopbox_attribute_items = $('.eshopbox_attributes').find('.eshopbox_attribute').get();

		eshopbox_attribute_items.sort(function(a, b) {
		   var compA = parseInt($(a).attr('rel'));
		   var compB = parseInt($(b).attr('rel'));
		   return (compA < compB) ? -1 : (compA > compB) ? 1 : 0;
		})
		$(eshopbox_attribute_items).each( function(idx, itm) { $('.eshopbox_attributes').append(itm); } );

		function attribute_row_indexes() {
			$('.eshopbox_attributes .eshopbox_attribute').each(function(index, el){
				$('.attribute_position', el).val( parseInt( $(el).index('.eshopbox_attributes .eshopbox_attribute') ) );
			});
		};
		...
		$('button.add_attribute').on('click', function(){
			...
			var size = $('.eshopbox_attributes .eshopbox_attribute').size();
			...
			if (!attribute_type) {
				...
				$('.eshopbox_attributes').append('<div class="eshopbox_attribute wc-metabox">\
						<h3>\
							<button type="button" class="remove_row button">' + eshopbox_writepanel_params.remove_label + '</button>\
							<div class="handlediv" title="' + eshopbox_writepanel_params.click_to_toggle + '"></div>\
							<strong class="attribute_name"></strong>\
						</h3>\
						<table cellpadding="0" cellspacing="0" class="eshopbox_attribute_data">\
							<tbody>\
								<tr>\
									<td class="attribute_name">\
										<label>' + eshopbox_writepanel_params.name_label + ':</label>\
									...
									<td rowspan="3">\
										<label>' + eshopbox_writepanel_params.values_label + ':</label>\
										<textarea name="attribute_values[' + size + ']" cols="5" rows="5" placeholder="' + eshopbox_writepanel_params.text_attribute_tip + '"></textarea>\
									</td>\
								</tr>\
								<tr>\
									<td>\
										<label><input type="checkbox" class="checkbox" ' + ( eshopbox_writepanel_params.default_attribute_visibility ? 'checked="checked"' : '' ) + ' name="attribute_visibility[' + size + ']" value="1" /> ' + eshopbox_writepanel_params.visible_label + '</label>\
									</td>\
								</tr>\
								<tr>\
									<td>\
										<div class="enable_variation show_if_variable" ' + enable_variation + '>\
										<label><input type="checkbox" class="checkbox" ' + ( eshopbox_writepanel_params.default_attribute_variation ? 'checked="checked"' : '' ) + ' name="attribute_variation[' + size + ']" value="1" /> ' + eshopbox_writepanel_params.used_for_variations_label + '</label>\
										</div>\
									</td>\
								</tr>\
							</tbody>\
				...
				// Reveal taxonomy row
				var thisrow = $('.eshopbox_attributes .eshopbox_attribute.' + attribute_type);
				$('.eshopbox_attributes').append( $(thisrow) );
				$(thisrow).show().find('.eshopbox_attribute_data').show();
		...
		$('.eshopbox_attributes').on('blur', 'input.attribute_name', function(){
			$(this).closest('.eshopbox_attribute').find('strong.attribute_name').text( $(this).val() );
		});

		$('.eshopbox_attributes').on('click', 'button.select_all_attributes', function(){
			$(this).closest('td').find('select option').attr("selected","selected");
			$(this).closest('td').find('select').trigger("liszt:updated");
			return false;
		});

		$('.eshopbox_attributes').on('click', 'button.select_no_attributes', function(){
			$(this).closest('td').find('select option').removeAttr("selected");
			$(this).closest('td').find('select').trigger("liszt:updated");
			return false;
		});

		$('.eshopbox_attributes').on('click', 'button.remove_row', function() {
			var answer = confirm(eshopbox_writepanel_params.remove_attribute);
			if (answer){
		...
		// Attribute ordering
		$('.eshopbox_attributes').sortable({
			items:'.eshopbox_attribute',
			cursor:'move',
			axis:'y',
			handle: 'h3',
		...
		$('.eshopbox_attributes').on('click', 'button.add_new_attribute', function() {

			$('.eshopbox_attributes').block({ message: null, overlayCSS: { background: '#fff url(' + eshopbox_writepanel_params.plugin_url + '/assets/images/ajax-loader.gif) no-repeat center', opacity: 0.6 } });

			var attribute = $(this).attr('data-attribute');
			var $wrapper = $(this).closest('.eshopbox_attribute_data');
			var new_attribute_name = prompt( eshopbox_writepanel_params.new_attribute_prompt );

			if ( new_attribute_name ) {

				var data = {
					action: 		'eshopbox_add_new_attribute',
					taxonomy:		attribute,
					term:			new_attribute_name,
					security: 		eshopbox_writepanel_params.add_attribute_nonce
				};
		...
		$('.save_attributes').on('click', function(){
			...
			var data = {
				post_id: 		eshopbox_writepanel_params.post_id,
				data:			$('.eshopbox_attributes').find('input, select, textarea').serialize(),
				action: 		'eshopbox_save_attributes',
				security: 		eshopbox_writepanel_params.save_attributes_nonce
			};
			...
			$.post( eshopbox_writepanel_params.ajax_url, data, function( response ) {
				...
				this_page = this_page.replace( 'post-new.php?', 'post.php?post=' + eshopbox_writepanel_params.post_id + '&action=edit&' );

				// Load variations panel
				$('#variable_product_options').block({ message: null, overlayCSS: { background: '#fff url(' + eshopbox_writepanel_params.plugin_url + '/assets/images/ajax-loader.gif) no-repeat center', opacity: 0.6 } });
