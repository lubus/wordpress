jQuery(document).ready(function($) {
	...
	function update_checkout() {
		...
		$('#order_methods, #order_review').block({message: null, overlayCSS: {background: '#fff url(' + eshopbox_params.ajax_loader_url + ') no-repeat center', backgroundSize: '16px 16px', opacity: 0.6}});

		var data = {
			action: 			'eshopbox_update_order_review',
			security: 			eshopbox_params.update_order_review_nonce,
	...
	if ( eshopbox_params.option_guest_checkout == 'yes' ) {
	...
	.on( 'blur change', '.input-text, select', function() {
		...
		if ( $parent.is( '.validate-required' ) ) {
			if ( $this.val() == '' ) {
				$parent.removeClass( 'eshopbox-validated' ).addClass( 'eshopbox-invalid eshopbox-invalid-required-field' );
				validated = false;
			}
		}
		...
		if ( $parent.is( '.validate-email' ) ) {
			if ( $this.val() ) {
				...
				if ( ! pattern.test( $this.val()  ) ) {
					$parent.removeClass( 'eshopbox-validated' ).addClass( 'eshopbox-invalid eshopbox-invalid-email' );
					validated = false;
				}
			}
		}
		...
		if ( validated ) {
			$parent.removeClass( 'eshopbox-invalid eshopbox-invalid-required-field' ).addClass( 'eshopbox-validated' );
		}
	} )
	...
	.submit( function() {
		...
		if ( $form.triggerHandler('checkout_place_order') !== false && $form.triggerHandler('checkout_place_order_' + $('#order_review input[name=payment_method]:checked').val() ) !== false ) {
			...
			if ( form_data["blockUI.isBlocked"] != 1 )
				$form.block({message: null, overlayCSS: {background: '#fff url(' + eshopbox_params.ajax_loader_url + ') no-repeat center', backgroundSize: '16px 16px', opacity: 0.6}});

			$.ajax({
				type: 		'POST',
				url: 		eshopbox_params.checkout_url,
				data: 		$form.serialize(),
				success: 	function( code ) {
						...
						try {
							...
							// Remove old errors
							$('.eshopbox-error, .eshopbox-message').remove();
	...
	$('form.checkout_coupon').submit( function() {
		...
		$form.addClass('processing').block({message: null, overlayCSS: {background: '#fff url(' + eshopbox_params.ajax_loader_url + ') no-repeat center', backgroundSize: '16px 16px', opacity: 0.6}});

		var data = {
			action: 			'eshopbox_apply_coupon',
			security: 			eshopbox_params.apply_coupon_nonce,
	...
	/* Localisation */
	var locale_json = eshopbox_params.locale.replace(/&quot;/g, '"');
	var locale = $.parseJSON( locale_json );
	var required = ' <abbr class="required" title="' + eshopbox_params.i18n_required_text + '">*</abbr>';
	...
	// Update on page load
	if ( eshopbox_params.is_checkout == 1 ) {
