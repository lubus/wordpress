<?php
/**
 * Plugin Name: EshopBox
 * Plugin URI: http://www.woothemes.com/eshopbox/
 ...
 * Main EshopBox Class
 *
 * Contains the main functions for EshopBox, stores variables, and handles error messages
 *
 * @class Woocommerce
 * @version	2.0.0
 * @since 1.4
...
class Woocommerce {
	...
	public function __construct() {
		...
		add_filter( 'eshopbox_shipping_methods', array( $this, 'core_shipping' ) );
		add_filter( 'eshopbox_payment_gateways', array( $this, 'core_gateways' ) );
	...
	public function action_links( $links ) {
		...
		$plugin_links = array(
			'<a href="' . admin_url( 'admin.php?page=eshopbox_settings' ) . '">' . __( 'Settings', 'eshopbox' ) . '</a>',
			'<a href="http://docs.woothemes.com/documentation/plugins/eshopbox/">' . __( 'Docs', 'eshopbox' ) . '</a>',
			'<a href="http://support.woothemes.com/">' . __( 'Premium Support', 'eshopbox' ) . '</a>',
		);
	...
	public function activate() {
		if ( eshopbox_get_page_id( 'shop' ) < 1 )
	...
	public function update() {
		if ( ! defined( 'IFRAME_REQUEST' ) && ( get_option( 'eshopbox_version' ) != $this->version || get_option( 'eshopbox_db_version' ) != $this->version ) )
			$this->install();
	}
	 ...
	 */
	function install() {
		include_once( 'admin/eshopbox-admin-install.php' );
	...
	function includes() {
		...
		// Functions
		include_once( 'eshopbox-core-functions.php' );					// Contains core functions for the front/back end
	...
	public function admin_includes() {
		include_once( 'admin/eshopbox-admin-init.php' );			// Admin section
	}
	...
	public function ajax_includes() {
		include_once( 'eshopbox-ajax.php' );						// Ajax functions for admin and the front-end
	}
	...
	public function frontend_includes() {
		// Functions
		include_once( 'eshopbox-hooks.php' );						// Template hooks used on the front-end
		include_once( 'eshopbox-functions.php' );					// Contains functions for various front-end events
	...
	/**
	 * Function used to Init EshopBox Template Functions - This makes them pluggable by plugins and themes.
	 *
	 * @access public
	 * @return void
	 */
	public function include_template_functions() {
		include_once( 'eshopbox-template.php' );
	}
	...
	/**
	 * Init EshopBox when BoxBeat Initialises.
	 *
	 * @access public
	 * @return void
	 */
	public function init() {
		//Before init action
		do_action( 'before_eshopbox_init' );

		// Set up localisation
		$this->load_plugin_textdomain();

		// Variables
		$this->template_url			= apply_filters( 'eshopbox_template_url', 'eshopbox/' );
		...
		if ( ! is_admin() || defined('DOING_AJAX') ) {
			...
			$session_class = apply_filters( 'eshopbox_session_handler', 'WC_Session_Handler' );
		...
		// Email Actions
		$email_actions = array( 'eshopbox_low_stock', 'eshopbox_no_stock', 'eshopbox_product_on_backorder', 'eshopbox_order_status_pending_to_processing', 'eshopbox_order_status_pending_to_completed', 'eshopbox_order_status_pending_to_on-hold', 'eshopbox_order_status_failed_to_processing', 'eshopbox_order_status_failed_to_completed', 'eshopbox_order_status_pending_to_processing', 'eshopbox_order_status_pending_to_on-hold', 'eshopbox_order_status_completed', 'eshopbox_new_customer_note' );
	...
	public function load_plugin_textdomain() {
		$locale = apply_filters( 'plugin_locale', get_locale(), 'eshopbox' );
		$formal = 'yes' == get_option( 'eshopbox_informal_localisation_type' ) ? 'informal' : 'formal';

		load_textdomain( 'eshopbox', WP_LANG_DIR . "/eshopbox/eshopbox-$locale.mo" );

		// Load admin specific MO files
		if ( is_admin() ) {
			load_textdomain( 'eshopbox', WP_LANG_DIR . "/eshopbox/eshopbox-admin-$locale.mo" );
			load_textdomain( 'eshopbox', $this->plugin_path() . "/i18n/languages/eshopbox-admin-$locale.mo" );
		}

		load_plugin_textdomain( 'eshopbox', false, dirname( plugin_basename( __FILE__ ) ) . "/i18n/languages/$formal" );
		load_plugin_textdomain( 'eshopbox', false, dirname( plugin_basename( __FILE__ ) ) . "/i18n/languages" );
	}
	 ...
	 *
	 * Templates are in the 'templates' folder. eshopbox looks for theme
	 * overrides in /theme/eshopbox/ by default
	 *
	 * For beginners, it also looks for a eshopbox.php template first. If the user adds
	 * this to the theme (containing a eshopbox() inside) this will be used for all
	 * eshopbox templates.
	 *
	 * @access public
	...
	public function template_loader( $template ) {
		...
		} elseif ( is_post_type_archive( 'product' ) || is_page( eshopbox_get_page_id( 'shop' ) ) ) {
	...
	public function generator() {
		echo "\n\n" . '<!-- EshopBox Version -->' . "\n" . '<meta name="generator" content="EshopBox ' . esc_attr( $this->version ) . '" />' . "\n\n";
	}
	...
	public function wp_head() {
		...
		if ( is_eshopbox() ) {
			$this->add_body_class( 'eshopbox' );
			$this->add_body_class( 'eshopbox-page' );
			return;
		}

		if ( is_checkout() || is_order_received_page() ) {
			$this->add_body_class( 'eshopbox-checkout' );
			$this->add_body_class( 'eshopbox-page' );
			return;
		}

		if ( is_cart() ) {
			$this->add_body_class( 'eshopbox-cart' );
			$this->add_body_class( 'eshopbox-page' );
			return;
		}

		if ( is_account_page() ) {
			$this->add_body_class( 'eshopbox-account' );
			$this->add_body_class( 'eshopbox-page' );
			return;
		}
	...
	public function init_taxonomy() {
		...
		// Get bases
		$product_category_slug 	= empty( $permalinks['category_base'] ) ? _x( 'product-category', 'slug', 'eshopbox' ) : $permalinks['category_base'];
		$product_tag_slug 		= empty( $permalinks['tag_base'] ) ? _x( 'product-tag', 'slug', 'eshopbox' ) : $permalinks['tag_base'];
		$product_attribute_base	= empty( $permalinks['attribute_base'] ) ? '' : trailingslashit( $permalinks['attribute_base'] );
		$product_permalink 		= empty( $permalinks['product_base'] ) ? _x( 'product', 'slug', 'eshopbox' ) : $permalinks['product_base'];
		...
		$show_in_menu = current_user_can( 'manage_eshopbox' ) ? 'eshopbox' : true;

		/**
		 * Taxonomies
		 **/
		do_action( 'eshopbox_register_taxonomy' );

		$admin_only_query_var = is_admin();

		register_taxonomy( 'product_type',
	        apply_filters( 'eshopbox_taxonomy_objects_product_type', array('product') ),
	        apply_filters( 'eshopbox_taxonomy_args_product_type', array(
	            'hierarchical' 			=> false,
		...
		register_taxonomy( 'product_cat',
	        apply_filters( 'eshopbox_taxonomy_objects_product_cat', array('product') ),
	        apply_filters( 'eshopbox_taxonomy_args_product_cat', array(
	            'hierarchical' 			=> true,
	            'update_count_callback' => '_eshopbox_term_recount',
	            'label' 				=> __( 'Product Categories', 'eshopbox'),
	            'labels' => array(
	                    'name' 				=> __( 'Product Categories', 'eshopbox'),
	                    'singular_name' 	=> __( 'Product Category', 'eshopbox'),
						'menu_name'			=> _x( 'Categories', 'Admin menu name', 'eshopbox' ),
	                    'search_items' 		=> __( 'Search Product Categories', 'eshopbox'),
	                    'all_items' 		=> __( 'All Product Categories', 'eshopbox'),
	                    'parent_item' 		=> __( 'Parent Product Category', 'eshopbox'),
	                    'parent_item_colon' => __( 'Parent Product Category:', 'eshopbox'),
	                    'edit_item' 		=> __( 'Edit Product Category', 'eshopbox'),
	                    'update_item' 		=> __( 'Update Product Category', 'eshopbox'),
	                    'add_new_item' 		=> __( 'Add New Product Category', 'eshopbox'),
	                    'new_item_name' 	=> __( 'New Product Category Name', 'eshopbox')
	            	),
	            'show_ui' 				=> true,
	            'query_var' 			=> true,
	            'capabilities'			=> array(
	    ...
	    register_taxonomy( 'product_tag',
	        apply_filters( 'eshopbox_taxonomy_objects_product_tag', array('product') ),
	        apply_filters( 'eshopbox_taxonomy_args_product_tag', array(
	            'hierarchical' 			=> false,
	            'update_count_callback' => '_eshopbox_term_recount',
	            'label' 				=> __( 'Product Tags', 'eshopbox'),
	            'labels' => array(
	                    'name' 				=> __( 'Product Tags', 'eshopbox'),
	                    'singular_name' 	=> __( 'Product Tag', 'eshopbox'),
						'menu_name'			=> _x( 'Tags', 'Admin menu name', 'eshopbox' ),
	                    'search_items' 		=> __( 'Search Product Tags', 'eshopbox'),
	                    'all_items' 		=> __( 'All Product Tags', 'eshopbox'),
	                    'parent_item' 		=> __( 'Parent Product Tag', 'eshopbox'),
	                    'parent_item_colon' => __( 'Parent Product Tag:', 'eshopbox'),
	                    'edit_item' 		=> __( 'Edit Product Tag', 'eshopbox'),
	                    'update_item' 		=> __( 'Update Product Tag', 'eshopbox'),
	                    'add_new_item' 		=> __( 'Add New Product Tag', 'eshopbox'),
	                    'new_item_name' 	=> __( 'New Product Tag Name', 'eshopbox')
	            	),
	            'show_ui' 				=> true,
	            'query_var' 			=> true,
				'capabilities'			=> array(
		...
		register_taxonomy( 'product_shipping_class',
	        apply_filters( 'eshopbox_taxonomy_objects_product_shipping_class', array('product', 'product_variation') ),
	        apply_filters( 'eshopbox_taxonomy_args_product_shipping_class', array(
	            'hierarchical' 			=> true,
	            'update_count_callback' => '_update_post_term_count',
	            'label' 				=> __( 'Shipping Classes', 'eshopbox'),
	            'labels' => array(
	                    'name' 				=> __( 'Shipping Classes', 'eshopbox'),
	                    'singular_name' 	=> __( 'Shipping Class', 'eshopbox'),
						'menu_name'			=> _x( 'Shipping Classes', 'Admin menu name', 'eshopbox' ),
	                    'search_items' 		=> __( 'Search Shipping Classes', 'eshopbox'),
	                    'all_items' 		=> __( 'All Shipping Classes', 'eshopbox'),
	                    'parent_item' 		=> __( 'Parent Shipping Class', 'eshopbox'),
	                    'parent_item_colon' => __( 'Parent Shipping Class:', 'eshopbox'),
	                    'edit_item' 		=> __( 'Edit Shipping Class', 'eshopbox'),
	                    'update_item' 		=> __( 'Update Shipping Class', 'eshopbox'),
	                    'add_new_item' 		=> __( 'Add New Shipping Class', 'eshopbox'),
	                    'new_item_name' 	=> __( 'New Shipping Class Name', 'eshopbox')
	            	),
	            'show_ui' 				=> true,
	            'show_in_nav_menus' 	=> false,
	            'query_var' 			=> $admin_only_query_var,
	    ...
	    register_taxonomy( 'shop_order_status',
	        apply_filters( 'eshopbox_taxonomy_objects_shop_order_status', array('shop_order') ),
	        apply_filters( 'eshopbox_taxonomy_args_shop_order_status', array(
	            'hierarchical' 			=> false,
	            'update_count_callback' => '_update_post_term_count',
	            'show_ui' 				=> false,
	            'show_in_nav_menus' 	=> false,
		...
		if ( $attribute_taxonomies = $this->get_attribute_taxonomies() ) {
			foreach ($attribute_taxonomies as $tax) {
		    	...
		    	if ($name) {
					...
					$show_in_nav_menus = apply_filters( 'eshopbox_attribute_show_in_nav_menus', false, $name );

		    		register_taxonomy( $name,
				        apply_filters( 'eshopbox_taxonomy_objects_' . $name, array('product') ),
				        apply_filters( 'eshopbox_taxonomy_args_' . $name, array(
				            'hierarchical' 				=> $hierarchical,
	            			'update_count_callback' 	=> '_update_post_term_count',
				            'labels' => array(
				                    'name' 						=> $label,
				                    'singular_name' 			=> $label,
				                    'search_items' 				=> __( 'Search', 'eshopbox') . ' ' . $label,
				                    'all_items' 				=> __( 'All', 'eshopbox') . ' ' . $label,
				                    'parent_item' 				=> __( 'Parent', 'eshopbox') . ' ' . $label,
				                    'parent_item_colon' 		=> __( 'Parent', 'eshopbox') . ' ' . $label . ':',
				                    'edit_item' 				=> __( 'Edit', 'eshopbox') . ' ' . $label,
				                    'update_item' 				=> __( 'Update', 'eshopbox') . ' ' . $label,
				                    'add_new_item' 				=> __( 'Add New', 'eshopbox') . ' ' . $label,
				                    'new_item_name' 			=> __( 'New', 'eshopbox') . ' ' . $label
				            	),
				            'show_ui' 					=> false,
				            'query_var' 				=> true,
				            'capabilities'			=> array(
		 ...
		 * Post Types
		 **/
		do_action( 'eshopbox_register_post_type' );

		register_post_type( "product",
			apply_filters( 'eshopbox_register_post_type_product',
				array(
					'labels' => array(
							'name' 					=> __( 'Products', 'eshopbox' ),
							'singular_name' 		=> __( 'Product', 'eshopbox' ),
							'menu_name'				=> _x( 'Products', 'Admin menu name', 'eshopbox' ),
							'add_new' 				=> __( 'Add Product', 'eshopbox' ),
							'add_new_item' 			=> __( 'Add New Product', 'eshopbox' ),
							'edit' 					=> __( 'Edit', 'eshopbox' ),
							'edit_item' 			=> __( 'Edit Product', 'eshopbox' ),
							'new_item' 				=> __( 'New Product', 'eshopbox' ),
							'view' 					=> __( 'View Product', 'eshopbox' ),
							'view_item' 			=> __( 'View Product', 'eshopbox' ),
							'search_items' 			=> __( 'Search Products', 'eshopbox' ),
							'not_found' 			=> __( 'No Products found', 'eshopbox' ),
							'not_found_in_trash' 	=> __( 'No Products found in trash', 'eshopbox' ),
							'parent' 				=> __( 'Parent Product', 'eshopbox' )
						),
					'description' 			=> __( 'This is where you can add new products to your store.', 'eshopbox' ),
					'public' 				=> true,
					'show_ui' 				=> true,
		...
		register_post_type( "product_variation",
			apply_filters( 'eshopbox_register_post_type_product_variation',
				array(
					'labels' => array(
							'name' 					=> __( 'Variations', 'eshopbox' ),
							'singular_name' 		=> __( 'Variation', 'eshopbox' ),
							'add_new' 				=> __( 'Add Variation', 'eshopbox' ),
							'add_new_item' 			=> __( 'Add New Variation', 'eshopbox' ),
							'edit' 					=> __( 'Edit', 'eshopbox' ),
							'edit_item' 			=> __( 'Edit Variation', 'eshopbox' ),
							'new_item' 				=> __( 'New Variation', 'eshopbox' ),
							'view' 					=> __( 'View Variation', 'eshopbox' ),
							'view_item' 			=> __( 'View Variation', 'eshopbox' ),
							'search_items' 			=> __( 'Search Variations', 'eshopbox' ),
							'not_found' 			=> __( 'No Variations found', 'eshopbox' ),
							'not_found_in_trash' 	=> __( 'No Variations found in trash', 'eshopbox' ),
							'parent' 				=> __( 'Parent Variation', 'eshopbox' )
						),
					'public' 				=> true,
					'show_ui' 				=> false,
					'capability_type' 		=> 'product',
	    ...
	    register_post_type( "shop_order",
		    apply_filters( 'eshopbox_register_post_type_shop_order',
				array(
					'labels' => array(
							'name' 					=> __( 'Orders', 'eshopbox' ),
							'singular_name' 		=> __( 'Order', 'eshopbox' ),
							'add_new' 				=> __( 'Add Order', 'eshopbox' ),
							'add_new_item' 			=> __( 'Add New Order', 'eshopbox' ),
							'edit' 					=> __( 'Edit', 'eshopbox' ),
							'edit_item' 			=> __( 'Edit Order', 'eshopbox' ),
							'new_item' 				=> __( 'New Order', 'eshopbox' ),
							'view' 					=> __( 'View Order', 'eshopbox' ),
							'view_item' 			=> __( 'View Order', 'eshopbox' ),
							'search_items' 			=> __( 'Search Orders', 'eshopbox' ),
							'not_found' 			=> __( 'No Orders found', 'eshopbox' ),
							'not_found_in_trash' 	=> __( 'No Orders found in trash', 'eshopbox' ),
							'parent' 				=> __( 'Parent Orders', 'eshopbox' ),
							'menu_name'				=> $menu_name
						),
					'description' 			=> __( 'This is where store orders are stored.', 'eshopbox' ),
					'public' 				=> false,
					'show_ui' 				=> true,
	    ...
	    register_post_type( "shop_coupon",
		    apply_filters( 'eshopbox_register_post_type_shop_coupon',
				array(
					'labels' => array(
							'name' 					=> __( 'Coupons', 'eshopbox' ),
							'singular_name' 		=> __( 'Coupon', 'eshopbox' ),
							'menu_name'				=> _x( 'Coupons', 'Admin menu name', 'eshopbox' ),
							'add_new' 				=> __( 'Add Coupon', 'eshopbox' ),
							'add_new_item' 			=> __( 'Add New Coupon', 'eshopbox' ),
							'edit' 					=> __( 'Edit', 'eshopbox' ),
							'edit_item' 			=> __( 'Edit Coupon', 'eshopbox' ),
							'new_item' 				=> __( 'New Coupon', 'eshopbox' ),
							'view' 					=> __( 'View Coupons', 'eshopbox' ),
							'view_item' 			=> __( 'View Coupon', 'eshopbox' ),
							'search_items' 			=> __( 'Search Coupons', 'eshopbox' ),
							'not_found' 			=> __( 'No Coupons found', 'eshopbox' ),
							'not_found_in_trash' 	=> __( 'No Coupons found in trash', 'eshopbox' ),
							'parent' 				=> __( 'Parent Coupon', 'eshopbox' )
						),
					'description' 			=> __( 'This is where you can add new coupons that customers can use in your store.', 'eshopbox' ),
					'public' 				=> true,
					'show_ui' 				=> true,
	...
	public function frontend_scripts() {
		...
		$lightbox_en 			= get_option( 'eshopbox_enable_lightbox' ) == 'yes' ? true : false;
		$chosen_en 				= get_option( 'eshopbox_enable_chosen' ) == 'yes' ? true : false;
		$ajax_cart_en			= get_option( 'eshopbox_enable_ajax_add_to_cart' ) == 'yes' ? true : false;
		...
		if ( is_checkout() ) {
			if ( $chosen_en ) {
				...
				wp_enqueue_style( 'eshopbox_chosen_styles', $this->plugin_url() . '/assets/css/chosen.css' );
			}

			wp_enqueue_script( 'wc-checkout', $frontend_script_path . 'checkout' . $suffix . '.js', array( 'jquery', 'eshopbox' ), $this->version, true );
		}
		...
		if ( $lightbox_en && ( is_product() || ( ! empty( $post->post_content ) && strstr( $post->post_content, '[product_page' ) ) ) ) {
			...
			wp_enqueue_style( 'eshopbox_prettyPhoto_css', $this->plugin_url() . '/assets/css/prettyPhoto.css' );
		}
		...
		wp_enqueue_script( 'eshopbox', $frontend_script_path . 'eshopbox' . $suffix . '.js', array( 'jquery', 'jquery-blockui', 'jquery-placeholder' ), $this->version, true );
		...
		$eshopbox_params = array(
			'countries'                        => json_encode( $this->countries->get_allowed_country_states() ),
			'plugin_url'                       => $this->plugin_url(),
			'ajax_url'                         => $this->ajax_url(),
			'ajax_loader_url'                  => apply_filters( 'eshopbox_ajax_loader_url', $this->plugin_url() . '/assets/images/ajax-loader@2x.gif' ),
			'i18n_select_state_text'           => esc_attr__( 'Select an option&hellip;', 'eshopbox' ),
			'i18n_required_rating_text'        => esc_attr__( 'Please select a rating', 'eshopbox' ),
			'i18n_no_matching_variations_text' => esc_attr__( 'Sorry, no products matched your selection. Please choose a different combination.', 'eshopbox' ),
			'i18n_required_text'               => esc_attr__( 'required', 'eshopbox' ),
			'i18n_view_cart'                   => esc_attr__( 'View Cart &rarr;', 'eshopbox' ),
			'review_rating_required'           => get_option( 'eshopbox_review_rating_required' ),
			'update_order_review_nonce'        => wp_create_nonce( "update-order-review" ),
			'apply_coupon_nonce'               => wp_create_nonce( "apply-coupon" ),
			'option_guest_checkout'            => get_option( 'eshopbox_enable_guest_checkout' ),
			'checkout_url'                     => add_query_arg( 'action', 'eshopbox-checkout', $this->ajax_url() ),
			'is_checkout'                      => is_page( eshopbox_get_page_id( 'checkout' ) ) ? 1 : 0,
			'update_shipping_method_nonce'     => wp_create_nonce( "update-shipping-method" ),
			'cart_url'                         => get_permalink( eshopbox_get_page_id( 'cart' ) ),
			'cart_redirect_after_add'          => get_option( 'eshopbox_cart_redirect_after_add' )
		);

		if ( is_checkout() || is_cart() )
			$eshopbox_params['locale'] = json_encode( $this->countries->get_country_locale() );

		wp_localize_script( 'eshopbox', 'eshopbox_params', apply_filters( 'eshopbox_params', $eshopbox_params ) );

		// CSS Styles
		if ( ! defined( 'WOOCOMMERCE_USE_CSS' ) )
			define( 'WOOCOMMERCE_USE_CSS', get_option( 'eshopbox_frontend_css' ) == 'yes' ? true : false );

		if ( WOOCOMMERCE_USE_CSS ) {
			$css = file_exists( get_stylesheet_directory() . '/eshopbox/style.css' ) ? get_stylesheet_directory_uri() . '/eshopbox/style.css' : $this->plugin_url() . '/assets/css/eshopbox.css';

			wp_enqueue_style( 'eshopbox_frontend_styles', $css );
		}
	}
	...
	public function check_jquery() {
		...
		if ( ! empty( $wp_scripts->registered['jquery']->ver ) && ! empty( $wp_scripts->registered['jquery']->src ) && 0 >= version_compare( $wp_scripts->registered['jquery']->ver, '1.7' ) ) {
			...
			wp_register_script( 'jquery', '/eshop-includes/js/jquery/jquery.js', array(), '1.7' );
	...
	public function mailer() {
		if ( empty( $this->eshopbox_email ) ) {
			$this->eshopbox_email = new WC_Emails();
		}
		return $this->eshopbox_email;
	}
	 ...
	 * Get an image size.
	 *
	 * Variable is filtered by eshopbox_get_image_size_{image_size}
	 *
	 * @access public
	...
	public function get_image_size( $image_size ) {
		...
		if ( ! in_array( $image_size, array( 'shop_thumbnail', 'shop_catalog', 'shop_single' ) ) )
			return apply_filters( 'eshopbox_get_image_size_' . $image_size, '' );
		...
		return apply_filters( 'eshopbox_get_image_size_' . $image_size, $size );
	}
	...
	public function add_error( $error ) {
		$this->errors[] = apply_filters( 'eshopbox_add_error', $error );
	}
	...
	public function add_message( $message ) {
		$this->messages[] = apply_filters( 'eshopbox_add_message', $message );
	}
	...
	public function redirect( $location, $status ) {
		...
		return apply_filters( 'eshopbox_redirect', $location );
	}
	...
	public function get_attribute_taxonomies() {
		...
		if ( false === ( $attribute_taxonomies = get_transient( $transient_name ) ) ) {
			...
			$attribute_taxonomies = $wpdb->get_results( "SELECT * FROM " . $wpdb->prefix . "eshopbox_attribute_taxonomies" );

			set_transient( $transient_name, $attribute_taxonomies );
		}

		return apply_filters( 'eshopbox_attribute_taxonomies', $attribute_taxonomies );
	}
	...
	public function attribute_taxonomy_name( $name ) {
		return 'pa_' . eshopbox_sanitize_taxonomy_name( $name );
	}
	...
	public function attribute_label( $name ) {
		...
		if ( taxonomy_is_product_attribute( $name ) ) {
			$name = eshopbox_sanitize_taxonomy_name( str_replace( 'pa_', '', $name ) );

			$label = $wpdb->get_var( $wpdb->prepare( "SELECT attribute_label FROM {$wpdb->prefix}eshopbox_attribute_taxonomies WHERE attribute_name = %s;", $name ) );
		...
		return apply_filters( 'eshopbox_attribute_label', $label, $name );
	}
	...
	public function attribute_orderby( $name ) {
		...
		$orderby = $wpdb->get_var( $wpdb->prepare( "SELECT attribute_orderby FROM " . $wpdb->prefix . "eshopbox_attribute_taxonomies WHERE attribute_name = %s;", $name ) );

		return apply_filters( 'eshopbox_attribute_orderby', $orderby, $name );
	}
	...
	public function get_coupon_discount_types() {
		if ( ! isset( $this->coupon_discount_types ) ) {
			$this->coupon_discount_types = apply_filters( 'eshopbox_coupon_discount_types', array(
    			'fixed_cart' 	=> __( 'Cart Discount', 'eshopbox' ),
    			'percent' 		=> __( 'Cart % Discount', 'eshopbox' ),
    			'fixed_product'	=> __( 'Product Discount', 'eshopbox' ),
    			'percent_product'	=> __( 'Product % Discount', 'eshopbox' )
    		) );
		}
	...
	public function nonce_field( $action, $referer = true , $echo = true ) {
		return wp_nonce_field('eshopbox-' . $action, '_n', $referer, $echo );
	}
	...
	public function nonce_url( $action, $url = '' ) {
		return add_query_arg( '_n', wp_create_nonce( 'eshopbox-' . $action ), $url );
	}


	/**
	 * Check a nonce and sets eshopbox error in case it is invalid.
	 *
	...
	public function verify_nonce( $action, $method='_POST', $error_message = false ) {
		...
		$name = '_n';
		$action = 'eshopbox-' . $action;

		if ( $error_message === false ) $error_message = __( 'Action failed. Please refresh the page and retry.', 'eshopbox' );
	...
	public function clear_product_transients( $post_id = 0 ) {
		...
		$post_transients_to_clear = array(
			...
			'wc_average_rating_',
			'wc_rating_count_',
			'eshopbox_product_type_', // No longer used
 ...
 * Init eshopbox class
 */
$GLOBALS['eshopbox'] = new Woocommerce();
