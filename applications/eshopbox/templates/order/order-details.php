	...
	<tbody>
		...
		if (sizeof($order->get_items())>0) {
			...
			foreach($order->get_items() as $item) {
				...
				echo '
					<tr class = "' . esc_attr( apply_filters( 'eshopbox_order_table_item_class', 'order_table_item', $item, $order ) ) . '">
						<td class="product-name">' .
							apply_filters( 'eshopbox_order_table_product_title', '<a href="' . get_permalink( $item['product_id'] ) . '">' . $item['name'] . '</a>', $item ) . ' ' .
							apply_filters( 'eshopbox_order_table_item_quantity', '<strong class="product-quantity">&times; ' . $item['qty'] . '</strong>', $item );
				...
				if ( $_product && $_product->exists() && $_product->is_downloadable() && $order->is_download_permitted() ) {
					...
					foreach ( $download_file_urls as $file_url => $download_file_url ) {
						...
						$filename = eshopbox_get_filename_from_url( $file_url );

						$links[] = '<small><a href="' . $download_file_url . '">' . sprintf( __( 'Download file%s', 'eshopbox' ), ( count( $download_file_urls ) > 1 ? ' ' . ( $i + 1 ) . ': ' : ': ' ) ) . $filename . '</a></small>';
...
<?php endif; ?>
		...
		<address><p>
			<?php
				if (!$order->get_formatted_billing_address()) _e( 'N/A', 'eshopbox' ); else echo $order->get_formatted_billing_address();
			?>
		</p></address>
