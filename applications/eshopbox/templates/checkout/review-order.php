...
<div id="order_review">
	...
	<table class="shop_table">
		...
		<tfoot>
			<tr class="cart-subtotal">
				<th><?php _e( 'Cart Subtotal', 'eshopbox' ); ?></th>
			...
			<tr class="discount">
				<th><?php _e( 'Cart Discount', 'eshopbox' ); ?></th>
			...
			<tr class="discount">
				<th><?php _e( 'Order Discount', 'eshopbox' ); ?></th>
				...
				if (sizeof($eshopbox->cart->get_cart())>0) :
					foreach ($eshopbox->cart->get_cart() as $cart_item_key => $values) :
						...
						if ($_product->exists() && $values['quantity']>0) :
							echo '
								<tr class="' . esc_attr( apply_filters('eshopbox_checkout_table_item_class', 'checkout_table_item', $values, $cart_item_key ) ) . '">
									<td class="product-name">' .
										apply_filters( 'eshopbox_checkout_product_title', $_product->get_title(), $_product ) . ' ' .
										apply_filters( 'eshopbox_checkout_item_quantity', '<strong class="product-quantity">&times; ' . $values['quantity'] . '</strong>', $values, $cart_item_key ) .
										$eshopbox->cart->get_item_data( $values ) .
									'</td>
	...
	<div id="payment">
		...
		<div class="form-row place-order">
			...
			<?php
			$order_button_text = apply_filters('eshopbox_order_button_text', __( 'Place order', 'eshopbox' ));

			echo apply_filters('eshopbox_order_button_html', '<input type="submit" class="button alt" name="eshopbox_checkout_place_order" id="place_order" value="' . $order_button_text . '" />' );
			?>
