...
if ( ( ! is_home() && ! is_front_page() && ! ( is_post_type_archive() && get_option( 'page_on_front' ) == eshopbox_get_page_id( 'shop' ) ) ) || is_paged() ) {

	echo $wrap_before;

	if ( ! empty( $home ) ) {
		echo $before . '<a class="home" href="' . apply_filters( 'eshopbox_breadcrumb_home_url', home_url() ) . '">' . $home . '</a>' . $after . $delimiter;
	}
	...
	if ( is_category() ) {
		...
		echo $prepend . $before . __( 'Products tagged &ldquo;', 'eshopbox' ) . $queried_object->name . '&rdquo;' . $after;
		...
		$_name = eshopbox_get_page_id( 'shop' ) ? get_the_title( eshopbox_get_page_id( 'shop' ) ) : '';
		...
		if ( is_search() ) {
			...
			echo $before . '<a href="' . get_post_type_archive_link('product') . '">' . $_name . '</a>' . $delimiter . __( 'Search results for &ldquo;', 'eshopbox' ) . get_search_query() . '&rdquo;' . $after;
		...
		echo $before . __( 'Error 404', 'eshopbox' ) . $after;
		...
		echo $before . __( 'Search results for &ldquo;', 'eshopbox' ) . get_search_query() . '&rdquo;' . $after;

	} elseif ( is_tag() ) {

			echo $before . __( 'Posts tagged &ldquo;', 'eshopbox' ) . single_tag_title('', false) . '&rdquo;' . $after;

	} elseif ( is_author() ) {

		$userdata = get_userdata($author);
		echo $before . __( 'Author:', 'eshopbox' ) . ' ' . $userdata->display_name . $after;

	}

	if ( get_query_var( 'paged' ) )
		echo ' (' . __( 'Page', 'eshopbox' ) . ' ' . get_query_var( 'paged' ) . ')';
