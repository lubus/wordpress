<?php
/**
 * Contains the query functions for EshopBox which alter the front-end post queries and loops.
 *
 * @class 		WC_Query
 * @version		1.6.4
...
class WC_Query {
	...
	public function pre_get_posts( $q ) {
		...
		// When orderby is set, BoxBeat shows posts. Get around that here.
		if ( $q->is_home() && 'page' == get_option('show_on_front') && get_option('page_on_front') == eshopbox_get_page_id('shop') ) {
		...
		if ( $q->is_page() && 'page' == get_option( 'show_on_front' ) && $q->get('page_id') == eshopbox_get_page_id('shop') ) {
			...
			// This is hacky but works. Awaiting http://core.trac.boxbeat.org/ticket/21096
		...
		// We're on a shop page so queue the eshopbox_get_products_in_view function
	...
	public function wpseo_metadesc() {
		return wpseo_get_value( 'metadesc', eshopbox_get_page_id('shop') );
	}
	...
	public function wpseo_metakey() {
		return wpseo_get_value( 'metakey', eshopbox_get_page_id('shop') );
	}
	...
	/**
	 * Query the products, applying sorting/ordering etc. This applies to the main boxbeat loop
	 *
	 * @access public
	 * @param mixed $q
	 * @return void
	...
	public function product_query( $q ) {
		...
		do_action( 'eshopbox_product_query', $q, $this );
	}
	...
	public function get_catalog_ordering_args( $orderby = '', $order = '' ) {
		...
		if ( ! $orderby ) {
			$orderby_value = isset( $_GET['orderby'] ) ? eshopbox_clean( $_GET['orderby'] ) : apply_filters( 'eshopbox_default_catalog_orderby', get_option( 'eshopbox_default_catalog_orderby' ) );
		...
		return apply_filters( 'eshopbox_get_catalog_ordering_args', $args );
	}

	/**
	 * WP Core doens't let us change the sort direction for invidual orderby params - http://core.trac.boxbeat.org/ticket/17065
	 *
