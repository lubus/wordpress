 ...
 * EshopBox coupons
 *
 * The EshopBox coupons class gets coupon data from storage and checks coupon validity
 *
 * @class 		WC_Coupon
...
class WC_Coupon {
	...
	public function __construct( $code ) {
		...
		$this->code  = apply_filters( 'eshopbox_coupon_code', $code );

		// Coupon data lets developers create coupons through code
		$coupon_data = apply_filters( 'eshopbox_get_shop_coupon_data', false, $code );
        ...
        if ( $coupon_data ) {
            ...
            $coupon_id 	= $wpdb->get_var( $wpdb->prepare( apply_filters( 'eshopbox_coupon_code_query', "SELECT ID FROM $wpdb->posts WHERE post_title = %s AND post_type = 'shop_coupon' AND post_status = 'publish'" ), $this->code ) );

            if ( ! $coupon_id )
            	return;

			$coupon             = get_post( $coupon_id );
			$coupon->post_title = apply_filters( 'eshopbox_coupon_code', $coupon->post_title );
        ...
        do_action( 'eshopbox_coupon_loaded', $this );
	}
	...
	public function is_valid() {
		...
		if ( $this->id ) {
			...
			if ( $this->minimum_amount > 0 ) {
				if ( $this->minimum_amount > $eshopbox->cart->subtotal ) {
					$valid = false;
			...
			if ( sizeof( $this->product_ids ) > 0 ) {
				...
				if ( sizeof( $eshopbox->cart->get_cart() ) > 0 ) {
					foreach( $eshopbox->cart->get_cart() as $cart_item_key => $cart_item ) {
			...
			$valid = apply_filters( 'eshopbox_coupon_is_valid', $valid, $this );
	...
	public function add_coupon_message( $msg_code ) {
		...
		if ( $msg_code < 200 )
			$eshopbox->add_error( $this->get_coupon_error( $msg_code ) );
		else
			$eshopbox->add_message( $this->get_coupon_message( $msg_code ) );
	}
	...
	public function get_coupon_message( $msg_code ) {
		...
		switch ( $msg_code ) {
			case self::WC_COUPON_SUCCESS:
				$msg = __( 'Coupon code applied successfully.', 'eshopbox' );
			break;
			default:
				$msg = '';
			break;
		...
		return apply_filters( 'eshopbox_coupon_message', $msg, $msg_code, $this );
	}
	...
	public function get_coupon_error( $err_code ) {
		...
		switch ( $err_code ) {
			case self::E_WC_COUPON_INVALID_FILTERED:
				$err = __( 'Coupon is not valid.', 'eshopbox' );
			break;
			case self::E_WC_COUPON_NOT_EXIST:
				$err = __( 'Coupon does not exist!', 'eshopbox' );
			break;
			case self::E_WC_COUPON_INVALID_REMOVED:
				$err = sprintf( __( 'Sorry, it seems the coupon "%s" is invalid - it has now been removed from your order.', 'eshopbox' ), $this->code );
			break;
			case self::E_WC_COUPON_NOT_YOURS_REMOVED:
				$err = sprintf( __( 'Sorry, it seems the coupon "%s" is not yours - it has now been removed from your order.', 'eshopbox' ), $this->code );
			break;
			case self::E_WC_COUPON_ALREADY_APPLIED:
				$err = __( 'Coupon code already applied!', 'eshopbox' );
			break;
			case self::E_WC_COUPON_ALREADY_APPLIED_INDIV_USE_ONLY:
				$err = sprintf( __( 'Sorry, coupon "%s" has already been applied and cannot be used in conjunction with other coupons.', 'eshopbox' ), $this->code );
			break;
			case self::E_WC_COUPON_USAGE_LIMIT_REACHED:
				$err = __( 'Coupon usage limit has been reached.', 'eshopbox' );
			break;
			case self::E_WC_COUPON_EXPIRED:
				$err = __( 'This coupon has expired.', 'eshopbox' );
			break;
			case self::E_WC_COUPON_MIN_SPEND_LIMIT_NOT_MET:
				$err = sprintf( __( 'The minimum spend for this coupon is %s.', 'eshopbox' ), eshopbox_price( $this->minimum_amount ) );
			break;
			case self::E_WC_COUPON_NOT_APPLICABLE:
				$err = __( 'Sorry, this coupon is not applicable to your cart contents.', 'eshopbox' );
			break;
			case self::E_WC_COUPON_NOT_VALID_SALE_ITEMS:
				$err = __( 'Sorry, this coupon is not valid for sale items.', 'eshopbox' );
			break;
			default:
				$err = '';
			break;
		...
		return apply_filters( 'eshopbox_coupon_error', $err, $err_code, $this );
	}
	...
	public static function get_generic_coupon_error( $err_code ) {
		...
		switch ( $err_code ) {
			...
			case self::E_WC_COUPON_PLEASE_ENTER:
				$err = __( 'Please enter a coupon code.', 'eshopbox' );
			break;
			default:
				$err = '';
			break;
		...
		return apply_filters( 'eshopbox_coupon_error', $err, $err_code, null );
	}
