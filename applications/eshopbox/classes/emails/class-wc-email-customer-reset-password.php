...
class WC_Email_Customer_Reset_Password extends WC_Email {
	...
	function __construct() {
		...
		$this->title 			= __( 'Reset password', 'eshopbox' );
		$this->description		= __( 'Customer reset password emails are sent when a customer resets their password.', 'eshopbox' );

		$this->template_html 	= 'emails/customer-reset-password.php';
		$this->template_plain 	= 'emails/plain/customer-reset-password.php';

		$this->subject 			= __( 'Password Reset for {blogname}', 'eshopbox');
		$this->heading      	= __( 'Password Reset Instructions', 'eshopbox');

		// Trigger
		add_action( 'eshopbox_reset_password_notification', array( $this, 'trigger' ), 10, 2 );
