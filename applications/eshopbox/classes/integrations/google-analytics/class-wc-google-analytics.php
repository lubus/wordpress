...
class WC_Google_Analytics extends WC_Integration {
	...
	public function __construct() {
        ...
        $this->method_title     	= __( 'Google Analytics', 'eshopbox' );
        $this->method_description	= __( 'Google Analytics is a free service offered by Google that generates detailed statistics about the visitors to a website.', 'eshopbox' );
		...
		// Actions
		add_action( 'eshopbox_update_options_integration_google_analytics', array( $this, 'process_admin_options') );

		// Tracking code
		add_action( 'wp_footer', array( $this, 'google_tracking_code' ) );
		add_action( 'eshopbox_thankyou', array( $this, 'ecommerce_tracking_code' ) );

		// Event tracking code
		add_action( 'eshopbox_after_add_to_cart_button', array( $this, 'add_to_cart' ) );
    ...
    function init_form_fields() {
    	...
    	$this->form_fields = array(
			'ga_id' => array(
				'title' 			=> __( 'Google Analytics ID', 'eshopbox' ),
				'description' 		=> __( 'Log into your google analytics account to find your ID. e.g. <code>UA-XXXXX-X</code>', 'eshopbox' ),
				'type' 				=> 'text',
		    	'default' 			=> get_option('eshopbox_ga_id') // Backwards compat
			),
			'ga_set_domain_name' => array(
				'title' 			=> __( 'Set Domain Name', 'eshopbox' ),
				'description' 		=> sprintf( __( '(Optional) Sets the <code>_setDomainName</code> variable. <a href="%s">See here for more information</a>.', 'eshopbox' ), 'https://developers.google.com/analytics/devguides/collection/gajs/gaTrackingSite#multipleDomains' ),
				'type' 				=> 'text',
		    	'default' 			=> ''
			),
			'ga_standard_tracking_enabled' => array(
				'title' 			=> __( 'Tracking code', 'eshopbox' ),
				'label' 			=> __( 'Add tracking code to your site\'s footer. You don\'t need to enable this if using a 3rd party analytics plugin.', 'eshopbox' ),
				'type' 				=> 'checkbox',
				'checkboxgroup'		=> 'start',
				'default' 			=> get_option('eshopbox_ga_standard_tracking_enabled') ? get_option('eshopbox_ga_standard_tracking_enabled') : 'no'  // Backwards compat
			),
			'ga_ecommerce_tracking_enabled' => array(
				'label' 			=> __( 'Add eCommerce tracking code to the thankyou page', 'eshopbox' ),
				'type' 				=> 'checkbox',
				'checkboxgroup'		=> '',
				'default' 			=> get_option('eshopbox_ga_ecommerce_tracking_enabled') ? get_option('eshopbox_ga_ecommerce_tracking_enabled') : 'no'  // Backwards compat
			),
			'ga_event_tracking_enabled' => array(
				'label' 			=> __( 'Add event tracking code for add to cart actions', 'eshopbox' ),
				'type' 				=> 'checkbox',
				'checkboxgroup'		=> 'end',
				'default' 			=> 'no'
			)
	...
	function ecommerce_tracking_code( $order_id ) {
		...
		if ( $order->get_items() ) {
			foreach ( $order->get_items() as $item ) {
				...
				$code .= "'" . esc_js( $_product->get_sku() ? __( 'SKU:', 'eshopbox' ) . ' ' . $_product->get_sku() : $_product->id ) . "',";
				$code .= "'" . esc_js( $item['name'] ) . "',";

				if ( isset( $_product->variation_data ) ) {

					$code .= "'" . esc_js( eshopbox_get_formatted_variation( $_product->variation_data, true ) ) . "',";
	...
	function add_to_cart() {
		...
		$parameters['category'] = "'" . __( 'Products', 'eshopbox' ) . "'";
		$parameters['action'] = "'" . __( 'Add to cart', 'eshopbox' ) . "'";
		$parameters['label'] = "'" . esc_js( $product->get_sku() ? __('SKU:', 'eshopbox') . ' ' . $product->get_sku() : "#" . $product->id ) . "'";
	...
	private function event_tracking_code( $parameters, $selector ) {
		...
		$parameters = apply_filters( 'eshopbox_ga_event_tracking_parameters', $parameters );
...
add_filter('eshopbox_integrations', 'add_google_analytics_integration' );
