...
class WC_ShareThis extends WC_Integration {
	...
	public function __construct() {
        $this->id					= 'sharethis';
        $this->method_title     	= __( 'ShareThis', 'eshopbox' );
        $this->method_description	= __( 'ShareThis offers a sharing widget which will allow customers to share links to products with their friends.', 'eshopbox' );
	...
	<span class="st_twitter"></span><span class="st_email"></span><span class="st_sharethis" st_image="{image}"></span><span class="st_plusone_button"></span>
		...
		// Actions
		add_action( 'eshopbox_update_options_integration_sharethis', array( $this, 'process_admin_options' ) );

		// Share widget
		add_action( 'eshopbox_share', array( $this, 'sharethis_code' ) );
    }
    ...
    function init_form_fields() {
    	...
    	$this->form_fields = array(
			'publisher_id' => array(
				'title' 		=> __( 'ShareThis Publisher ID', 'eshopbox' ),
				'description' 	=> sprintf( __( 'Enter your %1$sShareThis publisher ID%2$s to show social sharing buttons on product pages.', 'eshopbox' ), '<a href="http://sharethis.com/account/">', '</a>' ),
				'type' 			=> 'text',
				'default' 		=> get_option('eshopbox_sharethis')
			),
			'sharethis_code' => array(
				'title' 		=> __( 'ShareThis Code', 'eshopbox' ),
				'description' 	=> __( 'You can tweak the ShareThis code by editing this option.', 'eshopbox' ),
				'type' 			=> 'textarea',
...
add_filter('eshopbox_integrations', 'add_sharethis_integration' );