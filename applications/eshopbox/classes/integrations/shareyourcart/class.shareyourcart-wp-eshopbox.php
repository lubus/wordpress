...
if ( ! class_exists( 'ShareYourCartEshopBox', false ) ) {

	/**
	 * ShareYourCartEshopBox class.
	 *
	 * @extends ShareYourCartWordpressPlugin
	 */
	class ShareYourCartEshopBox extends ShareYourCartWordpressPlugin{
	    ...
	    /**
	     * This function is to be used by EshopBox from version 2.0
	     *
	     * @access public
	     * @return void
	     */
		...
		private function _getProductDetails20( $product_id ) {
			...
			//EshopBox actually echoes the image
			ob_start();
			echo $product->get_image(); // older EshopBox versions might allready echo, but newer versions don't, so force it anyway
			$image = ob_get_clean();
		...
		/**
		 * _loadEshopBox function.
		 *
		 * @access private
		 * @return void
		 */
		private function _loadEshopBox(){
			// Sometimes the EshopBox Class is not loaded...
			if ( ! class_exists( 'Woocommerce', false ) ) {
				require_once( ABSPATH . 'eshop-content/plugins/woocommerce/woocommerce.php' );
			}

			// Important Classes Not included
			if ( ! function_exists( 'has_post_thumbnail' ) ) {
				require_once( ABSPATH . 'eshop-includes/post-thumbnail-template.php' );
			}
		}
	}

	//TODO: see why this is not used
	add_action( ShareYourCartWordpressPlugin::getPluginFile(), array( 'ShareYourCartEshopBox','uninstallHook' ) );
}
