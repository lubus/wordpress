 ...
 * Customer
 *
 * The EshopBox customer class handles storage of the current customer's data, such as location.
 *
 * @class 		WC_Customer
 * @version		1.6.4
...
class WC_Customer {
	...
	public function __construct() {
		...
		if ( empty( $eshopbox->session->customer ) ) {

			$default = apply_filters( 'eshopbox_customer_default_location', get_option( 'eshopbox_default_country' ) );
			...
			$this->_data = $eshopbox->session->customer;
	...
	public function save_data() {
		if ( $this->_changed )
			$GLOBALS['eshopbox']->session->customer = $this->_data;
	}
	...
	public function set_shipping_to_base() {
		global $eshopbox;
		$default = get_option('eshopbox_default_country');
    	if ( strstr( $default, ':' ) ) {
    		list( $country, $state ) = explode( ':', $default );
    	} else {
    		$country = $default;
	...
	public function get_taxable_address() {
		...
		$tax_based_on = get_option( 'eshopbox_tax_based_on' );

		// Check shipping method at this point to see if we need special handling
		$chosen_method = empty( $eshopbox->session->chosen_shipping_method ) ? get_option('eshopbox_default_shipping_method') : $eshopbox->session->chosen_shipping_method;

		if ( apply_filters( 'eshopbox_apply_base_tax_for_local_pickup', true ) == true && in_array( $chosen_method, apply_filters( 'eshopbox_local_pickup_methods', array( 'local_pickup' ) ) ) ) {
			$tax_based_on = 'base';
		}
		...
		return apply_filters( 'eshopbox_customer_taxable_address', array( $country, $state, $postcode, $city ) );
	}
	...
	public function get_downloadable_products() {
		...
		if ( is_user_logged_in() ) :
			...
			$results = $wpdb->get_results( $wpdb->prepare("SELECT * FROM ".$wpdb->prefix."eshopbox_downloadable_product_permissions WHERE user_id = '%s' ORDER BY order_id, product_id, download_id", get_current_user_id()) );
			...
			if ($results) foreach ($results as $result) :
				...
				if ($result->order_id>0) :
					...
					$download_name = apply_filters(
						'eshopbox_downloadable_product_name',
						$_product->get_title() . ( $file_number > 0 ? ' &mdash; ' . sprintf( __( 'File %d', 'eshopbox' ), $file_number + 1 ) : '' ),
						$_product,
						$result->download_id,
						$file_number
					);
					...
					if ( $file_number == 1 ) {
						...
						$previous_result['download_name'] = apply_filters(
							'eshopbox_downloadable_product_name',
							$previous_result['download_name']. ' &mdash; ' . sprintf( __('File %d', 'eshopbox' ), $file_number ),
							$previous_product,
		...
		return apply_filters('eshopbox_customer_get_downloadable_products', $downloads);
	}
}