...
class WC_Shipping {
	...
	/** @var array Stores methods loaded into eshopbox. */
     ...
     */
    function init() {
		do_action( 'eshopbox_shipping_init' );

		$this->enabled = ( get_option('eshopbox_calc_shipping') == 'no' ) ? false : true;
	}
	...
	function load_shipping_methods( $package = false ) {
		...
		do_action( 'eshopbox_load_shipping_methods', $package );

		// Register methods through a filter
		$shipping_methods_to_load = apply_filters( 'eshopbox_shipping_methods', array() );
	...
	function sort_shipping_methods() {
		...
		// Get order option
		$ordering 	= (array) get_option('eshopbox_shipping_method_order');
		$order_end 	= 999;
	...
	function calculate_shipping( $packages = array() ) {
		...
		// Get chosen method
		if ( ! empty( $eshopbox->session->chosen_shipping_method ) )
		...
		if ( sizeof( $_available_methods ) > 0 ) {
			...
			if ( empty( $chosen_method ) || ! isset( $_available_methods[ $chosen_method ] ) || $eshopbox->session->available_methods_count != sizeof( $_available_methods ) ) {

				$chosen_method = apply_filters( 'eshopbox_shipping_chosen_method', get_option( 'eshopbox_default_shipping_method' ), $_available_methods );
				...
				// Store chosen method
				$eshopbox->session->chosen_shipping_method = $chosen_method;

				// Do action for this chosen method
				do_action( 'eshopbox_shipping_method_chosen', $chosen_method );
			}
		...
		$eshopbox->session->available_methods_count = sizeof( $_available_methods );
	}
	...
	function calculate_shipping_for_package( $package = array() ) {
		...
		if ( false === ( $stored_rates = get_transient( $package_hash ) ) ) {
			...
			$package['rates'] = apply_filters( 'eshopbox_package_rates', $package['rates'], $package );
	...
	function get_available_shipping_methods() {
		...
		return apply_filters( 'eshopbox_available_shipping_methods', $available_methods );
	}
	...
	function reset_shipping() {
		global $eshopbox;
		unset( $eshopbox->session->chosen_shipping_method );
		$this->shipping_total = 0;
		$this->shipping_taxes = array();
		$this->shipping_label = null;
		$this->packages = array();
	...
	function process_admin_options() {
		...
		update_option( 'eshopbox_default_shipping_method', $default_shipping_method );
		update_option( 'eshopbox_shipping_method_order', $order );
	}
 ...
 * @since 1.5.7
 */
function eshopbox_register_shipping_method( $shipping_method ) {
	$GLOBALS['eshopbox']->shipping->register_shipping_method( $shipping_method );
}