 ...
 * EshopBox cart
 *
 * The EshopBox cart class stores cart data and active coupons as well as handling customer sessions and some cart related urls.
...
class WC_Cart {
	...
	public function __construct() {
		$this->tax = new WC_Tax();
		$this->prices_include_tax = ( get_option( 'eshopbox_prices_include_tax' ) == 'yes' ) ? true : false;
		$this->tax_display_cart = get_option( 'eshopbox_tax_display_cart' );
		$this->dp = (int) get_option( 'eshopbox_price_num_decimals' );
    ...
    /**
	 * Loads the cart data from the PHP session during BoxBeat init and hooks in other methods.
     *
     * @access public
     * @return void
     */
    public function init() {
		...
		add_action('eshopbox_check_cart_items', array( $this, 'check_cart_items' ), 1 );
		add_action('eshopbox_check_cart_items', array( $this, 'check_cart_coupons' ), 1 );
		add_action('eshopbox_after_checkout_validation', array( $this, 'check_customer_coupons' ), 1 );
    }
	...
	/*-----------------------------------------------------------------------------------*/
		...
		public function get_cart_from_session() {
			...
			// Load the coupons
			$this->applied_coupons         = ( empty( $eshopbox->session->coupon_codes ) ) ? array() : array_filter( (array) $eshopbox->session->coupon_codes );
			$this->coupon_discount_amounts = ( empty( $eshopbox->session->coupon_amounts ) ) ? array() : array_filter( (array) $eshopbox->session->coupon_amounts );

			// Load the cart
			if ( isset( $eshopbox->session->cart ) && is_array( $eshopbox->session->cart ) ) {
				$cart = $eshopbox->session->cart;
				...
				foreach ( $cart as $key => $values ) {
					...
					if ( ! empty( $_product ) && $_product->exists() && $values['quantity'] > 0 ) {
						...
						$this->cart_contents[ $key ] = apply_filters( 'eshopbox_get_cart_item_from_session', array(
			...
			// Trigger action
			do_action( 'eshopbox_cart_loaded_from_session', $this );

			// Load totals
			$this->cart_contents_total 	= isset( $eshopbox->session->cart_contents_total ) ? $eshopbox->session->cart_contents_total : 0;
			$this->cart_contents_weight = isset( $eshopbox->session->cart_contents_weight ) ? $eshopbox->session->cart_contents_weight : 0;
			$this->cart_contents_count 	= isset( $eshopbox->session->cart_contents_count ) ? $eshopbox->session->cart_contents_count : 0;
			$this->cart_contents_tax 	= isset( $eshopbox->session->cart_contents_tax ) ? $eshopbox->session->cart_contents_tax : 0;
			$this->total 				= isset( $eshopbox->session->total ) ? $eshopbox->session->total : 0;
			$this->subtotal 			= isset( $eshopbox->session->subtotal ) ? $eshopbox->session->subtotal : 0;
			$this->subtotal_ex_tax 		= isset( $eshopbox->session->subtotal_ex_tax ) ? $eshopbox->session->subtotal_ex_tax : 0;
			$this->tax_total 			= isset( $eshopbox->session->tax_total ) ? $eshopbox->session->tax_total : 0;
			$this->taxes 				= isset( $eshopbox->session->taxes ) ? $eshopbox->session->taxes : array();
			$this->shipping_taxes		= isset( $eshopbox->session->shipping_taxes ) ? $eshopbox->session->shipping_taxes : array();
			$this->discount_cart 		= isset( $eshopbox->session->discount_cart ) ? $eshopbox->session->discount_cart : 0;
			$this->discount_total 		= isset( $eshopbox->session->discount_total ) ? $eshopbox->session->discount_total : 0;
			$this->shipping_total 		= isset( $eshopbox->session->shipping_total ) ? $eshopbox->session->shipping_total : 0;
			$this->shipping_tax_total 	= isset( $eshopbox->session->shipping_tax_total ) ? $eshopbox->session->shipping_tax_total : 0;
			$this->shipping_label		= isset( $eshopbox->session->shipping_label ) ? $eshopbox->session->shipping_label : '';
		...
		public function set_session() {
			...
			$eshopbox->session->cart           = $cart_session;
			$eshopbox->session->coupon_codes   = $this->applied_coupons;
			$eshopbox->session->coupon_amounts = $this->coupon_discount_amounts;

			// Store totals to avoid re-calc on page load
			$eshopbox->session->cart_contents_total  = $this->cart_contents_total;
			$eshopbox->session->cart_contents_weight = $this->cart_contents_weight;
			$eshopbox->session->cart_contents_count  = $this->cart_contents_count;
			$eshopbox->session->cart_contents_tax    = $this->cart_contents_tax;
			$eshopbox->session->total                = $this->total;
			$eshopbox->session->subtotal             = $this->subtotal;
			$eshopbox->session->subtotal_ex_tax      = $this->subtotal_ex_tax;
			$eshopbox->session->tax_total            = $this->tax_total;
			$eshopbox->session->shipping_taxes       = $this->shipping_taxes;
			$eshopbox->session->taxes                = $this->taxes;
			$eshopbox->session->discount_cart        = $this->discount_cart;
			$eshopbox->session->discount_total       = $this->discount_total;
			$eshopbox->session->shipping_total       = $this->shipping_total;
			$eshopbox->session->shipping_tax_total   = $this->shipping_tax_total;
			$eshopbox->session->shipping_label       = $this->shipping_label;

			if ( get_current_user_id() )
				$this->persistent_cart_update();

			do_action( 'eshopbox_cart_updated' );
		}
		...
		public function empty_cart( $clear_persistent_cart = true ) {
			...
			unset( $eshopbox->session->order_awaiting_payment, $eshopbox->session->coupon_codes, $eshopbox->session->coupon_amounts, $eshopbox->session->cart );

			if ( $clear_persistent_cart && get_current_user_id() )
				$this->persistent_cart_destroy();

			do_action( 'eshopbox_cart_emptied' );
		}
	...
	/*-----------------------------------------------------------------------------------*/
		...
		public function persistent_cart_update() {
			...
			update_user_meta( get_current_user_id(), '_eshopbox_persistent_cart', array(
				'cart' => $eshopbox->session->cart,
			) );
		}
		...
		public function persistent_cart_destroy() {
			delete_user_meta( get_current_user_id(), '_eshopbox_persistent_cart' );
		}
	...
	/*-----------------------------------------------------------------------------------*/
		...
		public function coupons_enabled() {
			$coupons_enabled = get_option( 'eshopbox_enable_coupons' ) == 'no' ? false : true;

			return apply_filters( 'eshopbox_coupons_enabled', $coupons_enabled );
		}
		...
		public function get_cart_contents_count() {
			return apply_filters( 'eshopbox_cart_contents_count', $this->cart_contents_count );
		}
		...
		public function check_cart_items() {
			...
			if ( is_wp_error( $result ) )
				$eshopbox->add_error( $result->get_error_message() );
		}
			...
			if ( ! empty( $this->applied_coupons ) ) {
				foreach ( $this->applied_coupons as $key => $code ) {
					...
					if ( is_wp_error( $coupon->is_valid() ) ) {
						...
						$eshopbox->session->set( 'coupon_codes', $this->applied_coupons );
			...
			foreach ( $this->get_cart() as $cart_item_key => $values ) {
				...
				if ( $_product->managing_stock() ) {
					...
					if ( ! $_product->is_in_stock() || ! $_product->has_enough_stock( $values['quantity'] ) ) {
						$error->add( 'out-of-stock', sprintf(__( 'Sorry, we do not have enough "%s" in stock to fulfill your order (%s in stock). Please edit your cart and try again. We apologise for any inconvenience caused.', 'eshopbox' ), $_product->get_title(), $_product->stock ) );
						return $error;
					}
					...
					if ( $values['variation_id'] && $_product->variation_has_stock && isset( $product_qty_in_cart[ $values['variation_id'] ] ) ) {
						...
						if ( ! $_product->has_enough_stock( $product_qty_in_cart[ $values['variation_id'] ] ) ) {
							$error->add( 'out-of-stock', sprintf(__( 'Sorry, we do not have enough "%s" in stock to fulfil your order (%s in stock). Please edit your cart and try again. We apologise for any inconvenience caused.', 'eshopbox' ), $_product->get_title(), $_product->stock ) );
							return $error;
						}
					 ...
					 */
					if ( get_option( 'eshopbox_hold_stock_minutes' ) > 0 && ! $_product->backorders_allowed() ) {

						$order_id = isset( $eshopbox->session->order_awaiting_payment ) ? absint( $eshopbox->session->order_awaiting_payment ) : 0;
						...
						$held_stock = $wpdb->get_var( $wpdb->prepare( "
							...
							LEFT JOIN {$wpdb->prefix}eshopbox_order_items as order_items ON posts.ID = order_items.order_id
							...
							LEFT JOIN {$wpdb->prefix}eshopbox_order_itemmeta as order_item_meta2 ON order_items.order_item_id = order_item_meta2.order_item_id
						...
						if ( $_product->stock < ( $held_stock + $in_cart ) ) {
							$error->add( 'out-of-stock', sprintf(__( 'Sorry, we do not have enough "%s" in stock to fulfil your order right now. Please try again in %d minutes or edit your cart and try again. We apologise for any inconvenience caused.', 'eshopbox' ), $_product->get_title(), get_option( 'eshopbox_hold_stock_minutes' ) ) );
							return $error;
						}
					}
				 ...
				 */
					...
					if ( ! $_product->is_in_stock() ) {
						$error->add( 'out-of-stock', sprintf(__( 'Sorry, "%s" is not in stock. Please edit your cart and try again. We apologise for any inconvenience caused.', 'eshopbox' ), $_product->get_title() ) );
						return $error;
					}
				}
			}
		...
		public function get_item_data( $cart_item, $flat = false ) {
			...
			if ( ! empty( $cart_item['data']->variation_id ) && is_array( $cart_item['variation'] ) ) {
				...
				foreach ( $cart_item['variation'] as $name => $value ) {
		            ...
		            // If this is a custom option slug, get the options name
		            	...
		            	$value = apply_filters( 'eshopbox_variation_option_name', $value );
					}
			...
			$other_data = apply_filters( 'eshopbox_get_item_data', array(), $cart_item );
		...
		public function get_cart_url() {
			$cart_page_id = eshopbox_get_page_id('cart');
			if ( $cart_page_id ) return apply_filters( 'eshopbox_get_cart_url', get_permalink( $cart_page_id ) );
		}
		...
		public function get_checkout_url() {
			$checkout_page_id = eshopbox_get_page_id('checkout');
			$checkout_url     = '';
			if ( $checkout_page_id ) {
				if ( is_ssl() || get_option('eshopbox_force_ssl_checkout') == 'yes' )
					$checkout_url = str_replace( 'http:', 'https:', get_permalink( $checkout_page_id ) );
				else
					$checkout_url = get_permalink( $checkout_page_id );
			}
			return apply_filters( 'eshopbox_get_checkout_url', $checkout_url );
		}
		...
		public function get_remove_url( $cart_item_key ) {
			...
			$cart_page_id = eshopbox_get_page_id('cart');
			if ($cart_page_id)
				return apply_filters( 'eshopbox_get_remove_url', $eshopbox->nonce_url( 'cart', add_query_arg( 'remove_item', $cart_item_key, get_permalink($cart_page_id) ) ) );
		}
		...
		public function get_taxes() {
			...
			return apply_filters( 'eshopbox_cart_get_taxes', $taxes, $this );
		}
		...
		public function get_formatted_taxes() {
			...
			foreach ( $taxes as $key => $tax )
				if ( is_numeric( $tax ) )
					$taxes[ $key ] = eshopbox_price( $tax );

			return apply_filters( 'eshopbox_cart_formatted_taxes', $taxes, $this );
		}
		...
		public function get_tax_totals() {
			...
			foreach ( $taxes as $key => $tax ) {
				...
				$tax_totals[ $code ]->formatted_amount  = eshopbox_price( $tax_totals[ $code ]->amount );
			}

			return apply_filters( 'eshopbox_cart_tax_totals', $tax_totals, $this );
		}
	...
	/*-----------------------------------------------------------------------------------*/
		...
		public function add_to_cart( $product_id, $quantity = 1, $variation_id = '', $variation = '', $cart_item_data = array() ) {
			...
			$cart_item_data = (array) apply_filters( 'eshopbox_add_cart_item_data', $cart_item_data, $product_id, $variation_id );
			...
			if ( ! $product_data->is_purchasable() ) {
				$eshopbox->add_error( sprintf( __( 'Sorry, &quot;%s&quot; cannot be purchased.', 'eshopbox' ), $product_data->get_title() ) );
				return false;
			}
			...
			if ( ! $product_data->is_in_stock() ) {
				...
				$eshopbox->add_error( sprintf( __( 'You cannot add &quot;%s&quot; to the cart because the product is out of stock.', 'eshopbox' ), $product_data->get_title() ) );

				return false;
			} elseif ( ! $product_data->has_enough_stock( $quantity ) ) {

				$eshopbox->add_error( sprintf(__( 'You cannot add that amount of &quot;%s&quot; to the cart because there is not enough stock (%s remaining).', 'eshopbox' ), $product_data->get_title(), $product_data->get_stock_quantity() ));
			...
			if ( $product_data->is_sold_individually() ) {
				...
				if ( $in_cart_quantity > 0 ) {
					$eshopbox->add_error( sprintf('<a href="%s" class="button">%s</a> %s', get_permalink(eshopbox_get_page_id('cart')), __( 'View Cart &rarr;', 'eshopbox' ), __( 'You already have this item in your cart.', 'eshopbox' ) ) );
					return false;
				}
			}
			...
			if ( $product_data->managing_stock() ) {
				...
				if ( $variation_id && $product_data->variation_has_stock ) {
					...
					if ( isset( $product_qty_in_cart[ $variation_id ] ) && ! $product_data->has_enough_stock( $product_qty_in_cart[ $variation_id ] + $quantity ) ) {
						$eshopbox->add_error( sprintf(__( '<a href="%s" class="button">%s</a> You cannot add that amount to the cart &mdash; we have %s in stock and you already have %s in your cart.', 'eshopbox' ), get_permalink(eshopbox_get_page_id('cart')), __( 'View Cart &rarr;', 'eshopbox' ), $product_data->get_stock_quantity(), $product_qty_in_cart[ $variation_id ] ));
						return false;
					}
				...
				// Products
					...
					if ( isset( $product_qty_in_cart[ $product_id ] ) && ! $product_data->has_enough_stock( $product_qty_in_cart[ $product_id ] + $quantity ) ) {
						$eshopbox->add_error( sprintf(__( '<a href="%s" class="button">%s</a> You cannot add that amount to the cart &mdash; we have %s in stock and you already have %s in your cart.', 'eshopbox' ), get_permalink(eshopbox_get_page_id('cart')), __( 'View Cart &rarr;', 'eshopbox' ), $product_data->get_stock_quantity(), $product_qty_in_cart[ $product_id ] ));
						return false;
					}
			...
			if ( $cart_item_key ) {
				...
				$this->cart_contents[$cart_item_key] = apply_filters( 'eshopbox_add_cart_item', array_merge( $cart_item_data, array(
			...
			do_action( 'eshopbox_add_to_cart', $cart_item_key, $product_id, $quantity, $variation_id, $variation, $cart_item_data );
		...
		public function set_quantity( $cart_item_key, $quantity = 1, $refresh_totals = true ) {
			...
			if ( $quantity == 0 || $quantity < 0 ) {
				do_action( 'eshopbox_before_cart_item_quantity_zero', $cart_item_key );
				unset( $this->cart_contents[$cart_item_key] );
			} else {
				$this->cart_contents[$cart_item_key]['quantity'] = $quantity;
				do_action( 'eshopbox_after_cart_item_quantity_update', $cart_item_key, $quantity );
			}
		...
		private function set_cart_cookies( $set = true ) {
			if ( ! headers_sent() ) {
				if ( $set ) {
					setcookie( "eshopbox_items_in_cart", "1", 0, COOKIEPATH, COOKIE_DOMAIN, false );
					setcookie( "eshopbox_cart_hash", md5( json_encode( $this->get_cart() ) ), 0, COOKIEPATH, COOKIE_DOMAIN, false );
				} else {
					setcookie( "eshopbox_items_in_cart", "0", time() - 3600, COOKIEPATH, COOKIE_DOMAIN, false );
					setcookie( "eshopbox_cart_hash", "0", time() - 3600, COOKIEPATH, COOKIE_DOMAIN, false );
				}
			}
		}
	...
	/*-----------------------------------------------------------------------------------*/
		...
		private function reset() {
			...
			unset( $eshopbox->session->cart_contents_total, $eshopbox->session->cart_contents_weight, $eshopbox->session->cart_contents_count, $eshopbox->session->cart_contents_tax, $eshopbox->session->total, $eshopbox->session->subtotal, $eshopbox->session->subtotal_ex_tax, $eshopbox->session->tax_total, $eshopbox->session->taxes, $eshopbox->session->shipping_taxes, $eshopbox->session->discount_cart, $eshopbox->session->discount_total, $eshopbox->session->shipping_total, $eshopbox->session->shipping_tax_total, $eshopbox->session->shipping_label );
		}
		...
		public function get_discounted_price( $values, $price, $add_totals = false ) {
					...
					if ( $coupon->apply_before_tax() && $coupon->is_valid() ) {
						...
						switch ( $coupon->type ) {
							...
							case "percent_product" :
								...
								$product_ids_on_sale = eshopbox_get_product_ids_on_sale();
								...
								// Apply filter
								$this_item_is_discounted = apply_filters( 'eshopbox_item_is_discounted', $this_item_is_discounted, $values, $before_tax = true, $coupon );
			...
			return apply_filters( 'eshopbox_get_discounted_price', $price, $values, $this );
		}
		...
		public function apply_product_discounts_after_tax( $values, $price ) {
			...
			if ( ! empty( $this->applied_coupons) ) {
				foreach ( $this->applied_coupons as $code ) {
					...
					do_action( 'eshopbox_product_discount_after_tax_' . $coupon->type, $coupon, $values, $price );
					...
					if ( ! $coupon->apply_before_tax() ) {
						...
						// Apply filter
						$this_item_is_discounted = apply_filters( 'eshopbox_item_is_discounted', $this_item_is_discounted, $values, $before_tax = false, $coupon );
		...
		public function apply_cart_discounts_after_tax() {
			...
			if ( $this->applied_coupons ) {
				foreach ( $this->applied_coupons as $code ) {
					...
					do_action( 'eshopbox_cart_discount_after_tax_' . $coupon->type, $coupon );
		...
		public function calculate_totals() {
			...
			do_action( 'eshopbox_before_calculate_totals', $this );
				...
				foreach ( $this->cart_contents as $cart_item_key => $values ) {
					...
					if ( $this->prices_include_tax ) {
						...
						if ( $_product->is_taxable() ) {
							...
							$tax_amount = get_option('eshopbox_tax_round_at_subtotal') == 'no' ? $this->tax->get_tax_total( $taxes ) : array_sum( $taxes );
					...
					foreach ($this->cart_contents as $cart_item_key => $values ) {
						...
						if ( $_product->is_taxable() ) {
							...
							if ( $tax_rates !== $base_tax_rates ) {
								...
								$line_subtotal_tax     = get_option('eshopbox_tax_round_at_subtotal') == 'yes' ? $line_subtotal_tax : $this->tax->round( $line_subtotal_tax );
							 ...
							 */
								...
								// Line subtotal + tax
								$line_subtotal_tax 		= get_option('eshopbox_tax_round_at_subtotal') == 'no' ? $this->tax->round( $tax_amount ) : $tax_amount;
						...
						// Line prices
						$line_tax 		= get_option('eshopbox_tax_round_at_subtotal') == 'no' ? $this->tax->round( $discounted_tax_amount ) : $discounted_tax_amount;
			...
			if ( is_checkout() || is_cart() || defined('WOOCOMMERCE_CHECKOUT') || defined('WOOCOMMERCE_CART') ) {
				...
				if ( get_option('eshopbox_tax_round_at_subtotal') == 'no' ) {
				...
				if ( $eshopbox->customer->is_vat_exempt() )
					$this->remove_taxes();
				...
				do_action( 'eshopbox_calculate_totals', $this );
				...
				$this->total = max( 0, apply_filters( 'eshopbox_calculated_total', number_format( $this->cart_contents_total + $this->tax_total + $this->shipping_tax_total + $this->shipping_total - $this->discount_total + $this->fee_total, $this->dp, '.', '' ), $this ) );
		...
		public function needs_payment() {
			...
			return apply_filters( 'eshopbox_cart_needs_payment', $needs_payment, $this );
		}
	...
	/*-----------------------------------------------------------------------------------*/
		...
		public function calculate_shipping() {
			...
			if ( $this->needs_shipping() && $this->show_shipping() ) {
				$eshopbox->shipping->calculate_shipping( $this->get_shipping_packages() );
			} else {
				$eshopbox->shipping->reset_shipping();
			}

			// Get totals for the chosen shipping method
			$this->shipping_total 		= $eshopbox->shipping->shipping_total;	// Shipping Total
			$this->shipping_label 		= $eshopbox->shipping->shipping_label;	// Shipping Label
			$this->shipping_taxes		= $eshopbox->shipping->shipping_taxes;	// Shipping Taxes
		}
		...
		public function get_shipping_packages() {
			...
			$packages[0]['destination']['country']   = $eshopbox->customer->get_shipping_country();
			$packages[0]['destination']['state']     = $eshopbox->customer->get_shipping_state();
			$packages[0]['destination']['postcode']  = $eshopbox->customer->get_shipping_postcode();
			$packages[0]['destination']['city']      = $eshopbox->customer->get_shipping_city();
			$packages[0]['destination']['address']   = $eshopbox->customer->get_shipping_address();
			$packages[0]['destination']['address_2'] = $eshopbox->customer->get_shipping_address_2();

			foreach ( $this->get_cart() as $item )
				if ( $item['data']->needs_shipping() )
					$packages[0]['contents_cost'] += $item['line_total'];

			return apply_filters( 'eshopbox_cart_shipping_packages', $packages );
		}
		...
		public function needs_shipping() {
			if ( get_option('eshopbox_calc_shipping')=='no' ) return false;
			...
			return apply_filters( 'eshopbox_cart_needs_shipping', $needs_shipping );
		}
		...
		public function show_shipping() {
			...
			if ( get_option( 'eshopbox_shipping_cost_requires_address' ) == 'yes' ) {
				if ( ! $eshopbox->customer->has_calculated_shipping() ) {
					if ( ! $eshopbox->customer->get_shipping_country() || ( ! $eshopbox->customer->get_shipping_state() && ! $eshopbox->customer->get_shipping_postcode() ) ) return false;
				}
			}

			$show_shipping = true;

			return apply_filters( 'eshopbox_cart_ready_to_calc_shipping', $show_shipping );
		...
		public function ship_to_billing_address_only() {
			if ( get_option('eshopbox_ship_to_billing_address_only') == 'yes' ) return true; else return false;
		}
			...
			if ( isset( $this->shipping_label ) ) {
				if ( $this->shipping_total > 0 ) {
					...
					if ( $this->tax_display_cart == 'excl' ) {
						...
						$return = eshopbox_price( $this->shipping_total );
						...
						$return = eshopbox_price( $this->shipping_total + $this->shipping_tax_total );

						if ( $this->shipping_tax_total > 0 && ! $this->prices_include_tax ) {
							$return .= ' <small>' . $eshopbox->countries->inc_tax_or_vat() . '</small>';
						}
		...
		public function get_cart_shipping_title() {
			if ( isset( $this->shipping_label ) ) {
				return __( 'via', 'eshopbox' ) . ' ' . $this->shipping_label;
			}
			return false;
		}
	...
	/*-----------------------------------------------------------------------------------*/
		...
		public function has_discount( $coupon_code ) {
			...
			$coupon_code = apply_filters( 'eshopbox_coupon_code', $coupon_code );
		...
		public function add_discount( $coupon_code ) {
			...
			if ( $the_coupon->id ) {
				...
				if ( ! $the_coupon->is_valid() ) {
					$eshopbox->add_error( $the_coupon->get_error_message() );
					return false;
				}

				// Check if applied
				if ( $eshopbox->cart->has_discount( $coupon_code ) ) {
				...
				if ( $the_coupon->individual_use == 'yes' ) {
					$this->applied_coupons = apply_filters( 'eshopbox_apply_individual_use_coupon', array(), $the_coupon, $this->applied_coupons );
				}
				...
				if ( $this->applied_coupons ) {
					foreach ( $this->applied_coupons as $code ) {
						...
						if ( $existing_coupon->individual_use == 'yes' && false === apply_filters( 'eshopbox_apply_with_individual_use_coupon', false, $the_coupon, $existing_coupon, $this->applied_coupons ) ) {
				...
				if ( $the_coupon->enable_free_shipping() ) {
					$eshopbox->session->chosen_shipping_method = 'free_shipping';
				}
				...
				do_action( 'eshopbox_applied_coupon', $coupon_code );
		...
		public function remove_coupons( $type = 0 ) {
			...
			if ( 1 == $type ) {
				...
				unset( $eshopbox->session->coupon_codes, $eshopbox->session->coupon_amounts );
	...
	/*-----------------------------------------------------------------------------------*/
		...
		public function calculate_fees() {
			...
			do_action( 'eshopbox_cart_calculate_fees', $this );
	...
	/*-----------------------------------------------------------------------------------*/
		...
		public function get_total() {
			return apply_filters( 'eshopbox_cart_total', eshopbox_price( $this->total ) );
		}
		...
		public function get_total_ex_tax() {
			...
			if ( $total < 0 ) $total = 0;
			return apply_filters( 'eshopbox_cart_total_ex_tax', eshopbox_price( $total ) );
		}
		...
		public function get_cart_total() {
			if ( ! $this->prices_include_tax ) {
				$cart_contents_total = eshopbox_price( $this->cart_contents_total );
			} else {
				$cart_contents_total = eshopbox_price( $this->cart_contents_total + $this->tax_total );
			}

			return apply_filters( 'eshopbox_cart_contents_total', $cart_contents_total );
		}
		...
		public function get_cart_subtotal( $compound = false ) {
			...
			if ( $compound ) {
				...
				$cart_subtotal = eshopbox_price( $this->cart_contents_total + $this->shipping_total + $this->get_taxes_total( false ) );
			...
			// Otherwise we show cart items totals only (before discount)
				...
				if ( $this->tax_display_cart == 'excl' ) {
					...
					$cart_subtotal = eshopbox_price( $this->subtotal_ex_tax );

					if ( $this->tax_total > 0 && $this->prices_include_tax ) {
						$cart_subtotal .= ' <small>' . $eshopbox->countries->ex_tax_or_vat() . '</small>';
					}

				} else {

					$cart_subtotal = eshopbox_price( $this->subtotal );

					if ( $this->tax_total > 0 && !$this->prices_include_tax ) {
						$cart_subtotal .= ' <small>' . $eshopbox->countries->inc_tax_or_vat() . '</small>';
					}

				}
			}

			return apply_filters( 'eshopbox_cart_subtotal', $cart_subtotal, $compound, $this );
		}
		...
		public function get_product_subtotal( $_product, $quantity ) {
			...
			if ( $taxable ) {
				...
				if ( $this->tax_display_cart == 'excl' ) {
					...
					$product_subtotal = eshopbox_price( $row_price );

					if ( $this->prices_include_tax && $this->tax_total > 0 )
						$product_subtotal .= ' <small class="tax_label">' . $eshopbox->countries->ex_tax_or_vat() . '</small>';
					...
					if ( ! $this->prices_include_tax && $this->tax_total > 0 )
						$product_subtotal .= ' <small class="tax_label">' . $eshopbox->countries->inc_tax_or_vat() . '</small>';
			...
			return apply_filters( 'eshopbox_cart_product_subtotal', $product_subtotal, $_product, $quantity, $this );
		}
		...
		public function get_cart_tax() {
			...
			if ( $cart_total_tax > 0 ) $return = eshopbox_price( $cart_total_tax );
			return apply_filters( 'eshopbox_get_cart_tax', $return );
		}
		...
		public function get_discounts_before_tax() {
			if ( $this->discount_cart ) {
				$discounts_before_tax = eshopbox_price( $this->discount_cart );
			} else {
				$discounts_before_tax = false;
			}
			return apply_filters( 'eshopbox_cart_discounts_before_tax', $discounts_before_tax, $this );
		}
		...
		public function get_discounts_after_tax() {
			if ( $this->discount_total ) {
				$discounts_after_tax = eshopbox_price( $this->discount_total );
			} else {
				$discounts_after_tax = false;
			}
			return apply_filters( 'eshopbox_cart_discounts_after_tax', $discounts_after_tax, $this );
		}
		...
		public function get_total_discount() {
			if ( $this->discount_total || $this->discount_cart ) {
				$total_discount = eshopbox_price( $this->discount_total + $this->discount_cart );
			} else {
				$total_discount = false;
			}
			return apply_filters( 'eshopbox_cart_total_discount', $total_discount, $this );
		}
}