...
class WC_Widget_Recent_Products extends WP_Widget {
	...
	function WC_Widget_Recent_Products() {
		...
		$this->woo_widget_cssclass = 'eshopbox widget_recent_products';
		$this->woo_widget_description = __( 'Display a list of your most recent products on your site.', 'eshopbox' );
		$this->woo_widget_idbase = 'eshopbox_recent_products';
		$this->woo_widget_name = __( 'EshopBox Recent Products', 'eshopbox' );
	...
	function widget($args, $instance) {
		...
		$title = apply_filters('widget_title', empty($instance['title']) ? __('New Products', 'eshopbox' ) : $instance['title'], $instance, $this->id_base);
	    ...
	    if ( $show_variations == '0' ) {
		    $query_args['meta_query'][] = $eshopbox->query->visibility_meta_query();
			$query_args['parent'] = '0';
	    }
			...
			while ( $r->have_posts()) {
				...
				echo '<li>
					<a href="' . get_permalink() . '">
						' . ( has_post_thumbnail() ? get_the_post_thumbnail( $r->post->ID, 'shop_thumbnail' ) : eshopbox_placeholder_img( 'shop_thumbnail' ) ) . ' ' . get_the_title() . '
...
?>
		...
		<label for="<?php echo $this->get_field_id('show_variations'); ?>"><?php _e( 'Show hidden product variations', 'eshopbox' ); ?></label></p>
