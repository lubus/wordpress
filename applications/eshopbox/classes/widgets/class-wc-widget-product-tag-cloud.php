...
class WC_Widget_Product_Tag_Cloud extends WP_Widget {
	...
	function WC_Widget_Product_Tag_Cloud() {
		...
		$this->woo_widget_cssclass = 'eshopbox widget_product_tag_cloud';
		$this->woo_widget_description = __( 'Your most used product tags in cloud format.', 'eshopbox' );
		$this->woo_widget_idbase = 'eshopbox_product_tag_cloud';
		$this->woo_widget_name = __( 'EshopBox Product Tags', 'eshopbox' );
	...
	function widget( $args, $instance ) {
		...
		if ( !empty($instance['title']) ) {
			...
			if ( 'product_tag' == $current_taxonomy ) {
				$title = __('Product Tags', 'eshopbox' );
			} else {
		...
		wp_tag_cloud( apply_filters('eshopbox_product_tag_cloud_widget_args', array('taxonomy' => $current_taxonomy) ) );
		echo "</div>\n";
		echo $after_widget;
	}
