...
class WC_Widget_Top_Rated_Products extends WP_Widget {
	...
	function WC_Widget_Top_Rated_Products() {
		...
		$this->woo_widget_cssclass = 'eshopbox widget_top_rated_products';
		$this->woo_widget_description = __( 'Display a list of top rated products on your site.', 'eshopbox' );
		$this->woo_widget_idbase = 'eshopbox_top_rated_products';
		$this->woo_widget_name = __( 'EshopBox Top Rated Products', 'eshopbox' );
	...
	function widget($args, $instance) {
		...
		$title = apply_filters('widget_title', empty($instance['title']) ? __('Top Rated Products', 'eshopbox' ) : $instance['title'], $instance, $this->id_base);

		if ( !$number = (int) $instance['number'] ) $number = 10;
		else if ( $number < 1 ) $number = 1;
		else if ( $number > 15 ) $number = 15;

		add_filter( 'posts_clauses',  array( $eshopbox->query, 'order_by_rating_post_clauses' ) );
		...
		wp_reset_query();
		remove_filter( 'posts_clauses', array( $eshopbox->query, 'order_by_rating_post_clauses' ) );
