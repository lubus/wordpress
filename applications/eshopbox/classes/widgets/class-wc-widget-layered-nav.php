...
class WC_Widget_Layered_Nav extends WP_Widget {
	...
	function WC_Widget_Layered_Nav() {
		...
		$this->woo_widget_cssclass 		= 'eshopbox widget_layered_nav';
		$this->woo_widget_description	= __( 'Shows a custom attribute in a widget which lets you narrow down the list of products when viewing product categories.', 'eshopbox' );
		$this->woo_widget_idbase 		= 'eshopbox_layered_nav';
		$this->woo_widget_name 			= __( 'EshopBox Layered Nav', 'eshopbox' );

		/* Widget settings. */
		$widget_ops = array( 'classname' => $this->woo_widget_cssclass, 'description' => $this->woo_widget_description );

		/* Create the widget. */
		$this->WP_Widget( 'eshopbox_layered_nav', $this->woo_widget_name, $widget_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		$taxonomy 		= $eshopbox->attribute_taxonomy_name($instance['attribute']);
		...
		if ( count( $terms ) > 0 ) {
			...
			if ( $display_type == 'dropdown' ) {
				...
				if ( $current_tax && $taxonomy == $current_tax ) {
					...
					echo '<option value="">' . sprintf( __( 'Any %s', 'eshopbox' ), $eshopbox->attribute_label( $taxonomy ) ) .'</option>';
					...
					foreach ( $terms as $term ) {
						...
						if ( $query_type == 'and' ) {
							...
							$count = sizeof( array_intersect( $_products_in_term, $eshopbox->query->filtered_product_ids ) );
						...
						// If this is an OR query, show all options so search can be expanded
							...
							$count = sizeof( array_intersect( $_products_in_term, $eshopbox->query->unfiltered_product_ids ) );
...
$attr_color = get_eshopbox_term_meta($term->term_id,'pa_color_swatches_id_color');
                                        if($count > 0){
					/*echo '<li ' . $class . '>';

					echo ( $count > 0 || $option_is_set ) ? '<a href="' . esc_url( apply_filters( 'eshopbox_layered_nav_link', $link ) ) . '" style=background-color:'.$attr_color.';>' : '<span>';
					...
					echo '</li>';*/
					        ...
					        echo ( $count > 0 || $option_is_set ) ? '<a href="' . esc_url( apply_filters( 'eshopbox_layered_nav_link', $link ) ) . '"><span class="checkbox"></span><span class="colorswatch" style=background-color:'.$attr_color.';></span>' : '';
	...
	function update( $new_instance, $old_instance ) {
		...
		if ( empty( $new_instance['title'] ) )
			$new_instance['title'] = $eshopbox->attribute_label( $new_instance['attribute'] );
	...
	function form( $instance ) {
		...
		<p><label for="<?php echo $this->get_field_id( 'attribute' ); ?>"><?php _e( 'Attribute:', 'eshopbox' ) ?></label>
		...
		<p><label for="<?php echo $this->get_field_id( 'display_type' ); ?>"><?php _e( 'Display Type:', 'eshopbox' ) ?></label>
		<select id="<?php echo esc_attr( $this->get_field_id( 'display_type' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'display_type' ) ); ?>">
			<option value="list" <?php selected( $instance['display_type'], 'list' ); ?>><?php _e( 'List', 'eshopbox' ); ?></option>
			<option value="dropdown" <?php selected( $instance['display_type'], 'dropdown' ); ?>><?php _e( 'Dropdown', 'eshopbox' ); ?></option>
		</select></p>

		<p><label for="<?php echo $this->get_field_id( 'query_type' ); ?>"><?php _e( 'Query Type:', 'eshopbox' ) ?></label>
		<select id="<?php echo esc_attr( $this->get_field_id( 'query_type' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'query_type' ) ); ?>">
			<option value="and" <?php selected( $instance['query_type'], 'and' ); ?>><?php _e( 'AND', 'eshopbox' ); ?></option>
			<option value="or" <?php selected( $instance['query_type'], 'or' ); ?>><?php _e( 'OR', 'eshopbox' ); ?></option>
		</select></p>
		<?php
	}
}
