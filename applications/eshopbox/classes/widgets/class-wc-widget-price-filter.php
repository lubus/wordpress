...
class WC_Widget_Price_Filter extends WP_Widget {
	...
	function WC_Widget_Price_Filter() {
		...
		$this->woo_widget_cssclass = 'eshopbox widget_price_filter';
		$this->woo_widget_description = __( 'Shows a price filter slider in a widget which lets you narrow down the list of shown products when viewing product categories.', 'eshopbox' );
		$this->woo_widget_idbase = 'eshopbox_price_filter';
		$this->woo_widget_name = __( 'EshopBox Price Filter', 'eshopbox' );
	...
	function widget( $args, $instance ) {
		...
		global $_chosen_attributes, $wpdb, $eshopbox, $wp_query, $wp;

		if ( ! is_post_type_archive('product') && ! is_tax( get_object_taxonomies( 'product' ) ) ) return; // Not on product page - return

		if ( sizeof( $eshopbox->query->unfiltered_product_ids ) == 0 ) return; // None shown - return
		...
		wp_localize_script( 'wc-price-slider', 'eshopbox_price_slider_params', array(
		...
		if ( sizeof( $eshopbox->query->layered_nav_product_ids ) === 0 ) {
			...
			$max = ceil( $wpdb->get_var(
				...
				', $wpdb->posts, $wpdb->postmeta, '_price', implode( ',', $eshopbox->query->layered_nav_product_ids )
			) ) );
		}
		...
		echo '<form method="get" action="' . $form_action . '">
			<div class="price_slider_wrapper">
				...
				<div class="price_slider_amount">
					<input type="text" id="min_price" name="min_price" value="'.esc_attr( $min_price ).'" data-min="'.esc_attr( $min ).'" placeholder="'.__('Min price', 'eshopbox' ).'" />
					<input type="text" id="max_price" name="max_price" value="'.esc_attr( $max_price ).'" data-max="'.esc_attr( $max ).'" placeholder="'.__( 'Max price', 'eshopbox' ).'" />
					<button type="submit" class="button">'.__( 'Filter', 'eshopbox' ).'</button>
					<div class="price_label" style="display:none;">
						'.__( 'Price:', 'eshopbox' ).' <span class="from"></span> &mdash; <span class="to"></span>
					</div>
					'.$fields.'
					<div class="clear"></div>
				</div>
	...
	function update( $new_instance, $old_instance ) {
		if (!isset($new_instance['title']) || empty($new_instance['title'])) $new_instance['title'] = __( 'Filter by price', 'eshopbox' );
