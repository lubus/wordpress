...
class WC_Shipping_Local_Pickup extends WC_Shipping_Method {
	...
	function __construct() {
		...
		$this->method_title = __( 'Local Pickup', 'eshopbox' );
		$this->init();
	}
	...
	function init_form_fields() {
    	...
    	$this->form_fields = array(
			'enabled' => array(
				...
				'type' 			=> 'checkbox',
				'label' 		=> __( 'Enable local pickup', 'eshopbox' ),
				'default' 		=> 'no'
			),
			'title' => array(
				...
				'default'		=> __( 'Local Pickup', 'eshopbox' ),
				'desc_tip'      => true,
			),
			'codes' => array(
	...
	function admin_options() {
		...
		<p><?php _e( 'Local pickup is a simple method which allows the customer to pick up their order themselves.', 'eshopbox' ); ?></p>
	...
	function is_available( $package ) {
    	...
    	if ( $this->enabled == "no" ) {
			...
			if ( is_array( $codes ) ) {
				...
				if ( ! $found_match ) {
					...
					} elseif ( get_option( 'eshopbox_allowed_countries' ) == 'specific' ) {
		...
		return apply_filters( 'eshopbox_shipping_' . $this->id . '_is_available', $is_available, $package );
	}
