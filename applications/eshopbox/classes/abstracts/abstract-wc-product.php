 ...
 *
 * The EshopBox product class handles individual product data.
 *
 * @class 		WC_Product
 * @version		2.0.0
...
class WC_Product {
	...
	public function __construct( $product ) {
		...
		if ( ( function_exists( 'get_called_class' ) && get_called_class() == 'WC_Product' ) || ( ! function_exists( 'get_called_class' ) && is_null( $this->product_type ) ) ) {
			_doing_it_wrong( 'WC_Product', __( 'The <code>WC_Product</code> class is now abstract. Use <code>get_product()</code> to instantiate an instance of a product instead of calling this class directly.', 'eshopbox' ), '2.0 of EshopBox' );
	...
	function get_gallery_attachment_ids() {
		if ( ! isset( $this->product_image_gallery ) ) {
			// Backwards compat
			$attachment_ids = get_posts( 'post_parent=' . $this->id . '&numberposts=-1&post_type=attachment&orderby=menu_order&order=ASC&post_mime_type=image&fields=ids&meta_key=_eshopbox_exclude_image&meta_value=0' );
    ...
    function get_stock_quantity() {
        return $this->managing_stock() ? apply_filters( 'eshopbox_stock_amount', $this->stock ) : '';
    }
	...
	function set_stock( $amount = null ) {
		...
		if ( $this->managing_stock() && ! is_null( $amount ) ) {
			...
			$eshopbox->clear_product_transients( $this->id ); // Clear transient
	...
	function set_stock_status( $status ) {
		...
		if ( $this->stock_status != $status ) {
			...
			do_action( 'eshopbox_product_set_stock_status', $this->id, $status );
		}
	}
	...
	function get_file_download_path( $download_id ) {
		...
		$file_paths = (array) apply_filters( 'eshopbox_file_download_paths', $file_paths, $this->id, null, null );
		...
		return apply_filters( 'eshopbox_file_download_path', $file_path, $this->id, $download_id );
	}
	...
	function is_sold_individually() {
		...
		return apply_filters( 'eshopbox_is_sold_individually', $return, $this );
	}
	 ...
	 */
	function is_taxable() {
		return $this->tax_status == 'taxable' && get_option( 'eshopbox_calc_taxes' ) == 'yes' ? true : false;
	}
	 ...
	 */
	function get_title() {
		return apply_filters( 'eshopbox_product_title', apply_filters( 'the_title', $this->post->post_title, $this->id ), $this );
	}
	 ...
	 */
	function get_parent() {
		return apply_filters('eshopbox_product_parent', $this->post->post_parent, $this);
	}
	...
	function add_to_cart_url() {
		return apply_filters( 'eshopbox_add_to_cart_url', remove_query_arg( 'added-to-cart', add_query_arg( 'add-to-cart', $this->id ) ) );
	}
	...
	function managing_stock() {
		return ( ! isset( $this->manage_stock ) || $this->manage_stock == 'no' || get_option('eshopbox_manage_stock') != 'yes' ) ? false : true;
	}
	...
	function get_availability() {
		...
		if ( $this->managing_stock() ) {
			if ( $this->is_in_stock() ) {
				...
				if ( $this->get_total_stock() > 0 ) {
					...
					$format_option = get_option( 'eshopbox_stock_format' );

					switch ( $format_option ) {
						case 'no_amount' :
							$format = __( 'In stock', 'eshopbox' );
						break;
						case 'low_amount' :
							$low_amount = get_option( 'eshopbox_notify_low_stock_amount' );

							$format = ( $this->get_total_stock() <= $low_amount ) ? __( 'Only %s left in stock', 'eshopbox' ) : __( 'In stock', 'eshopbox' );
						break;
						default :
							$format = __( '%s in stock', 'eshopbox' );
						break;
					}
					...
					if ( $this->backorders_allowed() && $this->backorders_require_notification() )
						$availability .= ' ' . __( '(backorders allowed)', 'eshopbox' );

				} else {

					if ( $this->backorders_allowed() ) {
						if ( $this->backorders_require_notification() ) {
							$availability = __( 'Available on backorder', 'eshopbox' );
							$class        = 'available-on-backorder';
						} else {
							$availability = __( 'In stock', 'eshopbox' );
						}
					} else {
						$availability = __( 'Out of stock', 'eshopbox' );
						$class        = 'out-of-stock';
					}
		...
		return apply_filters( 'eshopbox_get_availability', array( 'availability' => $availability, 'class' => $class ), $this );
	}
	...
	function is_visible() {
		...
		if ( get_option( 'eshopbox_hide_out_of_stock_items' ) == 'yes' && ! $this->is_in_stock() ) $visible = false;
		...
		return apply_filters( 'eshopbox_product_is_visible', $visible, $this->id );
	}
	 ...
	 */
	function get_price() {
		return apply_filters( 'eshopbox_get_price', $this->price, $this );
	}
	...
	function is_purchasable() {
		...
		return apply_filters( 'eshopbox_is_purchasable', $purchasable, $this );
	}
	...
	function get_price_including_tax( $qty = 1 ) {
		...
		if ( $this->is_taxable() ) {
			...
			if ( get_option('eshopbox_prices_include_tax') == 'no' ) {
		...
		return apply_filters( 'eshopbox_get_price_including_tax', $price, $qty, $this );
	}
	...
	function get_price_excluding_tax( $qty = 1 ) {
		...
		if ( $this->is_taxable() && get_option('eshopbox_prices_include_tax') == 'yes' ) {
		...
		return apply_filters( 'eshopbox_get_price_excluding_tax', $price, $qty, $this );
	}
	...
	function get_tax_class() {
		return apply_filters( 'eshopbox_product_tax_class', $this->tax_class, $this );
	}
	...
	function get_price_html( $price = '' ) {
		...
		if ( $this->price > 0 ) {
			...
			if ( $this->is_on_sale() && isset( $this->regular_price ) ) {
				...
				$price = apply_filters( 'eshopbox_sale_price_html', $price, $this );

			} else {

				$price .= eshopbox_price( $this->get_price() );

				$price = apply_filters( 'eshopbox_price_html', $price, $this );

			}
		} elseif ( $this->price === '' ) {

			$price = apply_filters( 'eshopbox_empty_price_html', '', $this );

		} elseif ( $this->price == 0 ) {

			if ( $this->is_on_sale() && isset( $this->regular_price ) ) {

				$price .= $this->get_price_html_from_to( $this->regular_price, __( 'Free!', 'eshopbox' ) );

				$price = apply_filters( 'eshopbox_free_sale_price_html', $price, $this );

			} else {

				$price = __( 'Free!', 'eshopbox' );

				$price = apply_filters( 'eshopbox_free_price_html', $price, $this );

			}
		}

		return apply_filters( 'eshopbox_get_price_html', $price, $this );
	}
	...
	function get_price_html_from_text() {
		return '<span class="from">' . _x('From:', 'min_price', 'eshopbox') . ' </span>';
	}
	...
	function get_price_html_from_to( $from, $to ) {
		return '<del>' . ( ( is_numeric( $from ) ) ? eshopbox_price( $from ) : $from ) . '</del> <ins>' . ( ( is_numeric( $to ) ) ? eshopbox_price( $to ) : $to ) . '</ins>';
	}
	...
	function get_rating_html( $rating = null ) {
		...
		if ( $rating > 0 ) {
			...
			$rating_html  = '<div class="star-rating" title="' . sprintf( __( 'Rated %s out of 5', 'eshopbox' ), $rating ) . '">';

			$rating_html .= '<span style="width:' . ( ( $rating / 5 ) * 100 ) . '%"><strong class="rating">' . $rating . '</strong> ' . __( 'out of 5', 'eshopbox' ) . '</span>';
	...
	function get_related( $limit = 5 ) {
		...
		// Meta query
		$meta_query = array();
		$meta_query[] = $eshopbox->query->visibility_meta_query();
	    $meta_query[] = $eshopbox->query->stock_status_meta_query();
	    $meta_query   = array_filter( $meta_query );

		// Get the posts
		$related_posts = get_posts( apply_filters('eshopbox_product_related_posts', array(
			'orderby'        => 'rand',
	...
	function get_attribute( $attr ) {
		...
		if ( isset( $attributes[ $attr ] ) || isset( $attributes[ 'pa_' . $attr ] ) ) {
			...
			if ( $attribute['is_taxonomy'] ) {
				...
				return implode( ', ', eshopbox_get_product_terms( $this->id, $attribute['name'], 'names' ) );
	...
	function enable_dimensions_display() {
		return get_option( 'eshopbox_enable_dimension_product_attributes' ) == 'yes' ? true : false;
	}
	...
	function get_dimensions() {
		if ( ! $this->dimensions ) {
			...
			if ( ! empty( $this->dimensions ) )
				$this->dimensions .= ' ' . get_option( 'eshopbox_dimension_unit' );
	...
	function list_attributes() {
		eshopbox_get_template( 'single-product/product-attributes.php', array(
			'product'    => $this
		) );
	}
    ...
    function get_image( $size = 'shop_thumbnail', $attr = array() ) {
		...
		if ( has_post_thumbnail( $this->id ) ) {
			...
			$image = eshopbox_placeholder_img( $size );
		}
