...
/**
 * EshopBox Shipping Method Class
 *
...
abstract class WC_Shipping_Method extends WC_Settings_API {
	...
	/** @var bool Whether the method has settings or not (In EshopBox > Settings > Shipping) */
	...
	function add_rate( $args = array() ) {
		...
		if ( ! is_array( $taxes ) && $taxes !== false && $total_cost > 0 && get_option( 'eshopbox_calc_taxes' ) == 'yes' && $this->tax_status == 'taxable' ) {
    ...
    function is_available( $package ) {
		...
		if ( isset( $eshopbox->cart->cart_contents_total ) && isset( $this->min_amount ) && $this->min_amount && $this->min_amount > $eshopbox->cart->cart_contents_total )
			return false;
		...
		return apply_filters( 'eshopbox_shipping_' . $this->id . '_is_available', true, $package );
    }
	 ...
	 */
	function get_title() {
		return apply_filters( 'eshopbox_shipping_method_title', $this->title, $this->id );
	}
	...
	function supports( $feature ) {
		return apply_filters( 'eshopbox_shipping_method_supports', in_array( $feature, $this->supports ) ? true : false, $feature, $this );
	}
}