...
/**
 * EshopBox Payment Gateway class
 *
...
abstract class WC_Payment_Gateway extends WC_Settings_API {
	...
	function get_return_url( $order = '' ) {
		...
		return apply_filters( 'eshopbox_get_return_url', $return_url );
	}
	 ...
	 */
	function get_title() {
		return apply_filters( 'eshopbox_gateway_title', $this->title, $this->id );
	}
	...
	function get_description() {
		return apply_filters( 'eshopbox_gateway_description', $this->description, $this->id );
	}
	...
	function get_icon() {
		...
		$icon = $this->icon ? '<img src="' . $eshopbox->force_ssl( $this->icon ) . '" alt="' . $this->get_title() . '" />' : '';

		return apply_filters( 'eshopbox_gateway_icon', $icon, $this->id );
	}
	...
	function supports( $feature ) {
		return apply_filters( 'eshopbox_payment_gateway_supports', in_array( $feature, $this->supports ) ? true : false, $feature, $this );
	}
