 ...
 *
 * Provides EshopBox with Mijireh Checkout integration.
 *
...
class WC_Gateway_Mijireh extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->method_title = __( 'Mijireh Checkout', 'eshopbox' );
		$this->icon 		= apply_filters( 'eshopbox_mijireh_checkout_icon', $eshopbox->plugin_url() . '/classes/gateways/mijireh/assets/images/credit_cards.png' );
		...
		add_action( 'eshopbox_api_wc_gateway_mijireh', array( $this, 'mijireh_notification' ) );
	}
    ...
    if( isset( $_GET['order_number'] ) ) {
  	  global $eshopbox;
  		...
  		try {
  			...
  			$eshopbox->add_error( __( 'Mijireh error:', 'eshopbox' ) . $e->getMessage() );
    ...
    public function init_form_fields() {
		$this->form_fields = array(
			'enabled' => array(
				...
				'type' => 'checkbox',
				'label' => __( 'Enable Mijireh Checkout', 'eshopbox' ),
				'default' => 'no'
				),
			'access_key' => array(
				'title' => __( 'Access Key', 'eshopbox' ),
				'type' => 'text',
				'description' => __( 'The Mijireh access key for your store.', 'eshopbox' ),
				'default' => '',
				'desc_tip'      => true,
				),
			'title' => array(
				...
				'default' => __( 'Credit Card', 'eshopbox' ),
				'desc_tip'      => true,
				),
			'description' => array(
				'title' => __( 'Description', 'eshopbox' ),
				'type' => 'textarea',
				'default' => __( 'Pay securely with your credit card.', 'eshopbox' ),
	 ...
	 */
  	public function admin_options() {
		?>
		<h3><?php _e( 'Mijireh Checkout', 'eshopbox' );?></h3>

		<?php if ( empty( $this->access_key ) ) : ?>
			<div id="wc_get_started" class="mijireh">
				<span class="main"><?php _e( 'Get started with Mijireh Checkout', 'eshopbox' ); ?></span>
				<span><a href="http://www.mijireh.com/integrations/eshopbox/">Mijireh Checkout</a> <?php _e( 'provides a fully PCI Compliant, secure way to collect and transmit credit card data to your payment gateway while keeping you in control of the design of your site. Mijireh supports a wide variety of payment gateways: Stripe, Authorize.net, PayPal, eWay, SagePay, Braintree, PayLeap, and more.', 'eshopbox' ); ?></span>

				<p><a href="http://secure.mijireh.com/signup" target="_blank" class="button button-primary"><?php _e( 'Join for free', 'eshopbox' ); ?></a> <a href="http://www.mijireh.com/integrations/eshopbox/" target="_blank" class="button"><?php _e( 'Learn more about EshopBox and Mijireh', 'eshopbox' ); ?></a></p>

			</div>
		<?php else : ?>
			<p><a href="http://www.mijireh.com/integrations/eshopbox/">Mijireh Checkout</a> <?php _e( 'provides a fully PCI Compliant, secure way to collect and transmit credit card data to your payment gateway while keeping you in control of the design of your site.', 'eshopbox' ); ?></p>
		<?php endif; ?>
    ...
    public function process_payment( $order_id ) {
		...
		foreach( $items as $item ) {
			...
			if ( get_option( 'eshopbox_prices_include_tax' ) == 'yes' ) {
		...
		// add meta data to identify eshopbox order
	...
	public function init_mijireh() {
		if ( ! class_exists( 'Mijireh' ) ) {
	    	...
	    	if ( ! isset( $this ) ) {
		    	$settings = get_option( 'eshopbox_' . 'mijireh_checkout' . '_settings', null );
    ...
    public static function add_page_slurp_meta() {
    	...
    	if ( self::is_slurp_page() ) {
        	wp_enqueue_style( 'mijireh_css', $eshopbox->plugin_url() . '/classes/gateways/mijireh/assets/css/mijireh.css' );
        	wp_enqueue_script( 'pusher', 'https://d3dy5gmtp8yhk7.cloudfront.net/1.11/pusher.min.js', null, false, true );
        	wp_enqueue_script( 'page_slurp', $eshopbox->plugin_url() . '/classes/gateways/mijireh/assets/js/page_slurp.js', array('jquery'), false, true );
