 ...
 * @author 		WooThemes
 * @category 	Admin
 * @package 	EshopBox/Admin/Reports
 * @version     2.0.0
 */
 ...
 * @return array
 */
function eshopbox_get_reports_charts() {
	$charts = array(
		'sales'     => array(
			'title'  => __( 'Sales', 'eshopbox' ),
			'charts' => array(
				"overview"          => array(
					'title'       => __( 'Overview', 'eshopbox' ),
					'description' => '',
					'hide_title'  => true,
					'function'    => 'eshopbox_sales_overview'
				),
				"sales_by_day"      => array(
					'title'       => __( 'Sales by day', 'eshopbox' ),
					'description' => '',
					'function'    => 'eshopbox_daily_sales'
				),
				"sales_by_month"    => array(
					'title'       => __( 'Sales by month', 'eshopbox' ),
					'description' => '',
					'function'    => 'eshopbox_monthly_sales'
				),
				"product_sales"     => array(
					'title'       => __( 'Product Sales', 'eshopbox' ),
					'description' => '',
					'function'    => 'eshopbox_product_sales'
				),
				"top_sellers"       => array(
					'title'       => __( 'Top sellers', 'eshopbox' ),
					'description' => '',
					'function'    => 'eshopbox_top_sellers'
				),
				"top_earners"       => array(
					'title'       => __( 'Top earners', 'eshopbox' ),
					'description' => '',
					'function'    => 'eshopbox_top_earners'
				),
				"sales_by_category" => array(
					'title'       => __( 'Sales by category', 'eshopbox' ),
					'description' => '',
					'function'    => 'eshopbox_category_sales'
				) )
		),
		'coupons'   => array(
			...
			'charts' => array(
				"overview"            => array(
					...
					'description' => '',
					'hide_title'  => true,
					'function'    => 'eshopbox_coupons_overview'
				),
				"discounts_by_coupon" => array(
					'title'       => __( 'Discounts by coupon', 'eshopbox' ),
					'description' => '',
					'function'    => 'eshopbox_coupon_discounts'
				)
			)
		),
		'customers' => array(
			'title'  => __( 'Customers', 'eshopbox' ),
			'charts' => array(
				"overview" => array(
					'title'       => __( 'Overview', 'eshopbox' ),
					'description' => '',
					'hide_title'  => true,
					'function'    => 'eshopbox_customer_overview'
				),
			)
		),
		'stock'     => array(
			'title'  => __( 'Stock', 'eshopbox' ),
			'charts' => array(
				"overview" => array(
					'title'       => __( 'Overview', 'eshopbox' ),
					'description' => '',
					'hide_title'  => true,
					'function'    => 'eshopbox_stock_overview'
				),
			)
		)
	);

	if ( get_option( 'eshopbox_calc_taxes' ) == 'yes' ) {
		$charts['sales']['charts']["taxes_by_month"] = array(
			'title'       => __( 'Taxes by month', 'eshopbox' ),
			'description' => '',
			'function'    => 'eshopbox_monthly_taxes'
		);
	}

	return apply_filters( 'eshopbox_reports_charts', $charts );
}
 ...
 * @return void
 */
function eshopbox_reports() {

	$charts = eshopbox_get_reports_charts();
	...
	<div class="wrap eshopbox">
		<div class="icon32 icon32-eshopbox-reports" id="icon-eshopbox"><br /></div><h2 class="nav-tab-wrapper woo-nav-tab-wrapper">
			<?php
				foreach ( $charts as $key => $chart ) {
					echo '<a href="'.admin_url( 'admin.php?page=eshopbox_reports&tab=' . urlencode( $key ) ).'" class="nav-tab ';
					if ( $current_tab == $key ) echo 'nav-tab-active';
					echo '">' . esc_html( $chart[ 'title' ] ) . '</a>';
				}
			?>
			<?php do_action('eshopbox_reports_tabs'); ?>
		</h2>
			...
			<ul class="subsubsub">
				<li><?php
					...
					foreach ( $charts[ $current_tab ]['charts'] as $key => $chart ) {
						...
						$link = '<a href="admin.php?page=eshopbox_reports&tab=' . urlencode( $current_tab ) . '&amp;chart=' . urlencode( $key ) . '" class="';
 ...
 * @return void
 */
function eshopbox_weekend_area_js() {
	?>
    ...
    jQuery("#placeholder").bind("plothover", function (event, pos, item) {
        if (item) {
            if (previousPoint != item.dataIndex) {
                ...
                if (item.series.label=="<?php echo esc_js( __( 'Sales amount', 'eshopbox' ) ) ?>") {

                	var y = item.datapoint[1].toFixed(2);
                	showTooltip(item.pageX, item.pageY, item.series.label + " - " + "<?php echo get_eshopbox_currency_symbol(); ?>" + y);

                } else if (item.series.label=="<?php echo esc_js( __( 'Number of sales', 'eshopbox' ) ) ?>") {
 ...
 * @return void
 */
function eshopbox_datepicker_js() {
	global $eshopbox;
	?>
	var dates = jQuery( "#from, #to" ).datepicker({
		...
		showOn: "button",
		buttonImage: "<?php echo $eshopbox->plugin_url(); ?>/assets/images/calendar.png",
 ...
 */
function eshopbox_sales_overview() {
	global $start_date, $end_date, $eshopbox, $wpdb, $wp_locale;

	$total_sales = $total_orders = $order_items = $discount_total = $shipping_total = 0;

	$order_totals = apply_filters( 'eshopbox_reports_sales_overview_order_totals', $wpdb->get_row( "
		...
		AND		term.slug			IN ('" . implode( "','", apply_filters( 'eshopbox_reports_order_statuses', array( 'completed', 'processing', 'on-hold' ) ) ) . "')
	" ) );

	$total_sales 	= $order_totals->total_sales;
	$total_orders 	= absint( $order_totals->total_orders );

	$discount_total = apply_filters( 'eshopbox_reports_sales_overview_discount_total', $wpdb->get_var( "
	...
	$shipping_total = apply_filters( 'eshopbox_reports_sales_overview_shipping_total', $wpdb->get_var( "
		SELECT SUM(meta.meta_value) AS total_sales FROM {$wpdb->posts} AS posts
	...
	$order_items = apply_filters( 'eshopbox_reports_sales_overview_order_items', absint( $wpdb->get_var( "
		SELECT SUM( order_item_meta.meta_value )
		FROM {$wpdb->prefix}eshopbox_order_items as order_items
		LEFT JOIN {$wpdb->prefix}eshopbox_order_itemmeta as order_item_meta ON order_items.order_item_id = order_item_meta.order_item_id
	...
	" ) ) );
	?>
	<div id="poststuff" class="eshopbox-reports-wrap">
		<div class="eshopbox-reports-sidebar">
			<div class="postbox">
				<h3><span><?php _e( 'Total sales', 'eshopbox' ); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ( $total_sales > 0 ) echo eshopbox_price($total_sales); else _e( 'n/a', 'eshopbox' ); ?></p>
				</div>
			</div>
			<div class="postbox">
				<h3><span><?php _e( 'Total orders', 'eshopbox' ); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ( $total_orders > 0 ) echo $total_orders . ' (' . $order_items . ' ' . __( 'items', 'eshopbox' ) . ')'; else _e( 'n/a', 'eshopbox' ); ?></p>
				</div>
			</div>
			<div class="postbox">
				<h3><span><?php _e( 'Average order total', 'eshopbox' ); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ($total_orders>0) echo eshopbox_price($total_sales/$total_orders); else _e( 'n/a', 'eshopbox' ); ?></p>
				</div>
			</div>
			<div class="postbox">
				<h3><span><?php _e( 'Average order items', 'eshopbox' ); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ($total_orders>0) echo number_format($order_items/$total_orders, 2); else _e( 'n/a', 'eshopbox' ); ?></p>
				</div>
			</div>
			<div class="postbox">
				<h3><span><?php _e( 'Discounts used', 'eshopbox' ); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ($discount_total>0) echo eshopbox_price($discount_total); else _e( 'n/a', 'eshopbox' ); ?></p>
				</div>
			</div>
			<div class="postbox">
				<h3><span><?php _e( 'Total shipping costs', 'eshopbox' ); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ($shipping_total>0) echo eshopbox_price($shipping_total); else _e( 'n/a', 'eshopbox' ); ?></p>
				</div>
			</div>
		</div>
		<div class="eshopbox-reports-main">
			<div class="postbox">
				<h3><span><?php _e( 'This month\'s sales', 'eshopbox' ); ?></span></h3>
	...
	$orders = apply_filters('eshopbox_reports_sales_overview_orders', $wpdb->get_results( "
	...
	<script type="text/javascript">
		jQuery(function(){
			...
			var plot = jQuery.plot(placeholder, [ { label: "<?php echo esc_js( __( 'Number of sales', 'eshopbox' ) ) ?>", data: d }, { label: "<?php echo esc_js( __( 'Sales amount', 'eshopbox' ) ) ?>", data: d2, yaxis: 2 } ], {
				legend: {
			...
			<?php eshopbox_weekend_area_js(); ?>
			<?php eshopbox_tooltip_js(); ?>
		});
	</script>
	<?php
}
...
function eshopbox_daily_sales() {
	...
	// Get order ids and dates in range
	$orders = apply_filters( 'eshopbox_reports_daily_sales_orders', $wpdb->get_results( "
	...
	if ( $orders ) {
		...
		foreach ( $orders as $order ) {
			...
			// Get items
			$order_items += apply_filters( 'eshopbox_reports_daily_sales_order_items', absint( $wpdb->get_var( $wpdb->prepare( "
				SELECT SUM( order_item_meta.meta_value )
				FROM {$wpdb->prefix}eshopbox_order_items as order_items
				LEFT JOIN {$wpdb->prefix}eshopbox_order_itemmeta as order_item_meta ON order_items.order_item_id = order_item_meta.order_item_id
				WHERE	order_id = %d
	...
	<form method="post" action="">
		<p><label for="from"><?php _e( 'From:', 'eshopbox' ); ?></label> <input type="text" name="start_date" id="from" readonly="readonly" value="<?php echo esc_attr( date('Y-m-d', $start_date) ); ?>" /> <label for="to"><?php _e( 'To:', 'eshopbox' ); ?></label> <input type="text" name="end_date" id="to" readonly="readonly" value="<?php echo esc_attr( date('Y-m-d', $end_date) ); ?>" /> <input type="submit" class="button" value="<?php _e( 'Show', 'eshopbox' ); ?>" /></p>
	</form>

	<div id="poststuff" class="eshopbox-reports-wrap">
		<div class="eshopbox-reports-sidebar">
			<div class="postbox">
				<h3><span><?php _e( 'Total sales in range', 'eshopbox' ); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ( $total_sales > 0 ) echo eshopbox_price( $total_sales ); else _e( 'n/a', 'eshopbox' ); ?></p>
				</div>
			</div>
			<div class="postbox">
				<h3><span><?php _e( 'Total orders in range', 'eshopbox' ); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ( $total_orders > 0 ) echo $total_orders . ' (' . $order_items . ' ' . __( 'items', 'eshopbox' ) . ')'; else _e( 'n/a', 'eshopbox' ); ?></p>
				</div>
			</div>
			<div class="postbox">
				<h3><span><?php _e( 'Average order total in range', 'eshopbox' ); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ( $total_orders > 0 ) echo eshopbox_price( $total_sales / $total_orders ); else _e( 'n/a', 'eshopbox' ); ?></p>
				</div>
			</div>
			<div class="postbox">
				<h3><span><?php _e( 'Average order items in range', 'eshopbox' ); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ( $total_orders > 0 ) echo number_format( $order_items / $total_orders, 2 ); else _e( 'n/a', 'eshopbox' ); ?></p>
				</div>
			</div>
		...
		<div class="eshopbox-reports-main">
			<div class="postbox">
				<h3><span><?php _e( 'Sales in range', 'eshopbox' ); ?></span></h3>
				<div class="inside chart">
					<div id="placeholder" style="width:100%; overflow:hidden; height:568px; position:relative;"></div>
					<div id="cart_legend"></div>
				</div>
...
function eshopbox_monthly_sales() {
	...
	for ( $count = 0; $count < 12; $count++ ) {
		...
		$months_orders = apply_filters( 'eshopbox_reports_monthly_sales_orders', $wpdb->get_row( $wpdb->prepare( "
			SELECT SUM(meta.meta_value) AS total_sales, COUNT(posts.ID) AS total_orders FROM {$wpdb->posts} AS posts
		...
		// Count order items
		$order_items += apply_filters( 'eshopbox_reports_monthly_sales_order_items', absint( $wpdb->get_var( $wpdb->prepare( "
			SELECT SUM( order_item_meta.meta_value )
			FROM {$wpdb->prefix}eshopbox_order_items as order_items
			LEFT JOIN {$wpdb->prefix}eshopbox_order_itemmeta as order_item_meta ON order_items.order_item_id = order_item_meta.order_item_id
			LEFT JOIN {$wpdb->posts} AS posts ON order_items.order_id = posts.ID
	...
	?>
	<form method="post" action="">
		<p><label for="show_year"><?php _e( 'Year:', 'eshopbox' ); ?></label>
		<select name="show_year" id="show_year">
			<?php
				for ( $i = $first_year; $i <= date( 'Y' ); $i++ )
					printf('<option value="%s" %s>%s</option>', $i, selected( $current_year, $i, false ), $i );
			?>
		</select> <input type="submit" class="button" value="<?php _e( 'Show', 'eshopbox' ); ?>" /></p>
	</form>
	<div id="poststuff" class="eshopbox-reports-wrap">
		<div class="eshopbox-reports-sidebar">
			<div class="postbox">
				<h3><span><?php _e( 'Total sales for year', 'eshopbox' ); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ($total_sales>0) echo eshopbox_price($total_sales); else _e( 'n/a', 'eshopbox' ); ?></p>
				</div>
			</div>
			<div class="postbox">
				<h3><span><?php _e( 'Total orders for year', 'eshopbox' ); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ( $total_orders > 0 ) echo $total_orders . ' (' . $order_items . ' ' . __( 'items', 'eshopbox' ) . ')'; else _e( 'n/a', 'eshopbox' ); ?></p>
				</div>
			</div>
			<div class="postbox">
				<h3><span><?php _e( 'Average order total for year', 'eshopbox' ); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ($total_orders>0) echo eshopbox_price($total_sales/$total_orders); else _e( 'n/a', 'eshopbox' ); ?></p>
				</div>
			</div>
			<div class="postbox">
				<h3><span><?php _e( 'Average order items for year', 'eshopbox' ); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ($total_orders>0) echo number_format($order_items/$total_orders, 2); else _e( 'n/a', 'eshopbox' ); ?></p>
				</div>
			</div>
		...
		<div class="eshopbox-reports-main">
			<div class="postbox">
				<h3><span><?php _e( 'Monthly sales for year', 'eshopbox' ); ?></span></h3>
				<div class="inside chart">
					<div id="placeholder" style="width:100%; overflow:hidden; height:568px; position:relative;"></div>
					<div id="cart_legend"></div>
				</div>
...
function eshopbox_top_sellers() {
	...
	global $start_date, $end_date, $eshopbox, $wpdb;
	...
	// Get order ids and dates in range
	$order_items = apply_filters( 'eshopbox_reports_top_sellers_order_items', $wpdb->get_results( "
		SELECT order_item_meta_2.meta_value as product_id, SUM( order_item_meta.meta_value ) as item_quantity FROM {$wpdb->prefix}eshopbox_order_items as order_items

		LEFT JOIN {$wpdb->prefix}eshopbox_order_itemmeta as order_item_meta ON order_items.order_item_id = order_item_meta.order_item_id
		LEFT JOIN {$wpdb->prefix}eshopbox_order_itemmeta as order_item_meta_2 ON order_items.order_item_id = order_item_meta_2.order_item_id
		LEFT JOIN {$wpdb->posts} AS posts ON order_items.order_id = posts.ID
		LEFT JOIN {$wpdb->term_relationships} AS rel ON posts.ID = rel.object_ID
		LEFT JOIN {$wpdb->term_taxonomy} AS tax USING( term_taxonomy_id )
		LEFT JOIN {$wpdb->terms} AS term USING( term_id )
	...
	<table class="bar_chart">
		<thead>
			<tr>
				<th><?php _e( 'Product', 'eshopbox' ); ?></th>
				<th><?php _e( 'Sales', 'eshopbox' ); ?></th>
			</tr>
		</thead>
		<tbody>
			<?php
				...
				foreach ( $found_products as $product_id => $sales ) {
					...
					if ( $product_title ) {
						...
						$orders_link = admin_url( 'edit.php?s&post_status=all&post_type=shop_order&action=-1&s=' . urlencode( $product_title ) . '&shop_order_status=' . implode( ",", apply_filters( 'eshopbox_reports_order_statuses', array( 'completed', 'processing', 'on-hold' ) ) ) );
					} else {
						$product_name = __( 'Product does not exist', 'eshopbox' );
						$orders_link = admin_url( 'edit.php?s&post_status=all&post_type=shop_order&action=-1&s=&shop_order_status=' . implode( ",", apply_filters( 'eshopbox_reports_order_statuses', array( 'completed', 'processing', 'on-hold' ) ) ) );
					}

					$orders_link = apply_filters( 'eshopbox_reports_order_link', $orders_link, $product_id, $product_title );
...
function eshopbox_top_earners() {
	...
	// Get order ids and dates in range
	$order_items = apply_filters( 'eshopbox_reports_top_earners_order_items', $wpdb->get_results( "
		SELECT order_item_meta_2.meta_value as product_id, SUM( order_item_meta.meta_value ) as line_total FROM {$wpdb->prefix}eshopbox_order_items as order_items
	...
	<table class="bar_chart">
		<thead>
			<tr>
				<th><?php _e( 'Product', 'eshopbox' ); ?></th>
				<th colspan="2"><?php _e( 'Sales', 'eshopbox' ); ?></th>
			</tr>
		</thead>
		<tbody>
			<?php
				...
				foreach ( $found_products as $product_id => $sales ) {
					...
					if ( $product_title ) {
						...
						$product_name = __( 'Product no longer exists', 'eshopbox' );
						$orders_link = admin_url( 'edit.php?s&post_status=all&post_type=shop_order&action=-1&s=&shop_order_status=' . implode( ",", apply_filters( 'eshopbox_reports_order_statuses', array( 'completed', 'processing', 'on-hold' ) ) ) );
					}

					$orders_link = apply_filters( 'eshopbox_reports_order_link', $orders_link, $product_id, $product_title );

					echo '<tr><th>' . $product_name . '</th><td width="1%"><span>' . eshopbox_price( $sales ) . '</span></td><td class="bars"><a href="' . esc_url( $orders_link ) . '" style="width:' . esc_attr( $width ) . '%">&nbsp;</a></td></tr>';
				}
			?>
		</tbody>
	</table>
...
function eshopbox_product_sales() {
	...
	if ( $chosen_product_ids && is_array( $chosen_product_ids ) ) {
		...
		// Get order items
		$order_items = apply_filters( 'eshopbox_reports_product_sales_order_items', $wpdb->get_results( "
			...
			LEFT JOIN {$wpdb->prefix}eshopbox_order_itemmeta as order_item_meta ON order_items.order_item_id = order_item_meta.order_item_id
			LEFT JOIN {$wpdb->prefix}eshopbox_order_itemmeta as order_item_meta_2 ON order_items.order_item_id = order_item_meta_2.order_item_id
			LEFT JOIN {$wpdb->prefix}eshopbox_order_itemmeta as order_item_meta_3 ON order_items.order_item_id = order_item_meta_3.order_item_id
			LEFT JOIN {$wpdb->posts} AS posts ON order_items.order_id = posts.ID
			LEFT JOIN {$wpdb->term_relationships} AS rel ON posts.ID = rel.object_ID
			LEFT JOIN {$wpdb->term_taxonomy} AS tax USING( term_taxonomy_id )
			LEFT JOIN {$wpdb->terms} AS term USING( term_id )
		...
		?>
		<h4><?php printf( __( 'Sales for %s:', 'eshopbox' ), implode( ', ', $chosen_product_titles ) ); ?></h4>
		<table class="bar_chart">
			<thead>
				<tr>
					<th><?php _e( 'Month', 'eshopbox' ); ?></th>
					<th colspan="2"><?php _e( 'Sales', 'eshopbox' ); ?></th>
				</tr>
			</thead>
			<tbody>
				<?php
					if ( sizeof( $product_sales ) > 0 ) {
						foreach ( $product_sales as $date => $sales ) {
							...
							$orders_link = admin_url( 'edit.php?s&post_status=all&post_type=shop_order&action=-1&s=' . urlencode( implode( ' ', $chosen_product_titles ) ) . '&m=' . date( 'Ym', strtotime( $date . '01' ) ) . '&shop_order_status=' . implode( ",", apply_filters( 'eshopbox_reports_order_statuses', array( 'completed', 'processing', 'on-hold' ) ) ) );
							$orders_link = apply_filters( 'eshopbox_reports_order_link', $orders_link, $chosen_product_ids, $chosen_product_titles );

							echo '<tr><th><a href="' . esc_url( $orders_link ) . '">' . date_i18n( 'F', strtotime( $date . '01' ) ) . '</a></th>
							<td width="1%"><span>' . esc_html( $sales ) . '</span><span class="alt">' . eshopbox_price( $product_totals[ $date ] ) . '</span></td>
							<td class="bars">
						...
						echo '<tr><td colspan="3">' . __( 'No sales :(', 'eshopbox' ) . '</td></tr>';
					}
				?>
			</tbody>
		</table>
		...
		?>
		<form method="post" action="">
			<p><select id="product_ids" name="product_ids[]" class="ajax_chosen_select_products" multiple="multiple" data-placeholder="<?php _e( 'Search for a product&hellip;', 'eshopbox' ); ?>" style="width: 400px;"></select> <input type="submit" style="vertical-align: top;" class="button" value="<?php _e( 'Show', 'eshopbox' ); ?>" /></p>
			<script type="text/javascript">
				jQuery(function(){
					...
					jQuery("select.ajax_chosen_select_products").ajaxChosen({
					    ...
					    afterTypeDelay: 100,
					    data:		{
					    	action: 		'eshopbox_json_search_products',
...
function eshopbox_coupons_overview() {
	...
	$total_order_count = apply_filters( 'eshopbox_reports_coupons_overview_total_order_count', absint( $wpdb->get_var( "
	...
	$coupon_totals = apply_filters( 'eshopbox_reports_coupons_overview_totals', $wpdb->get_row( "
	...
	$coupons_by_count = apply_filters( 'eshopbox_reports_coupons_overview_coupons_by_count', $wpdb->get_results( "
	...
	$coupons_by_amount = apply_filters( 'eshopbox_reports_coupons_overview_coupons_by_count', $wpdb->get_results( "
	...
	<div id="poststuff" class="eshopbox-reports-wrap">
		<div class="eshopbox-reports-sidebar">
			<div class="postbox">
				<h3><span><?php _e( 'Total orders containing coupons', 'eshopbox' ); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ( $coupon_totals->order_count > 0 ) echo absint( $coupon_totals->order_count ); else _e( 'n/a', 'eshopbox' ); ?></p>
				</div>
			</div>
			<div class="postbox">
				<h3><span><?php _e( 'Percent of orders containing coupons', 'eshopbox' ); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ( $coupon_totals->order_count > 0 ) echo round( $coupon_totals->order_count / $total_order_count * 100, 2 ) . '%'; else _e( 'n/a', 'eshopbox' ); ?></p>
				</div>
			</div>
			<div class="postbox">
				<h3><span><?php _e( 'Total coupon discount', 'eshopbox' ); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ( $coupon_totals->total_discount > 0 ) echo eshopbox_price( $coupon_totals->total_discount ); else _e( 'n/a', 'eshopbox' ); ?></p>
				</div>
			</div>
		</div>
		<div class="eshopbox-reports-main">
			<div class="eshopbox-reports-left">
				<div class="postbox">
					<h3><span><?php _e( 'Most popular coupons', 'eshopbox' ); ?></span></h3>
					<div class="inside">
							...
							<?php
								if ( $coupons_by_count ) {
									foreach ( $coupons_by_count as $coupon ) {
										...
										echo '<li><a href="' . $link . '" class="code"><span><span>' . esc_html( $coupon->order_item_name ). '</span></span></a> - ' . sprintf( _n( 'Used 1 time', 'Used %d times', $coupon->count, 'eshopbox' ), absint( $coupon->count ) ) . '</li>';
									}
								} else {
									echo '<li>' . __( 'No coupons found', 'eshopbox' ) . '</li>';
								}
							?>
						</ul>
					</div>
			...
			<div class="eshopbox-reports-right">
				<div class="postbox">
					<h3><span><?php _e( 'Greatest discount amount', 'eshopbox' ); ?></span></h3>
					<div class="inside">
						<ul class="wc_coupon_list wc_coupon_list_block">
							<?php
								if ( $coupons_by_amount ) {
									foreach ( $coupons_by_amount as $coupon ) {
										...
										echo '<li><a href="' . $link . '" class="code"><span><span>' . esc_html( $coupon->order_item_name ). '</span></span></a> - ' . sprintf( __( 'Discounted %s', 'eshopbox' ), eshopbox_price( $coupon->amount ) ) . '</li>';
									}
								} else {
									echo '<li>' . __( 'No coupons found', 'eshopbox' ) . '</li>';
								}
...
/**
 * eshopbox_coupon_discounts function.
 *
 * @access public
 * @return void
 */
function eshopbox_coupon_discounts() {
	...
	$order_statuses = implode( "','", apply_filters( 'eshopbox_reports_order_statuses', array( 'completed', 'processing', 'on-hold' ) ) );

	$used_coupons = apply_filters( 'eshopbox_reports_coupons_sales_used_coupons', $wpdb->get_col( "
	...
	<form method="post" action="" class="report_filters">
		<p>
			<label for="show_year"><?php _e( 'Show:', 'eshopbox' ); ?></label>
			<select name="show_year" id="show_year">
				<?php
					for ( $i = $first_year; $i <= date( 'Y' ); $i++ )
						printf( '<option value="%s" %s>%s</option>', $i, selected( $current_year, $i, false ), $i );
			...
			<input type="submit" class="button" value="<?php _e( 'Show', 'eshopbox' ); ?>" />
		</p>
	</form>
	...
	if ( ! empty( $_POST['show_coupons'] ) && count( $_POST['show_coupons'] ) > 0 ) {
		...
		foreach( $coupons as $coupon ) {
			...
			$coupon_amounts = apply_filters( 'eshopbox_reports_coupon_sales_order_totals', $wpdb->get_results( $wpdb->prepare( "
		...
		?>
		<div class="eshopbox-wide-reports-wrap">
			<table class="widefat">
				<thead>
					<tr>
						<th><?php _e( 'Coupon', 'eshopbox' ); ?></th>
						<?php
							$column_count = 0;
							for ( $count = 0; $count < 12; $count++ ) :
						...
						<?php endfor; ?>
						<th><strong><?php _e( 'Total', 'eshopbox' ); ?></strong></th>
					</tr>
				</thead>
				...
				<tbody><?php
					...
					foreach( $coupon_sales as $coupon_code => $sales ) {
						...
						for ( $count = 0; $count < 12; $count ++ ) {
							...
							echo '<td>' . eshopbox_price( $amount ) . '</td>';
						...
						echo '<td><strong>' . eshopbox_price( array_sum( $sales ) ) . '</strong></td>';
					...
					echo '<tr><th><strong>' . __( 'Total', 'eshopbox' ) . '</strong></th>';

					foreach( $monthly_totals as $month => $totals )
						echo '<td><strong>' . eshopbox_price( $totals ) . '</strong></td>';

					echo '<td><strong>' .  eshopbox_price( array_sum( $monthly_totals ) ) . '</strong></td></tr>';
		...
		<div id="poststuff" class="eshopbox-reports-wrap">
			<div class="eshopbox-reports-sidebar">
				<div class="postbox">
					<h3><span><?php _e( 'Top coupon', 'eshopbox' ); ?></span></h3>
					<div class="inside">
						<p class="stat"><?php
							echo $top_coupon_name . ' (' . eshopbox_price( $top_coupon_sales ) . ')';
						?></p>
					</div>
				</div>
				<div class="postbox">
					<h3><span><?php _e( 'Worst coupon', 'eshopbox' ); ?></span></h3>
					<div class="inside">
						<p class="stat"><?php
							echo $worst_coupon_name . ' (' . eshopbox_price( $worst_coupon_sales ) . ')';
						?></p>
					</div>
				</div>
				<div class="postbox">
					<h3><span><?php _e( 'Discount average', 'eshopbox' ); ?></span></h3>
					<div class="inside">
						<p class="stat"><?php
								echo eshopbox_price( array_sum( $coupon_totals ) / count( $coupon_totals ) );
						?></p>
					</div>
				</div>
				<div class="postbox">
					<h3><span><?php _e( 'Discount median', 'eshopbox' ); ?></span></h3>
					<div class="inside">
						<p class="stat"><?php
							if ( count( $median_coupon_sales ) == 2 )
								echo __( 'N/A', 'eshopbox' );
							elseif ( count( $median_coupon_sales ) % 2 )
								...
								echo eshopbox_price( $median_coupon_sales[ count( $median_coupon_sales ) / 2 ] );
						?></p>
					</div>
				</div>
			</div>
			<div class="eshopbox-reports-main">
				<div class="postbox">
					<h3><span><?php _e( 'Monthly discounts by coupon', 'eshopbox' ); ?></span></h3>
					<div class="inside chart">
						<div id="placeholder" style="width:100%; overflow:hidden; height:568px; position:relative;"></div>
						<div id="cart_legend"></div>
					</div>
...
function eshopbox_customer_overview() {
	...
	$customer_orders = apply_filters( 'eshopbox_reports_customer_overview_customer_orders', $wpdb->get_row( "
		SELECT SUM(meta.meta_value) AS total_sales, COUNT(posts.ID) AS total_orders FROM {$wpdb->posts} AS posts
	...
	$guest_orders = apply_filters( 'eshopbox_reports_customer_overview_guest_orders', $wpdb->get_row( "
		SELECT SUM(meta.meta_value) AS total_sales, COUNT(posts.ID) AS total_orders FROM {$wpdb->posts} AS posts
	...
	<div id="poststuff" class="eshopbox-reports-wrap">
		<div class="eshopbox-reports-sidebar">
			<div class="postbox">
				<h3><span><?php _e( 'Total customers', 'eshopbox' ); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ($total_customers>0) echo $total_customers; else _e( 'n/a', 'eshopbox' ); ?></p>
				</div>
			</div>
			<div class="postbox">
				<h3><span><?php _e( 'Total customer sales', 'eshopbox' ); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ($total_customer_sales>0) echo eshopbox_price($total_customer_sales); else _e( 'n/a', 'eshopbox' ); ?></p>
				</div>
			</div>
			<div class="postbox">
				<h3><span><?php _e( 'Total guest sales', 'eshopbox' ); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ($total_guest_sales>0) echo eshopbox_price($total_guest_sales); else _e( 'n/a', 'eshopbox' ); ?></p>
				</div>
			</div>
			<div class="postbox">
				<h3><span><?php _e( 'Total customer orders', 'eshopbox' ); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ($total_customer_orders>0) echo $total_customer_orders; else _e( 'n/a', 'eshopbox' ); ?></p>
				</div>
			</div>
			<div class="postbox">
				<h3><span><?php _e( 'Total guest orders', 'eshopbox' ); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ($total_guest_orders>0) echo $total_guest_orders; else _e( 'n/a', 'eshopbox' ); ?></p>
				</div>
			</div>
			<div class="postbox">
				<h3><span><?php _e( 'Average orders per customer', 'eshopbox' ); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php if ($total_customer_orders>0 && $total_customers>0) echo number_format($total_customer_orders/$total_customers, 2); else _e( 'n/a', 'eshopbox' ); ?></p>
				</div>
			</div>
		</div>
		<div class="eshopbox-reports-main">
			<div class="postbox">
				<h3><span><?php _e( 'Signups per day', 'eshopbox' ); ?></span></h3>
				<div class="inside chart">
					<div id="placeholder" style="width:100%; overflow:hidden; height:568px; position:relative;"></div>
					<div id="cart_legend"></div>
				</div>
...
function eshopbox_stock_overview() {
	...
	// Low/No stock lists
	$lowstockamount = get_option('eshopbox_notify_low_stock_amount');
	if (!is_numeric($lowstockamount)) $lowstockamount = 1;

	$nostockamount = get_option('eshopbox_notify_no_stock_amount');
	...
	// Merge results
	$low_in_stock = apply_filters( 'eshopbox_reports_stock_overview_products', $low_stock_products + $low_stock_variations + $low_stock_variable_products + $out_of_stock_status_products );
	?>
	<div id="poststuff" class="eshopbox-reports-wrap halved">
		<div class="eshopbox-reports-left">
			<div class="postbox">
				<h3><span><?php _e( 'Low stock', 'eshopbox' ); ?></span></h3>
				<div class="inside">
					<?php
					if ( $low_in_stock ) {
						...
						foreach ( $low_in_stock as $product_id => $parent ) {
							...
							if ( $sku )
								$title .= ' (' . __( 'SKU', 'eshopbox' ) . ': ' . esc_html( $sku ) . ')';
							...
							printf( '<li><a href="%s"><small>' .  _n('%d in stock', '%d in stock', $stock, 'eshopbox') . '</small> %s</a></li>', $product_url, $stock, $title );

						}
						echo '</ul>';
					} else {
						echo '<p>'.__( 'No products are low in stock.', 'eshopbox' ).'</p>';
					}
					?>
				</div>
			</div>
		...
		<div class="eshopbox-reports-right">
			<div class="postbox">
				<h3><span><?php _e( 'Out of stock', 'eshopbox' ); ?></span></h3>
				<div class="inside">
					<?php
					if ( $low_in_stock ) {
						echo '<ul class="stock_list">';
						foreach ( $low_in_stock as $product_id => $parent ) {
							...
							if ( $stock == '' )
								printf( '<li><a href="%s"><small>' .  __('Marked out of stock', 'eshopbox') . '</small> %s</a></li>', $product_url, $title );
							else
								printf( '<li><a href="%s"><small>' .  _n('%d in stock', '%d in stock', $stock, 'eshopbox') . '</small> %s</a></li>', $product_url, $stock, $title );
						...
						echo '<p>'.__( 'No products are out in stock.', 'eshopbox' ).'</p>';
					}
					?>
				</div>
			</div>
...
function eshopbox_monthly_taxes() {
	...
	<div id="poststuff" class="eshopbox-reports-wrap">
		<div class="eshopbox-reports-sidebar">
			<div class="postbox">
				<h3><span><?php _e( 'Total taxes for year', 'eshopbox' ); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php
						if ( $total_tax > 0 )
							echo eshopbox_price( $total_tax );
						else
							_e( 'n/a', 'eshopbox' );
					?></p>
				</div>
			...
			<div class="postbox">
				<h3><span><?php _e( 'Total product taxes for year', 'eshopbox' ); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php
						if ( $total_sales_tax > 0 )
							echo eshopbox_price( $total_sales_tax );
						else
							_e( 'n/a', 'eshopbox' );
					?></p>
				</div>
			...
			<div class="postbox">
				<h3><span><?php _e( 'Total shipping tax for year', 'eshopbox' ); ?></span></h3>
				<div class="inside">
					<p class="stat"><?php
						if ( $total_shipping_tax > 0 )
							echo eshopbox_price( $total_shipping_tax );
						else
							_e( 'n/a', 'eshopbox' );
					?></p>
				</div>
		...
		<div class="eshopbox-reports-main">
			<table class="widefat">
				<thead>
					<tr>
						<th><?php _e( 'Month', 'eshopbox' ); ?></th>
						<th class="total_row"><?php _e( 'Total Sales', 'eshopbox' ); ?> <a class="tips" data-tip="<?php _e("This is the sum of the 'Order Total' field within your orders.", 'eshopbox'); ?>" href="#">[?]</a></th>
						<th class="total_row"><?php _e( 'Total Shipping', 'eshopbox' ); ?> <a class="tips" data-tip="<?php _e("This is the sum of the 'Shipping Total' field within your orders.", 'eshopbox'); ?>" href="#">[?]</a></th>
						<th class="total_row"><?php _e( 'Total Product Taxes', 'eshopbox' ); ?> <a class="tips" data-tip="<?php _e("This is the sum of the 'Cart Tax' field within your orders.", 'eshopbox'); ?>" href="#">[?]</a></th>
						<th class="total_row"><?php _e( 'Total Shipping Taxes', 'eshopbox' ); ?> <a class="tips" data-tip="<?php _e("This is the sum of the 'Shipping Tax' field within your orders.", 'eshopbox'); ?>" href="#">[?]</a></th>
						<th class="total_row"><?php _e( 'Total Taxes', 'eshopbox' ); ?> <a class="tips" data-tip="<?php _e("This is the sum of the 'Cart Tax' and 'Shipping Tax' fields within your orders.", 'eshopbox'); ?>" href="#">[?]</a></th>
						<th class="total_row"><?php _e( 'Net profit', 'eshopbox' ); ?> <a class="tips" data-tip="<?php _e("Total sales minus shipping and tax.", 'eshopbox'); ?>" href="#">[?]</a></th>
						<?php
				...
				<tfoot>
					<tr>
						<?php
							...
							echo '
								<td>' . __( 'Total', 'eshopbox' ) . '</td>
								<td class="total_row">' . eshopbox_price( $total['gross'] ) . '</td>
								<td class="total_row">' . eshopbox_price( $total['shipping'] ) . '</td>
								<td class="total_row">' . eshopbox_price( $total['order_tax'] ) . '</td>
								<td class="total_row">' . eshopbox_price( $total['shipping_tax'] ) . '</td>
								<td class="total_row">' . eshopbox_price( $total['total_tax'] ) . '</td>
								<td class="total_row">' . eshopbox_price( $total['gross'] - $total['shipping'] - $total['total_tax'] ) . '</td>';

							foreach ( $tax_row_labels as $label )
								if ( isset( $total['tax_rows'][ $label ] ) )
									echo '<td class="tax_row">' . eshopbox_price( $total['tax_rows'][ $label ] ) . '</td>';
								else
									echo '<td class="tax_row">' .  eshopbox_price( 0 ) . '</td>';
						?>
					</tr>
					<tr>
						<th colspan="<?php echo 7 + sizeof( $tax_row_labels ); ?>"><button class="button toggle_tax_rows"><?php _e( 'Toggle tax rows', 'eshopbox' ); ?></button></th>
					</tr>
				</tfoot>
				<tbody>
					<?php
						foreach ( $taxes as $month => $tax ) {
							...
							echo '<tr class="' . $alt . '">
								<td>' . $month . '</td>
								<td class="total_row">' . eshopbox_price( $tax['gross'] ) . '</td>
								<td class="total_row">' . eshopbox_price( $tax['shipping'] ) . '</td>
								<td class="total_row">' . eshopbox_price( $tax['order_tax'] ) . '</td>
								<td class="total_row">' . eshopbox_price( $tax['shipping_tax'] ) . '</td>
								<td class="total_row">' . eshopbox_price( $tax['total_tax'] ) . '</td>
								<td class="total_row">' . eshopbox_price( $tax['gross'] - $tax['shipping'] - $tax['total_tax'] ) . '</td>';
							...
							foreach ( $tax_row_labels as $label ) {
								...
								echo '<td class="tax_row">' . eshopbox_price( $row_total ) . '</td>';
							}
...
/**
 * eshopbox_category_sales function.
 *
 * @access public
 * @return void
 */
function eshopbox_category_sales() {
		...
		<p>
			...
			<select multiple="multiple" class="chosen_select" id="show_categories" name="show_categories[]" style="width: 300px;">
				<?php
					...
					echo eshopbox_walk_category_dropdown_tree( $categories, 0, $r );
				?>
			</select>
	...
	// Get order items
	$order_items = apply_filters( 'eshopbox_reports_category_sales_order_items', $wpdb->get_results( $wpdb->prepare( "
	...
	if ( ! empty( $_POST['show_categories'] ) && sizeof( $_POST['show_categories'] ) > 0 ) {
		...
		<div class="eshopbox-wide-reports-wrap">
			<table class="widefat">
				<thead>
					<tr>
						<th><?php _e( 'Category', 'eshopbox' ); ?></th>
						<?php
							$column_count = 0;
							for ( $count = 0; $count < 12; $count++ ) :
								if ( $count >= date ( 'm' ) && $current_year == date( 'Y' ) )
				...
				<tbody><?php
					...
					foreach ( $categories as $category ) {
						...
						for ( $count = 0; $count < 12; $count++ ) {
							...
							$category_sales_html .= '<td>' . eshopbox_price( $total ) . '</td>';
						...
						$category_sales_html .= '<td><strong>' . eshopbox_price( $cat_total ) . '</strong></td>';
					...
					for ( $count = 0; $count < 12; $count++ )
						if ( $count >= date( 'm' ) && $current_year == date( 'Y' ) )
							...
							echo '<td><strong>' . eshopbox_price( $month_totals[ $count ] ) . '</strong></td>';
					echo '<td><strong>' .  eshopbox_price( array_sum( $month_totals ) ) . '</strong></td></tr>';
		...
		<div id="poststuff" class="eshopbox-reports-wrap">
			<div class="eshopbox-reports-sidebar">
				<div class="postbox">
					<h3><span><?php _e( 'Top category', 'eshopbox' ); ?></span></h3>
					<div class="inside">
						<p class="stat"><?php
							echo $top_cat_name . ' (' . eshopbox_price( $top_cat ) . ')';
						?></p>
					</div>
				</div>
				<?php if ( sizeof( $category_totals ) > 1 ) : ?>
				<div class="postbox">
					<h3><span><?php _e( 'Worst category', 'eshopbox' ); ?></span></h3>
					<div class="inside">
						<p class="stat"><?php
							echo $bottom_cat_name . ' (' . eshopbox_price( $bottom_cat ) . ')';
						?></p>
					</div>
				</div>
				<div class="postbox">
					<h3><span><?php _e( 'Category sales average', 'eshopbox' ); ?></span></h3>
					<div class="inside">
						<p class="stat"><?php
							if ( sizeof( $category_totals ) > 0 )
								echo eshopbox_price( array_sum( $category_totals ) / sizeof( $category_totals ) );
							else
								echo __( 'N/A', 'eshopbox' );
						?></p>
					</div>
				...
				<div class="postbox">
					<h3><span><?php _e( 'Category sales median', 'eshopbox' ); ?></span></h3>
					<div class="inside">
						<p class="stat"><?php
							if ( sizeof( $category_totals ) == 0 )
								echo __( 'N/A', 'eshopbox' );
							elseif ( sizeof( $category_totals ) % 2 )
								...
								echo eshopbox_price( $category_totals[ sizeof( $category_totals ) / 2 ] );
						?></p>
					</div>
				</div>
				<?php endif; ?>
			...
			<div class="eshopbox-reports-main">
				<div class="postbox">
					<h3><span><?php _e( 'Monthly sales by category', 'eshopbox' ); ?></span></h3>
					<div class="inside chart">
						<div id="placeholder" style="width:100%; overflow:hidden; height:568px; position:relative;"></div>
						<div id="cart_legend"></div>
					</div>
