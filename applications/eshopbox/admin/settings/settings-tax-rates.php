 ...
 * @return void
 */
function eshopbox_tax_rates_setting() {
	global $eshopbox, $current_section, $wpdb;
	...
	foreach( $tax_classes as $class )
		if ( sanitize_title( $class ) == $current_section )
			$current_class = $class;
	?>
	<h3><?php printf( __( 'Tax Rates for the "%s" Class', 'eshopbox' ), $current_class ? esc_html( $current_class ) : __( 'Standard', 'eshopbox' ) ); ?></h3>
	<p><?php printf( __( 'Define tax rates for countries and states below. <a href="%s">See here</a> for available alpha-2 country codes.', 'eshopbox' ), 'http://en.wikipedia.org/wiki/ISO_3166-1#Current_codes' ); ?></p>
	<table class="wc_tax_rates widefat">
		<thead>
			<tr>
				<th class="sort">&nbsp;</th>

				<th width="8%"><?php _e( 'Country&nbsp;Code', 'eshopbox' ); ?>&nbsp;<span class="tips" data-tip="<?php _e('A 2 digit country code, e.g. US. Leave blank to apply to all.', 'eshopbox'); ?>">[?]</span></th>

				<th width="8%"><?php _e( 'State&nbsp;Code', 'eshopbox' ); ?>&nbsp;<span class="tips" data-tip="<?php _e('A 2 digit state code, e.g. AL. Leave blank to apply to all.', 'eshopbox'); ?>">[?]</span></th>

				<th><?php _e( 'ZIP/Postcode', 'eshopbox' ); ?>&nbsp;<span class="tips" data-tip="<?php _e('Postcode for this rule. Semi-colon (;) separate multiple values. Leave blank to apply to all areas. Wildcards (*) can be used. Ranges for numeric postcodes (e.g. 12345-12350) will be expanded into individual postcodes.', 'eshopbox'); ?>">[?]</span></th>

				<th><?php _e( 'City', 'eshopbox' ); ?>&nbsp;<span class="tips" data-tip="<?php _e('Cities for this rule. Semi-colon (;) separate multiple values. Leave blank to apply to all cities.', 'eshopbox'); ?>">[?]</span></th>

				<th width="8%"><?php _e( 'Rate&nbsp;%', 'eshopbox' ); ?>&nbsp;<span class="tips" data-tip="<?php _e( 'Enter a tax rate (percentage) to 4 decimal places.', 'eshopbox' ); ?>">[?]</span></th>

				<th width="8%"><?php _e( 'Tax&nbsp;Name', 'eshopbox' ); ?>&nbsp;<span class="tips" data-tip="<?php _e('Enter a name for this tax rate.', 'eshopbox'); ?>">[?]</span></th>

				<th width="8%"><?php _e( 'Priority', 'eshopbox' ); ?>&nbsp;<span class="tips" data-tip="<?php _e('Choose a priority for this tax rate. Only 1 matching rate per priority will be used. To define multiple tax rates for a single area you need to specify a different priority per rate.', 'eshopbox'); ?>">[?]</span></th>

				<th width="8%"><?php _e( 'Compound', 'eshopbox' ); ?>&nbsp;<span class="tips" data-tip="<?php _e('Choose whether or not this is a compound rate. Compound tax rates are applied on top of other tax rates.', 'eshopbox'); ?>">[?]</span></th>

				<th width="8%"><?php _e( 'Shipping', 'eshopbox' ); ?>&nbsp;<span class="tips" data-tip="<?php _e('Choose whether or not this tax rate also gets applied to shipping.', 'eshopbox'); ?>">[?]</span></th>
		...
		<tfoot>
			<tr>
				<th colspan="10">
					<a href="#" class="button plus insert"><?php _e( 'Insert row', 'eshopbox' ); ?></a>
					<a href="#" class="button minus remove"><?php _e( 'Remove selected row(s)', 'eshopbox' ); ?></a>

					<a href="#" download="tax_rates.csv" class="button export"><?php _e( 'Export CSV', 'eshopbox' ); ?></a>
					<a href="<?php echo admin_url( 'admin.php?import=eshopbox_tax_rate_csv' ); ?>" class="button import"><?php _e( 'Import CSV', 'eshopbox' ); ?></a>
				</th>
			</tr>
		</tfoot>
		<tbody id="rates">
			<?php
				$rates = $wpdb->get_results( $wpdb->prepare(
					"SELECT * FROM {$wpdb->prefix}eshopbox_tax_rates
					...
					<tr>
						...
						<td class="postcode">
							<input type="text" value="<?php
								$locations = $wpdb->get_col( $wpdb->prepare( "SELECT location_code FROM {$wpdb->prefix}eshopbox_tax_rate_locations WHERE location_type='postcode' AND tax_rate_id = %d ORDER BY location_code", $rate->tax_rate_id ) );
						...
						<td class="city">
							<input type="text" value="<?php
								$locations = $wpdb->get_col( $wpdb->prepare( "SELECT location_code FROM {$wpdb->prefix}eshopbox_tax_rate_locations WHERE location_type='city' AND tax_rate_id = %d ORDER BY location_code", $rate->tax_rate_id ) );
								echo esc_attr( implode( '; ', $locations ) );
		...
		jQuery( function() {
			...
			jQuery('.wc_tax_rates .remove').click(function() {
				...
				if ( $tbody.find('tr.current').size() > 0 ) {
					...
					alert('<?php echo esc_js( __( 'No row(s) selected', 'eshopbox' ) ); ?>');
				}
				return false;
			});
			...
			jQuery('.wc_tax_rates .export').click(function() {
				...
				var csv_data = "data:application/csv;charset=utf-8,<?php _e( 'Country Code', 'eshopbox' ); ?>,<?php _e( 'State Code', 'eshopbox' ); ?>,<?php _e( 'ZIP/Postcode', 'eshopbox' ); ?>,<?php _e( 'City', 'eshopbox' ); ?>,<?php _e( 'Rate %', 'eshopbox' ); ?>,<?php _e( 'Tax Name', 'eshopbox' ); ?>,<?php _e( 'Priority', 'eshopbox' ); ?>,<?php _e( 'Compound', 'eshopbox' ); ?>,<?php _e( 'Shipping', 'eshopbox' ); ?>,<?php _e( 'Tax Class', 'eshopbox' ); ?>\n";
			...
			var availableCountries = [<?php
				$countries = array();
				foreach ( $eshopbox->countries->get_allowed_countries() as $value => $label )
			...
			var availableStates = [<?php
				$countries = array();
				foreach ( $eshopbox->countries->get_allowed_country_states() as $value => $label )
...
/**
 * eshopbox_tax_rates_setting_save function.
 *
 * @access public
 * @return void
 */
function eshopbox_tax_rates_setting_save() {
	...
	foreach ( $tax_rate_country as $key => $value ) {
		...
		if ( $key == 'new' ) {
			...
			foreach ( $value as $new_key => $new_value ) {
				...
				// Sanitize + format
				$country  = strtoupper( eshopbox_clean( $tax_rate_country[ $key ][ $new_key ] ) );
				$state    = strtoupper( eshopbox_clean( $tax_rate_state[ $key ][ $new_key ] ) );
				$postcode = eshopbox_clean( $tax_rate_postcode[ $key ][ $new_key ] );
				$city     = eshopbox_clean( $tax_rate_city[ $key ][ $new_key ] );
				$rate     = number_format( eshopbox_clean( $tax_rate[ $key ][ $new_key ] ), 4, '.', '' );
				$name     = eshopbox_clean( $tax_rate_name[ $key ][ $new_key ] );
				$priority = absint( eshopbox_clean( $tax_rate_priority[ $key ][ $new_key ] ) );
				$compound = isset( $tax_rate_compound[ $key ][ $new_key ] ) ? 1 : 0;
				$shipping = isset( $tax_rate_shipping[ $key ][ $new_key ] ) ? 1 : 0;

				if ( ! $name )
					$name = __( 'Tax', 'eshopbox' );
				...
				$wpdb->insert(
					$wpdb->prefix . "eshopbox_tax_rates",
					array(
				...
				if ( ! empty( $postcode ) ) {
					...
					$postcodes = array_map( 'strtoupper', array_map( 'eshopbox_clean', $postcodes ) );
					...
					$wpdb->query( "INSERT INTO {$wpdb->prefix}eshopbox_tax_rate_locations ( location_code, tax_rate_id, location_type ) VALUES " . implode( ',', $postcode_query ) );
				}

				if ( ! empty( $city ) ) {
					$cities = explode( ';', $city );
					$cities = array_map( 'strtoupper', array_map( 'eshopbox_clean', $cities ) );
					foreach( $cities as $city ) {
						$wpdb->insert(
						$wpdb->prefix . "eshopbox_tax_rate_locations",
							array(
								'location_code' => $city,
		...
		// ...whereas the others are updated
			...
			if ( $_POST['remove_tax_rate'][ $key ] == 1 ) {
				$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}eshopbox_tax_rate_locations WHERE tax_rate_id = %d;", $tax_rate_id ) );
				$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}eshopbox_tax_rates WHERE tax_rate_id = %d;", $tax_rate_id ) );
				continue;
			}

			// Sanitize + format
			$country  = strtoupper( eshopbox_clean( $tax_rate_country[ $key ] ) );
			$state    = strtoupper( eshopbox_clean( $tax_rate_state[ $key ] ) );
			$rate     = number_format( eshopbox_clean( $tax_rate[ $key ] ), 4, '.', '' );
			$name     = eshopbox_clean( $tax_rate_name[ $key ] );
			$priority = absint( eshopbox_clean( $tax_rate_priority[ $key ] ) );
			...
			if ( isset( $tax_rate_postcode[ $key ] ) ) {
				// Delete old
				$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}eshopbox_tax_rate_locations WHERE tax_rate_id = %d AND location_type = 'postcode';", $tax_rate_id ) );

				// Add changed
				$postcode  = eshopbox_clean( $tax_rate_postcode[ $key ] );
				$postcodes = explode( ';', $postcode );
				$postcodes = array_map( 'strtoupper', array_map( 'eshopbox_clean', $postcodes ) );
			...
			if ( isset( $tax_rate_city[ $key ] ) ) {
				// Delete old
				$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}eshopbox_tax_rate_locations WHERE tax_rate_id = %d AND location_type = 'city';", $tax_rate_id ) );

				// Add changed
				$city   = eshopbox_clean( $tax_rate_city[ $key ] );
				$cities = explode( ';', $city );
				$cities = array_map( 'strtoupper', array_map( 'eshopbox_clean', $cities ) );
				foreach( $cities as $city ) {
					if ( $city ) {
