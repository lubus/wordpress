<?php
/**
 * Functions used for displaying the EshopBox dashboard widgets
 *
 * @author 		WooThemes
 * @category 	Admin
 * @package 	EshopBox/Admin/Dashboard
 * @version     1.6.4
 */
...
if ( current_user_can( 'view_eshopbox_reports' ) || current_user_can( 'manage_eshopbox' ) || current_user_can( 'publish_shop_orders' ) )
	add_action( 'wp_dashboard_setup', 'eshopbox_init_dashboard_widgets' );
 ...
 * @return void
 */
function eshopbox_init_dashboard_widgets() {
	...
	$sales_heading .= '<a href="index.php?wc_sales_month='.($current_month_offset-1).'" class="previous">&larr; '.date_i18n('F', strtotime('01-'.($the_month_num-1).'-2011')).'</a><span>'.__( 'Monthly Sales', 'eshopbox' ).'</span>';

	if ( current_user_can( 'publish_shop_orders' ) ) {
		wp_add_dashboard_widget( 'eshopbox_dashboard_right_now', __( 'EshopBox Right Now', 'eshopbox' ), 'eshopbox_dashboard_widget_right_now' );
		wp_add_dashboard_widget( 'eshopbox_dashboard_recent_orders', __( 'EshopBox Recent Orders', 'eshopbox' ), 'eshopbox_dashboard_recent_orders');
		wp_add_dashboard_widget( 'eshopbox_dashboard_recent_reviews', __( 'EshopBox Recent Reviews', 'eshopbox' ), 'eshopbox_dashboard_recent_reviews' );
	}

	if ( current_user_can( 'view_eshopbox_reports' ) || current_user_can( 'publish_shop_orders' ) ) {
		wp_add_dashboard_widget( 'eshopbox_dashboard_sales', $sales_heading, 'eshopbox_dashboard_sales' );
	}
 ...
 * @return void
 */
function eshopbox_dashboard_widget_right_now() {
	global $eshopbox;

	$product_count      = wp_count_posts( 'product' );
	$product_cat_count  = wp_count_terms( 'product_cat' );
	$product_tag_count  = wp_count_terms( 'product_tag' );
	$product_attr_count = count( $eshopbox->get_attribute_taxonomies() );
	...
	<div class="table table_shop_content">
		<p class="sub eshopbox_sub"><?php _e( 'Shop Content', 'eshopbox' ); ?></p>
		<table>
			<tr class="first">

				<?php
					$num  = number_format_i18n( $product_count->publish );
					$text = _n( 'Product', 'Products', intval( $product_count->publish ), 'eshopbox' );
			...
			<tr>
				...
				<?php
					...
					$text = _n( 'Product Category', 'Product Categories', $product_cat_count, 'eshopbox' );
			...
			<tr>
				...
				<?php
					...
					$text = _n( 'Product Tag', 'Product Tags', $product_tag_count, 'eshopbox' );
			...
			<tr>
				...
				<?php
					...
					$text = _n( 'Attribute', 'Attributes', $product_attr_count, 'eshopbox' );
					$link = add_query_arg( array( 'page' => 'eshopbox_attributes' ), get_admin_url( null, 'admin.php' ) );
					$num  = '<a href="' . esc_url($link ) . '">' . esc_html( $num )  . '</a>';
					$text = '<a href="' . esc_url( $link ) . '">' . esc_html( $text ) . '</a>';
				?>
			...
			<?php do_action( 'eshopbox_right_now_shop_content_table_end' ); ?>

		</table>
	</div>

	<div class="table table_orders">
		<p class="sub eshopbox_sub"><?php _e( 'Orders', 'eshopbox' ); ?></p>
		<table>
			<tr class="first">

				<?php
					$num  = number_format_i18n( $pending_count );
					$text = __( 'Pending', 'eshopbox' );
			...
			<?php do_action( 'eshopbox_right_now_orders_table_end' ); ?>
	...
	<div class="versions">
		<p id="wc-version-message">
			<?php printf( __( 'You are using <strong>EshopBox %s</strong>.', 'eshopbox' ), $eshopbox->version ); ?>
		</p>
	</div>
	<?php
}
 ...
 * @return void
 */
function eshopbox_dashboard_recent_orders() {
	...
	if ($orders) :
		...
		foreach ($orders as $order) :
			...
			echo '
			<li>
				<span class="order-status '.sanitize_title($this_order->status).'">'.ucwords(__($this_order->status, 'eshopbox')).'</span> <a href="'.admin_url('post.php?post='.$order->ID).'&action=edit">' . get_the_time( __( 'l jS \of F Y h:i:s A', 'eshopbox' ), $order->ID ) . '</a><br />
				<small>'.sizeof($this_order->get_items()).' '._n('item', 'items', sizeof($this_order->get_items()), 'eshopbox').' <span class="order-cost">'.__('Total:', 'eshopbox' ) . ' ' . eshopbox_price($this_order->order_total).'</span></small>
			</li>';

		endforeach;
		echo '</ul>';
	else:
		echo '<p>' . __( 'There are no product orders yet.', 'eshopbox' ) . '</p>';
	endif;
}
 ...
 * @return void
 */
function eshopbox_dashboard_recent_reviews() {
	global $wpdb;
	...
	if ( $comments ) {
		...
		foreach ( $comments as $comment ) {
			...
			echo '<div class="star-rating" title="' . $rating . '">
				<span style="width:'. ( $rating * 20 ) . '%">' . $rating . ' ' . __( 'out of 5', 'eshopbox' ) . '</span></div>';
		...
		echo '<p>' . __( 'There are no product reviews yet.', 'eshopbox' ) . '</p>';
	}
}
 ...
 * @return void
 */
function eshopbox_dashboard_sales() {

	add_action( 'admin_footer', 'eshopbox_dashboard_sales_js' );
 ...
 * @return void
 */
function eshopbox_dashboard_sales_js() {

	global $eshopbox, $wp_locale;
	...
	$args = array(
	    ...
	    'tax_query' => array(
	    	array(
				...
				'terms' => apply_filters( 'eshopbox_reports_order_statuses', array( 'completed', 'processing', 'on-hold' ) ),
				'field' => 'slug',
				'operator' => 'IN'
			)
	    )
	...
	$params = array(
		...
		'sales_amount'    	=> eshopbox_price( array_sum( $order_amounts ) ),
		'sold' 				=> __( 'Sold', 'eshopbox' ),
		'earned'    		=> __( 'Earned', 'eshopbox' ),
	...
	wp_register_script( 'eshopbox_dashboard_sales', $eshopbox->plugin_url() . '/assets/js/admin/dashboard_sales' . $suffix . '.js', array( 'jquery', 'flot', 'flot-resize' ), '1.0' );
	wp_register_script( 'flot', $eshopbox->plugin_url() . '/assets/js/admin/jquery.flot'.$suffix.'.js', 'jquery', '1.0' );
	wp_register_script( 'flot-resize', $eshopbox->plugin_url() . '/assets/js/admin/jquery.flot.resize'.$suffix.'.js', 'jquery', '1.0' );

	wp_localize_script( 'eshopbox_dashboard_sales', 'params', $params );

	wp_print_scripts('eshopbox_dashboard_sales');
}