...
function wpfp_widget_init() {
    ...
    //*** users favorites widget ***//
     function wpfp_users_favorites_widget_function($args)
            {
         extract($args);
        $options = wpfp_get_options();
    ...
    function wpfp_users_favorites_widget_view($args) {
        ...
        $path =home_url();
        $path =$path.'/mywishlist/';
        //echo  $path;
        //echo home_url();
        $title = empty($options['uf_widget_title']) ? 'Users Favorites' : $options['uf_widget_title'];
        echo $before_widget;
        echo $before_title.'<a href="'.$path.'">'.$title.'</a>'.$after_title;
        $favorite_post_ids = wpfp_get_users_favorites();

		$limit = $options['uf_widget_limit'];
                
                add_shortcode('Users Favourite List','wpfp_users_favorites_widget_function' );
