...
function wc_table_rate_admin_shipping_rows( $table_rate_shipping ) {
	...
	<table id="shipping_rates" class="shippingrows widefat" cellspacing="0" style="position:relative;">
		<thead>
			<tr>
				...
				<th class="cost_per_weight"><?php echo __('Cost per', 'wc_table_rate') . ' ' . get_option('eshopbox_weight_unit'); ?>&nbsp;<a class="tips" data-tip="<?php _e('Cost per item, excluding tax.', 'wc_table_rate'); ?>">[?]</a></th>
	...
	jQuery('#eshopbox_table_rate_calculation_type').change(function(){
	...
	jQuery('#shipping_rates a.add').live('click', function(){
		...
		jQuery('#eshopbox_table_rate_calculation_type').change();
	...
	jQuery('#shipping_rates a.remove').live('click', function(){
		...
		if (answer) {
			...
			var data = {
				action: 'eshopbox_table_rate_delete',
				rate_id: rate_ids,
...
function wc_table_rate_admin_shipping_class_priorities( $shipping_method_id ) {
	...
	$classes = $eshopbox->shipping->get_shipping_classes();
	if (!$classes) :
		echo '<p class="description">' . __('No shipping classes exist - you can ignore this option :)', 'wc_table_rate') . '</p>';
	else :
		$priority = get_option( 'eshopbox_table_rate_default_priority_' . $shipping_method_id ) != '' ? get_option( 'eshopbox_table_rate_default_priority_' . $shipping_method_id ) : 10;
		?>
		<table class="widefat" style="position:relative;">
			...
			<tbody>
				<tr>
					...
					<td><input type="text" size="2" name="eshopbox_table_rate_default_priority" value="<?php echo $priority; ?>" /></td>
				</tr>
    			<?php
    			$eshopbox_table_rate_priorities = get_option( 'eshopbox_table_rate_priorities_' . $shipping_method_id );
        		foreach ($classes as $class) {
					$priority = (isset($eshopbox_table_rate_priorities[$class->slug])) ? $eshopbox_table_rate_priorities[$class->slug] : 10;

					echo '<tr><th>'.$class->name.'</th><td><input type="text" value="'.$priority.'" size="2" name="eshopbox_table_rate_priorities['.$class->slug.']" /></td></tr>';
...
function wc_table_rate_admin_shipping_rows_process( $shipping_method_id ) {
	...
	if ( empty( $_POST['eshopbox_table_rate_calculation_type'] ) ) {

		if ( isset( $_POST['eshopbox_table_rate_priorities'] ) ) {
			$priorities = array_map('intval', (array) $_POST['eshopbox_table_rate_priorities']);
			update_option( 'eshopbox_table_rate_priorities_' . $shipping_method_id, $priorities );
		}

		if ( isset( $_POST['eshopbox_table_rate_default_priority'] ) ) {
			update_option('eshopbox_table_rate_default_priority_' . $shipping_method_id, (int) esc_attr( $_POST['eshopbox_table_rate_default_priority'] ) );
		}

	} else {
		delete_option( 'eshopbox_table_rate_priorities_' . $shipping_method_id );
		delete_option( 'eshopbox_table_rate_default_priority_' . $shipping_method_id );
	}

	// Save rates
	$rate_ids			 		= isset( $_POST['rate_id'] ) ? array_map( 'intval', $_POST['rate_id'] ) : array();
	$shipping_class 			= isset( $_POST['shipping_class'] ) ? array_map( 'eshopbox_clean', $_POST['shipping_class'] ) : array();
	$shipping_condition 		= isset( $_POST['shipping_condition'] ) ? array_map( 'eshopbox_clean', $_POST['shipping_condition'] ) : array();
	$shipping_min 				= isset( $_POST['shipping_min'] ) ? array_map( 'eshopbox_clean', $_POST['shipping_min'] ) : array();
	$shipping_max 				= isset( $_POST['shipping_max'] ) ? array_map( 'eshopbox_clean', $_POST['shipping_max'] ) : array();
	$shipping_cost 				= isset( $_POST['shipping_cost'] ) ? array_map( 'eshopbox_clean', $_POST['shipping_cost'] ) : array();
	$shipping_per_item 			= isset( $_POST['shipping_per_item'] ) ? array_map( 'eshopbox_clean', $_POST['shipping_per_item'] ) : array();
	$shipping_cost_per_weight	= isset( $_POST['shipping_cost_per_weight'] ) ? array_map( 'eshopbox_clean', $_POST['shipping_cost_per_weight'] ) : array();
	$cost_percent				= isset( $_POST['shipping_cost_percent'] ) ? array_map( 'eshopbox_clean', $_POST['shipping_cost_percent'] ) : array();
	$shipping_label 			= isset( $_POST['shipping_label'] ) ? array_map( 'eshopbox_clean', $_POST['shipping_label'] ) : array();
	$shipping_priority 			= ( isset( $_POST['shipping_priority'] ) ) ? array_map( 'eshopbox_clean', $_POST['shipping_priority'] ) : array();
	...
	for ( $i = 0; $i <= $max_key; $i++ ) {
		...
		if ( $rate_id > 0 ) {
			...
			// Update row
			$wpdb->update(
				$wpdb->prefix . 'eshopbox_shipping_table_rates',
				array(
