 ...
 */
	define( 'TABLE_RATE_SHIPPING_VERSION', '2.4.0' );
	/**
	 * Localisation
	 */
	...
	if ( get_option( 'hide_table_rate_welcome_notice' ) == '' )
		add_action( 'admin_notices', 'eshopbox_table_rate_welcome_notice' );

	function eshopbox_table_rate_welcome_notice() {
		global $eshopbox;

		wp_enqueue_style( 'eshopbox-activation', $eshopbox->plugin_url() . '/assets/css/activation.css' );
		?>
	...
	/**
	 * eshopbox_init_shipping_table_rate function.
	 *
	 * @access public
	 * @return void
	 */
	add_action( 'eshopbox_shipping_init', 'eshopbox_init_shipping_table_rate' );

	function eshopbox_init_shipping_table_rate() {
		...
		class WC_Shipping_Table_Rate extends WC_Shipping_Method {
			...
			function __construct( $instance = false ) {
		        ...
		        $this->rates_table 		= $wpdb->prefix . 'eshopbox_shipping_table_rates';
		    ...
		    function is_available( $package ) {
		    	...
		    	return apply_filters( 'eshopbox_shipping_' . $this->id . '_is_available', $available, $package, $this );
		    }
		    ...
		    function get_cart_shipping_class_id( $package ) {
				...
				if ( sizeof( $found_shipping_classes ) == 1 ) {
					...
					$priority 	= get_option('eshopbox_table_rate_default_priority_' . $this->number );
					$priorities = get_option( 'eshopbox_table_rate_priorities_' . $this->number );
	...
	/**
	 * eshopbox_register_table_rates function.
	 *
	 * @access public
	 * @param mixed $package
	 * @return void
	 */
	add_action( 'eshopbox_load_shipping_methods', 'eshopbox_register_table_rates' );

	function eshopbox_register_table_rates( $package ) {
		global $eshopbox;

		// Register the main class
		eshopbox_register_shipping_method( 'WC_Shipping_Table_Rate' );

		if ( ! $package ) return;

		// Get zone for package
		$zone = eshopbox_get_shipping_zone( $package );

		if ( defined('WP_DEBUG') && WP_DEBUG == true )
			$eshopbox->add_message( 'Customer matched shipping zone <strong>' . $zone->zone_name . '</strong> (#' . $zone->zone_id . ')' );
	 ...
	 */
	function eshopbox_get_shipping_method_table_rate( $instance = false ) {
