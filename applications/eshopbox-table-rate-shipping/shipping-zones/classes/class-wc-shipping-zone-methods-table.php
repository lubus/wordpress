        ...
        $eshopbox->add_inline_js("
			...
			jQuery('table.shippingmethods tbody').sortable({
				...
				update: function(event, ui) {
					...
					jQuery.post( ajaxurl, { action: 'eshopbox_shipping_method_ordering', shipping_method_id: shipping_method_id, prev_shipping_method_id: prev_shipping_method_id, next_shipping_method_id: next_shipping_method_id }, function(response) {
        	...
        	case 'title' :
        		...
        		return '
        			...
        			<div class="row-actions">
        				<span class="id">ID: ' . $item->instance_id . ' | </span><span><a href="' . add_query_arg( 'method', $item->number, add_query_arg( 'zone',  $this->zone_id, admin_url( 'admin.php?page=shipping_zones' ) ) ) . '">' . __( 'Edit' , 'wc_table_rate' ) . '</a> | </span><span class="trash"><a class="submitdelete" href="' . wp_nonce_url( add_query_arg( 'delete_method', $item->number ), 'eshopbox_delete_method' ) . '">' . __( 'Delete', 'wc_table_rate' ) . '</a></span>
        			</div>';

        	case 'type' :
        		return $item->method_title;
        	case 'enabled' :
        		return ( $item->enabled == 'yes' ) ? '<img src="' . $eshopbox->plugin_url() . '/assets/images/success.png" alt="yes" width="16px" />' : '&ndash;';
        	case 'rows' :
    ...
    function process_bulk_action() {
        ...
        if ( 'delete' === $this->current_action() ) {
        	...
        	if ( $items ) foreach ( $items as $id ) {
        		...
        		$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}eshopbox_shipping_zone_shipping_methods WHERE shipping_method_id = %d", $id ) );
        		$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}eshopbox_shipping_table_rates WHERE shipping_method_id = %d", $id ) );
        		delete_option( 'eshopbox_table_rate_priorities_' . $id );
        		delete_option( 'eshopbox_table_rate_default_priority_' . $id );
        	}
