...
class WC_Shipping_Zone {
		...
		foreach ( $this->shipping_methods as $shipping_method ) {
			...
			if ( is_callable( $shipping_method['callback'] ) ) {
				...
				if ( $method->enabled == 'yes' ) {
					eshopbox_register_shipping_method( $method );
				
					if ( defined('WP_DEBUG') && WP_DEBUG == true )
						$eshopbox->add_message( 'Registering shipping method instance <strong>' . $method->title . '</strong> (#' . $method->instance_id . ')' );
				} else {
					if ( defined('WP_DEBUG') && WP_DEBUG == true )
						$eshopbox->add_message( 'Shipping method instance <strong>' . $method->title . '</strong> (#' . $method->instance_id . ') is disabled' );
				}
	...
	function add_shipping_method( $type ) {
		...
		$wpdb->insert( 
			$wpdb->prefix . 'eshopbox_shipping_zone_shipping_methods',
			array( 
