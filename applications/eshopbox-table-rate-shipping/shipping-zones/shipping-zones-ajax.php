 ...
 * Re-order zones
 */
function eshopbox_zone_ordering() {
...
add_action( 'wp_ajax_eshopbox_zone_ordering', 'eshopbox_zone_ordering' );


/**
 * Re-order shipping methods
 */
function eshopbox_shipping_method_ordering() {
	// check permissions again and make sure we have what we need
	if ( ! current_user_can('manage_eshopbox') )
		die( -1 );
	...
	$zone_id = absint( $wpdb->get_var( "SELECT zone_id FROM {$wpdb->prefix}eshopbox_shipping_zone_shipping_methods WHERE shipping_method_id = " . $shipping_method_id ) );

	$siblings = $wpdb->get_results("
		SELECT shipping_method_id, shipping_method_order FROM {$wpdb->prefix}eshopbox_shipping_zone_shipping_methods
...
add_action( 'wp_ajax_eshopbox_shipping_method_ordering', 'eshopbox_shipping_method_ordering' );