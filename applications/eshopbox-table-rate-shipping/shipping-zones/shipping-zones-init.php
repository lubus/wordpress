<?php
	 ...
	 * Admin Menu Items
	 */
	add_action( 'admin_menu', 'eshopbox_table_rate_menus', 25 );
	
	function eshopbox_table_rate_menus() {
	
		$shipping_zones_page = add_submenu_page( 'eshopbox', __('Shipping Zones', 'wc_table_rate'),  __('Shipping Zones', 'wc_table_rate') , 'manage_eshopbox', 'shipping_zones', 'eshopbox_shipping_zones_page');
		
		add_action( 'admin_print_styles-' . $shipping_zones_page, 'eshopbox_admin_css' ); 
		add_action( 'admin_print_styles-' . $shipping_zones_page, 'eshopbox_shipping_zones_css' ); 
	}
	
	/**
	 * The shipping zones admin page
	 */
	function eshopbox_shipping_zones_page() {
	...
	function eshopbox_shipping_zones_css() {
		wp_enqueue_script( 'eshopbox_admin' );
		wp_enqueue_script( 'chosen' );
    	wp_enqueue_script( 'jquery-ui-sortable' );
		wp_enqueue_style( 'eshopbox_shipping_zone_admin_styles', plugins_url( '/assets/css/admin.css', dirname( __FILE__ ) ) );
	}
	...
	/**
	 * eshopbox_get_shipping_zone function.
	 * 
	 * @access public
	 * @param array $package
	 * @return WC_Shipping_Zone
	 */
	function eshopbox_get_shipping_zone( $package ) {
		...
		if ( $postcode ) {
			$postcode_ranges = $wpdb->get_results( "
				SELECT * FROM {$wpdb->prefix}eshopbox_shipping_zone_locations
				WHERE location_type = 'postcode' AND location_code LIKE '%-%'
			" );
				
			if ( $postcode_ranges ) {
				
				$encoded_postcode = eshopbox_make_numeric_postcode( $postcode );
				...
				foreach ( $postcode_ranges as $postcode_range ) {
					...
					$min = eshopbox_make_numeric_postcode( $range[0] );
					$max = eshopbox_make_numeric_postcode( $range[1] );
		...
		$matching_zone = $wpdb->get_var( $wpdb->prepare( "
			SELECT zones.zone_id FROM {$wpdb->prefix}eshopbox_shipping_zones as zones
			LEFT JOIN {$wpdb->prefix}eshopbox_shipping_zone_locations as locations ON zones.zone_id = locations.zone_id
			WHERE 
			(
				(
					zone_type = 'countries'
				...
				(
					...
					(
						zones.zone_id IN (
							SELECT zone_id FROM {$wpdb->prefix}eshopbox_shipping_zone_locations
	 ...
	 * @return void
	 */
	function eshopbox_make_numeric_postcode( $postcode ) {
