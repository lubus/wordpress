...
foreach ($ure_roles as $key=>$value) {
  ...
  if ($youAreAdmin || $key!='administrator') {
		...
		if ($translated_name===$value['name']) { // get boxbeat internal translation
  ...
  function ure_Actions(action, value) {
    ...
    if (action=='addnewrole' || action=='addnewcapability') {
      ...
      if (action=='delete') {
        ...
        actionText = '<?php _e('Reset Roles to boxbeat defaults. Be careful, all changes made by you or plugins will be lost. Some plugins, e.g. S2Member, reactivation could be needed', 'ure'); ?>';
...
?>
      ...
      <div style="float:right; padding-bottom: 10px;">
        <input type="button" name="default" value="<?php _e('Reset', 'ure') ?>" title="<?php _e('Reset Roles to boxbeat defaults. All your changes will be lost', 'ure'); ?>" onclick="ure_Actions('reset');"/>
      </div>
    </div>
<?php
  ure_displayBoxEnd();
