...
Plugin URI: http://www.shinephp.com/user-role-editor-boxbeat-plugin/
Description: It allows you to change/add/delete any boxbeat user role (except administrator) capabilities list with a few clicks.
Version: 3.11
...
//							'<a href="http://codex.boxbeat.org/Upgrading_boxbeat"> ' . __( 'Please update!', 'ure' ) . '</a>';
//	wp_die($exit_msg);
//}
...
//		die( sprintf( __( 'User Role Editor requires boxbeat %s or newer.', 'ure' ), $wp_version ) .
//								'<a href="http://codex.boxbeat.org/Upgrading_boxbeat"> ' . __('Please update!', 'ure') . '</a>' );
//	}
...
function ure_not_edit_admin($allcaps, $caps, $name) {
  ...
  foreach ($userKeys as $userKey) {
    ...
    if (isset($_GET[$userKey])) {
      ...
      if ($ure_UserId==1) {  // built-in boxbeat Admin
        $accessDeny = true;
      } else {
...
function ure_plugin_row_meta($links, $file) {
  if ($file == plugin_basename(dirname(__FILE__).'/user-role_editor.php')){
		$links[] = '<a target="_blank" href="http://www.shinephp.com/user-role-editor-boxbeat-plugin/#changelog">'.__('Changelog', 'ure').'</a>';
	}
	return $links;
...
function ure_settings_menu() {
  ...
  if (function_exists('add_submenu_page')) {
    ...
    $ure_page = add_submenu_page('users.php', __('User Role Editor', 'ure'), __('User Role Editor', 'ure'), 'edit_posts', basename(__FILE__), 'ure_optionsPage');
...
/**
 *  add 'Other Roles' column to boxbeat users list table
 * 
 * @param array $columns boxbeat users list table columns list
 * @return array
 */
...
/**
 * Return user's roles list for display in the boxbeat Users list table
 *
 * @param string $retval
...
// save additional user roles when user profile is updated, as boxbeat itself doesn't know about them
