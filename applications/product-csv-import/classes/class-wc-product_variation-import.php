...
class WC_CSV_Product_Variation_Import extends WC_CSV_Product_Import {
	...
	public function __construct() {
		...
		$this->import_page = 'eshopbox_variation_csv';
	}
	...
	function process_product( $post ) {
		...
		$postdata = array( 'ID' => $post_id );
		$postdata['post_title'] = sprintf( __('Variation #%s of %s', 'eshopbox'), $post_id, get_the_title( $post_parent ) );
		...
		if ( ! empty( $post['postmeta'] ) ) {
           // echo "<pre>";print_r($post['postmeta']);echo "</pre>";exit;
			$sale_price = 0;
			foreach ( $post['postmeta'] as $meta ) {
				...
				if ( $key ) {
					...
					$value = maybe_unserialize( $meta['value'] );
						...
						if($key=='_sale_price'){
						//-- update_post_meta( $post['post_parent'], '_sale_price',  $meta['value'] );
							$sale_price = $meta['value'];
							update_post_meta( $post['post_parent'], '_price',  $meta['value'] );
						}
						if($key=='_regular_price'){
							update_post_meta( $post['post_parent'], '_min_variation_regular_price',  $meta['value'] );
						}
				}
			}
			//-- Custom code ---
		...
		if ( ! empty( $post['images'] ) ) {
			...
			if ( $post['images'] ) foreach ( $post['images'] as $image ) {
				...
				if ( ! is_wp_error( $attachment_id ) ) {
					...
					update_post_meta( $attachment_id, '_eshopbox_exclude_image', 0 );
	...
	function header() {
		echo '<div class="wrap"><div class="icon32" id="icon-eshopbox-importer"><br></div>';
	...
	function greet() {
		...
		//wp_import_upload_form( 'admin.php?import=eshopbox_variation_csv&amp;step=1&amp;merge=' . ( ! empty( $_GET['merge'] ) ? 1 : 0 ) );

		$action = 'admin.php?import=eshopbox_variation_csv&amp;step=1&amp;merge=' . ( ! empty( $_GET['merge'] ) ? 1 : 0 );
