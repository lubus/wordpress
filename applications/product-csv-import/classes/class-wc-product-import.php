...
class WC_CSV_Product_Import extends WP_Importer {
	...
	public function __construct() {
		...
		$this->import_page = 'eshopbox_csv';

		$this->file_url_import_enabled = apply_filters('eshopbox_csv_product_file_url_import_enabled', true);
	...
	function dispatch() {
		...
		switch ( $step ) {
			...
			case 1:
				...
				break;
			case 2:
				check_admin_referer( 'import-eshopbox' );
...
<form action="<?php echo admin_url( 'admin.php?import=' . $this->import_page . '&step=2&merge=' . $merge ); ?>" method="post">
	<?php wp_nonce_field( 'import-eshopbox' ); ?>
					...
					<?php
						if ( strstr( $key, 'tax:' ) ) {
							...
							<select name="map_to[<?php echo $key; ?>]">
								...
								<?php if( function_exists( 'eshopbox_gpf_install' ) ) : ?>
...
<?php
	...
	function process_product( $post ) {
		...
		if ( ! empty( $post['gpf_data'] ) && is_array( $post['gpf_data'] ) ) {
			...
			update_post_meta( $post_id, '_eshopbox_gpf_data', $post['gpf_data'] );
	...
	function greet() {
		...
		//wp_import_upload_form( 'admin.php?import=eshopbox_csv&amp;step=1&amp;merge=' . ( ! empty( $_GET['merge'] ) ? 1 : 0 ) );

		$action = 'admin.php?import=eshopbox_csv&amp;step=1&amp;merge=' . ( ! empty( $_GET['merge'] ) ? 1 : 0 );
