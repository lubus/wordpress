<?php
/**
 * eshopbox CSV Importer class for managing parsing of CSV files.
 */
class WC_CSV_Parser {
	...
	function parse_product( $item ) {
		...
		if ( $merging ) {
			...
			if ( ! $post_id && empty( $item['sku'] ) ) {
				...
				if ( $post_id ) {
					...
					/*$found_product_id = $wpdb->get_var( "SELECT ID FROM $wpdb->posts WHERE ID = " . absint( $post_id ) . " AND post_type='" . $this->post_type . "';" );
		...
		if ( $merging ) {
			...
			$gpf_data = array_filter( (array) maybe_unserialize( get_post_meta( $post_id, '_eshopbox_gpf_data', true ) ) );
		...
		foreach ( $item as $key => $value ) {
			...
			if ( $this->post_type == 'product' && ! $value ) continue;
			...
			elseif ( strstr( $key, 'tax:' ) ) {
				...
				if ( $taxonomy == 'product_type' ) {
					...
					if ( ! array_key_exists( $term, $this->allowed_product_types ) ) {
						$WC_CSV_Product_Import->log->add( sprintf( __('> > > Product type "%s" not allowed - using simple.', 'wc_csv_import'), $term ) );
			...
			elseif ( strstr( $key, 'attribute:' ) ) {
				...
				if ( substr( $attribute_key, 0, 3 ) == 'pa_' ) {
					...
					if ( ! taxonomy_exists( $taxonomy ) ) {
						...
						$exists_in_db = $wpdb->get_var( "SELECT attribute_id FROM " . $wpdb->prefix . "eshopbox_attribute_taxonomies WHERE attribute_name = '" . $nicename . "';" );

						if ( ! $exists_in_db ) {

							// Create the taxonomy
							$wpdb->insert( $wpdb->prefix . "eshopbox_attribute_taxonomies", array( 'attribute_name' => $nicename, 'attribute_type' => 'select' ), array( '%s', '%s' ) );
